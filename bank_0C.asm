                      ORG $0C8000

OWEventTileNum:       db $71,$71,$71,$71,$71,$71,$6E,$11
                      db $43,$43,$43,$11,$68,$3D,$69,$69
                      db $69,$3D,$6E,$3D,$43,$43,$43,$00
                      db $43,$00,$11,$3F,$11,$50,$20,$50
                      db $11,$3F,$3F,$11,$71,$11,$43,$43
                      db $43,$11,$71,$3D,$69,$69,$69,$3D
                      db $6E,$3D,$43,$43,$43,$3D,$68,$3D
                      db $3F,$3F,$3F,$3D,$6E,$3D,$3F,$3F
                      db $11,$00,$43,$00,$11,$3F,$11,$50
                      db $11,$43,$43,$11,$11,$20,$3D,$69
                      db $69,$00,$11,$50,$3D,$43,$43,$50
                      db $50,$11,$11,$10,$3F,$11,$50,$3D
                      db $71,$3D,$3F,$3F,$11,$3D,$6E,$3D
                      db $3F,$3F,$3F,$3D,$03,$03,$03,$03
                      db $03,$03,$11,$43,$43,$43,$11,$71
                      db $3D,$69,$69,$69,$3D,$11,$3D,$43
                      db $43,$43,$00,$11,$11,$10,$3F,$11
                      db $50,$50,$71,$11,$10,$3F,$11,$50
                      db $03,$60,$75,$60,$03,$03,$11,$00
                      db $43,$00,$11,$71,$3D,$50,$3F,$50
                      db $3D,$11,$3D,$3F,$3F,$11,$00,$11
                      db $11,$10,$3F,$11,$50,$50,$71,$11
                      db $10,$3F,$11,$50,$60,$75,$75,$75
                      db $60,$03,$3D,$43,$43,$43,$3D,$71
                      db $3D,$3F,$3F,$3F,$3D,$11,$3D,$3F
                      db $3F,$11,$00,$11,$71,$10,$3F,$11
                      db $50,$50,$71,$71,$10,$3F,$11,$50
                      db $69,$20,$69,$69,$20,$50,$50,$43
                      db $43,$43,$43,$11,$11,$75,$75,$75
                      db $75,$75,$75,$11,$10,$10,$11,$75
                      db $75,$11,$43,$43,$11,$75,$75,$75
                      db $75,$75,$75,$75,$69,$20,$69,$69
                      db $20,$50,$50,$43,$43,$43,$43,$11
                      db $11,$75,$10,$10,$75,$75,$75,$11
                      db $00,$00,$11,$75,$75,$11,$50,$50
                      db $11,$75,$75,$75,$11,$11,$75,$75
                      db $69,$20,$69,$69,$20,$50,$50,$43
                      db $43,$43,$43,$11,$11,$10,$03,$10
                      db $75,$75,$11,$00,$43,$00,$11,$75
                      db $11,$50,$69,$50,$11,$75,$75,$11
                      db $43,$11,$75,$75,$69,$20,$69,$69
                      db $20,$50,$50,$43,$43,$43,$43,$11
                      db $11,$10,$03,$03,$10,$75,$11,$00
                      db $43,$43,$00,$11,$11,$50,$69,$69
                      db $50,$11,$75,$11,$43,$43,$11,$75
                      db $11,$75,$75,$75,$75,$75,$75,$75
                      db $75,$75,$75,$75,$75,$75,$75,$75
                      db $75,$75,$75,$75,$75,$75,$75,$75
                      db $75,$75,$75,$75,$75,$75,$A6,$A7
                      db $75,$75,$75,$75,$20,$3D,$56,$57
                      db $71,$20,$20,$3D,$64,$65,$71,$50
                      db $20,$3D,$71,$71,$71,$71,$50,$3D
                      db $71,$71,$71,$71,$11,$3D,$12,$13
                      db $14,$15,$75,$3D,$49,$4A,$59,$5A
                      db $20,$3D,$56,$57,$71,$20,$20,$3D
                      db $64,$65,$71,$50,$20,$3D,$71,$71
                      db $71,$71,$50,$3D,$12,$13,$14,$15
                      db $11,$3D,$31,$32,$33,$34,$75,$3D
                      db $49,$4A,$59,$5A,$20,$3D,$56,$57
                      db $71,$20,$20,$3D,$64,$65,$71,$50
                      db $20,$3D,$12,$13,$14,$15,$50,$3D
                      db $31,$32,$33,$34,$11,$3D,$35,$36
                      db $37,$38,$75,$3D,$49,$4A,$59,$5A
                      db $3F,$3F,$3F,$3F,$3F,$3F,$3F,$3F
                      db $3F,$3F,$3F,$3F,$3F,$76,$76,$76
                      db $76,$3F,$76,$76,$F0,$F1,$76,$76
                      db $76,$76,$F1,$F0,$76,$76,$3F,$76
                      db $76,$76,$76,$3F,$3F,$3F,$3F,$3F
                      db $3F,$3F,$3F,$3F,$3F,$3F,$3F,$3F
                      db $3F,$76,$76,$76,$76,$3F,$76,$F1
                      db $F0,$F1,$F1,$76,$76,$F0,$F1,$F0
                      db $F0,$76,$3F,$76,$76,$76,$76,$3F
                      db $3F,$3F,$3F,$3F,$3F,$3F,$3F,$3F
                      db $3F,$3F,$3F,$3F,$3F,$76,$76,$76
                      db $76,$3F,$76,$E3,$11,$11,$E3,$76
                      db $76,$43,$11,$11,$43,$76,$3F,$76
                      db $76,$76,$76,$3F,$3F,$3F,$3F,$3F
                      db $3F,$3F,$3F,$3F,$3F,$3F,$3F,$3F
                      db $3F,$76,$76,$76,$76,$3F,$76,$E3
                      db $E4,$E4,$E3,$76,$76,$F1,$F1,$F1
                      db $F1,$76,$3F,$76,$76,$76,$76,$3F
                      db $3F,$3F,$3F,$3F,$3F,$3F,$3F,$3F
                      db $3F,$3F,$3F,$3F,$76,$E3,$E4,$E4
                      db $E3,$76,$76,$FE,$FF,$FF,$FE,$76
                      db $76,$11,$F1,$F1,$11,$76,$76,$76
                      db $76,$76,$76,$76,$3F,$3F,$3F,$3F
                      db $3F,$3F,$3F,$3F,$3F,$3F,$3F,$3F
                      db $76,$E3,$E4,$E4,$E3,$76,$F2,$F3
                      db $F4,$F4,$F3,$F2,$F0,$F1,$F0,$F1
                      db $F0,$F1,$76,$76,$76,$76,$76,$76
                      db $3F,$3F,$3F,$3F,$3F,$3F,$3F,$E3
                      db $E4,$E4,$E3,$3F,$F2,$F3,$F4,$F4
                      db $F3,$F2,$CE,$CF,$CD,$CD,$CF,$CE
                      db $11,$DE,$DF,$DF,$DE,$11,$76,$76
                      db $76,$76,$76,$76,$3F,$E3,$E4,$E4
                      db $E3,$3F,$F2,$F3,$F4,$F4,$F3,$F2
                      db $CB,$CC,$CD,$CD,$CC,$CB,$DB,$DC
                      db $DD,$DD,$DC,$DB,$ED,$DE,$DF,$DF
                      db $DE,$ED,$76,$76,$76,$76,$76,$76
                      db $3F,$E3,$E4,$E4,$E3,$3F,$F2,$F3
                      db $F4,$F4,$F3,$F2,$CB,$CC,$CD,$CD
                      db $CC,$CB,$DB,$DC,$DD,$DD,$DC,$DB
                      db $FB,$FC,$FD,$FD,$FC,$FB,$ED,$EE
                      db $EF,$EF,$EE,$ED,$3F,$E3,$E4,$E4
                      db $E3,$3F,$F2,$F3,$F4,$F4,$F3,$F2
                      db $CB,$CC,$CD,$CD,$CC,$CB,$DB,$DC
                      db $DD,$DD,$DC,$DB,$EB,$EC,$BB,$BB
                      db $EC,$EB,$FB,$FC,$FD,$FD,$FC,$FB
                      db $3F,$3F,$3F,$3F,$3F,$3F,$3F,$E3
                      db $E4,$E4,$E3,$3F,$F2,$F3,$F4,$F4
                      db $F3,$F2,$CB,$CC,$CD,$CD,$CC,$CB
                      db $DB,$DC,$DD,$DD,$DC,$DB,$FB,$FC
                      db $FD,$FD,$FC,$FB,$3F,$50,$20,$69
                      db $69,$69,$3F,$10,$42,$41,$69,$69
                      db $03,$11,$3C,$4F,$50,$69,$5D,$5D
                      db $6B,$6B,$6E,$43,$5D,$5D,$6B,$6B
                      db $6D,$6D,$4F,$4F,$3C,$3C,$4F,$6E
                      db $75,$75,$75,$75,$75,$75,$E2,$E5
                      db $B2,$B3,$75,$75,$84,$EA,$D4,$D5
                      db $75,$75,$35,$36,$37,$38,$75,$75
                      db $35,$36,$37,$38,$01,$75,$49,$4A
                      db $59,$5A,$11,$10,$69,$69,$69,$69
                      db $69,$69,$69,$50,$43,$43,$50,$69
                      db $69,$DA,$8A,$8A,$3D,$69,$43,$DA
                      db $8A,$8A,$3D,$43,$75,$00,$43,$43
                      db $00,$75,$75,$20,$20,$20,$20,$75
                      db $69,$69,$69,$69,$69,$69,$69,$69
                      db $69,$69,$69,$69,$69,$50,$43,$43
                      db $50,$69,$43,$DA,$8A,$8A,$3D,$43
                      db $75,$DA,$8A,$8A,$3D,$75,$75,$00
                      db $43,$43,$00,$75,$69,$69,$69,$69
                      db $69,$69,$69,$69,$69,$69,$69,$69
                      db $69,$69,$69,$69,$69,$69,$43,$10
                      db $43,$43,$10,$43,$75,$DA,$8A,$8A
                      db $3D,$75,$75,$DA,$8A,$8A,$3D,$75
                      db $43,$43,$43,$43,$43,$43,$75,$75
                      db $75,$75,$75,$75,$75,$75,$75,$75
                      db $75,$75,$75,$75,$75,$75,$75,$75
                      db $75,$75,$75,$75,$75,$75,$75,$75
                      db $75,$75,$75,$75,$10,$43,$9A,$9A
                      db $10,$43,$3D,$71,$6A,$6A,$3D,$75
                      db $00,$43,$43,$43,$00,$75,$20,$20
                      db $69,$20,$20,$11,$50,$20,$69,$20
                      db $50,$11,$11,$43,$43,$43,$11,$75
                      db $50,$43,$9A,$9A,$50,$69,$3D,$71
                      db $6A,$6A,$3D,$43,$DA,$AD,$5D,$5D
                      db $3D,$75,$DA,$AD,$5D,$5D,$3D,$75
                      db $00,$43,$43,$43,$00,$11,$50,$20
                      db $69,$20,$50,$11,$50,$43,$9A,$9A
                      db $50,$69,$3D,$71,$6A,$6A,$3D,$69
                      db $DA,$AD,$5D,$5D,$3D,$43,$DA,$AD
                      db $5D,$5D,$3D,$75,$00,$43,$43,$43
                      db $00,$75,$20,$20,$69,$20,$20,$11
                      db $50,$43,$9A,$9A,$50,$69,$3D,$71
                      db $6A,$6A,$3D,$69,$DA,$AD,$5D,$5D
                      db $3D,$69,$DA,$AD,$5D,$5D,$3D,$43
                      db $00,$43,$43,$43,$00,$75,$20,$20
                      db $69,$20,$20,$75,$50,$43,$9A,$9A
                      db $50,$69,$3D,$71,$6A,$6A,$3D,$69
                      db $DA,$AD,$5D,$5D,$3D,$69,$DA,$AD
                      db $5D,$5D,$3D,$69,$00,$43,$43,$43
                      db $00,$43,$20,$20,$69,$20,$20,$75
                      db $6E,$3D,$A4,$A4,$A4,$3D,$68,$3D
                      db $B4,$B4,$B4,$3D,$6E,$3D,$73,$73
                      db $73,$00,$43,$00,$3F,$3F,$3F,$50
                      db $20,$50,$3F,$3F,$3F,$3F,$43,$3F
                      db $3F,$3F,$3F,$3F,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $6A,$6A,$5D,$5D,$6F,$5F,$6A,$6A
                      db $5F,$6F,$5F,$56,$6F,$5F,$5F,$6F
                      db $5F,$6F,$5F,$58,$6A,$6A,$5F,$6F
                      db $5D,$6B,$5D,$6B,$6B,$6D,$6B,$6E
                      db $5F,$6F,$6C,$58,$6A,$6A,$5F,$6F
                      db $5D,$5D,$5D,$5D,$6A,$6A,$5D,$5D
                      db $6C,$71,$5F,$6F,$68,$68,$52,$71
                      db $6A,$6A,$6A,$6A,$6D,$6B,$6E,$6B
                      db $6F,$5F,$30,$6C,$6A,$6A,$6F,$5F
                      db $4E,$4E,$6A,$6A,$4B,$3F,$3F,$4B
                      db $3F,$4B,$4B,$3F,$71,$71,$6C,$68
                      db $A6,$A7,$B6,$B7,$A6,$A7,$A9,$B7
                      db $6B,$6E,$6B,$5D,$5D,$6F,$6F,$5F
                      db $5F,$63,$5F,$6F,$5D,$68,$5D,$68
                      db $74,$68,$68,$74,$71,$68,$68,$68
                      db $43,$68,$71,$68,$3E,$3E,$43,$43
                      db $5D,$5D,$9A,$9A,$3F,$5B,$5B,$3F
                      db $5B,$3F,$3F,$5B,$6A,$6A,$68,$68
                      db $5D,$6B,$5D,$6B,$6B,$6E,$6B,$6E
                      db $6B,$6B,$6B,$6B,$9A,$9A,$3E,$3E
                      db $71,$71,$71,$71,$6B,$C5,$6B,$C5
                      db $4B,$3F,$4B,$3F,$6A,$6A,$9A,$9A
                      db $6D,$6B,$6E,$6B,$6F,$5F,$71,$6C
                      db $9A,$9A,$6F,$5F,$8C,$8D,$8C,$8D
                      db $6B,$6E,$6B,$6D,$6D,$6D,$71,$6E
                      db $6E,$11,$5D,$6F,$5D,$6F,$6A,$6A
                      db $5D,$5D,$6A,$6A,$68,$68,$5F,$6F
                      db $6D,$6C,$6E,$71,$71,$71,$71,$71
                      db $8C,$8D,$43,$43,$9A,$9A,$8C,$8D
                      db $71,$71,$71,$71,$6E,$68,$68,$68
                      db $71,$68,$6E,$68,$71,$68,$71,$68
                      db $6E,$71,$6E,$71,$10,$03,$6F,$6F
                      db $4B,$5B,$3F,$4B,$5B,$3F,$4B,$3F
                      db $4B,$10,$03,$71,$A4,$A4,$B4,$B4
                      db $A4,$A4,$B4,$B4,$6B,$5D,$6B,$68
                      db $68,$68,$74,$74,$74,$68,$70,$5D
                      db $3E,$3E,$3E,$3E,$9A,$9A,$6A,$6A
                      db $6A,$6A,$6F,$6F,$C5,$6B,$C5,$6B
                      db $5D,$5D,$5D,$5D,$6A,$6A,$6F,$5F
                      db $5F,$71,$6C,$68,$71,$11,$68,$68
                      db $43,$43,$68,$6C,$56,$57,$9E,$9F
                      db $54,$55,$9E,$9F,$67,$57,$9E,$9F
                      db $11,$11,$11,$11,$5F,$6F,$6A,$6A
                      db $6B,$5D,$6B,$5D,$2B,$2B,$6A,$6A
                      db $2B,$6E,$2B,$6E,$6F,$5F,$71,$6C
                      db $6A,$6A,$6F,$5F,$99,$6E,$6C,$6D
                      db $71,$6C,$6F,$5F,$6F,$5F,$6A,$6A
                      db $5F,$6F,$6C,$71,$71,$71,$71,$71
                      db $6F,$6F,$6A,$6A,$6A,$6A,$5F,$6F
                      db $6D,$6C,$6E,$89,$6B,$6B,$68,$6B
                      db $71,$71,$68,$68,$6B,$5D,$6B,$5D
                      db $6B,$6B,$6B,$6B,$5D,$5D,$5D,$5D
                      db $6A,$6A,$5D,$5D,$6B,$5D,$6B,$68
                      db $6A,$6A,$6A,$6A,$5D,$6B,$5D,$6B
                      db $71,$B6,$71,$71,$B7,$71,$71,$71
                      db $71,$71,$71,$A6,$71,$71,$A7,$71
                      db $71,$71,$A7,$A6,$9A,$9A,$6A,$6A
                      db $4B,$75,$4B,$75,$5B,$5B,$75,$75
                      db $5B,$3F,$75,$75,$76,$76,$76,$76
                      db $3F,$3F,$3F,$3F,$68,$5D,$68,$5D
                      db $6C,$4F,$6C,$68,$6C,$4F,$6C,$6C
                      db $5D,$68,$4F,$6C,$5D,$68,$5D,$5D
                      db $4F,$6C,$68,$6C,$4F,$6C,$6C,$6C
                      db $4F,$68,$6C,$6C,$5D,$68,$4F,$68
                      db $68,$4F,$6C,$6C,$5D,$5D,$68,$68
                      db $6E,$6D,$6D,$6E,$2B,$2B,$2B,$2B
                      db $6A,$6A,$2B,$2B,$91,$92,$A0,$A1
                      db $A2,$A3,$93,$94,$6A,$6A,$9A,$9A
                      db $C5,$6B,$C5,$6B,$C5,$C5,$C5,$C5
                      db $6B,$C5,$6B,$C5,$71,$71,$71,$A6
                      db $3D,$A4,$3D,$B4,$A4,$3D,$B4,$3D
                      db $61,$61,$61,$61,$5D,$5D,$5D,$68
                      db $71,$71,$68,$68,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA
                      db $BA,$BA,$BA,$BA,$BA,$BA,$BA,$BA

OWEventTileProp:      db $00,$10,$84,$50,$01,$10,$D0,$83
                      db $90,$01,$18,$50,$82,$5C,$02,$10
                      db $90,$50,$82,$14,$04,$50,$90,$10
                      db $94,$58,$81,$54,$00,$18,$81,$14
                      db $81,$58,$02,$54,$10,$D0,$83,$90
                      db $01,$10,$50,$82,$5C,$81,$10,$00
                      db $50,$82,$14,$02,$10,$18,$50,$82
                      db $58,$02,$10,$90,$50,$81,$58,$05
                      db $D4,$50,$90,$10,$94,$58,$81,$54
                      db $00,$D0,$82,$90,$02,$D0,$58,$50
                      db $81,$5C,$83,$50,$81,$14,$01,$54
                      db $90,$82,$50,$00,$58,$81,$54,$81
                      db $10,$00,$50,$81,$58,$00,$54,$81
                      db $10,$00,$50,$82,$58,$86,$10,$00
                      db $D0,$83,$90,$01,$10,$50,$82,$5C
                      db $02,$10,$D0,$50,$82,$14,$83,$50
                      db $00,$58,$81,$54,$01,$90,$10,$81
                      db $50,$00,$58,$81,$54,$02,$10,$50
                      db $1C,$82,$10,$0C,$D0,$10,$14,$50
                      db $90,$10,$50,$14,$58,$54,$10,$D0
                      db $50,$81,$58,$00,$D4,$83,$50,$00
                      db $58,$81,$54,$01,$90,$10,$81,$50
                      db $00,$58,$81,$54,$00,$50,$82,$1C
                      db $81,$10,$00,$50,$82,$14,$81,$10
                      db $00,$50,$82,$58,$02,$10,$D0,$50
                      db $81,$58,$00,$D4,$81,$50,$02,$10
                      db $50,$58,$81,$54,$00,$90,$81,$10
                      db $01,$50,$58,$81,$54,$01,$1C,$58
                      db $82,$1C,$83,$14,$00,$54,$82,$14
                      db $85,$1C,$03,$D4,$10,$50,$94,$81
                      db $1C,$00,$54,$82,$14,$87,$1C,$00
                      db $58,$82,$1C,$83,$14,$00,$54,$82
                      db $14,$02,$1C,$10,$50,$82,$1C,$03
                      db $D4,$50,$10,$94,$81,$1C,$81,$54
                      db $81,$14,$82,$1C,$01,$54,$14,$82
                      db $1C,$00,$58,$82,$1C,$83,$14,$00
                      db $54,$82,$14,$81,$10,$00,$50,$81
                      db $1C,$05,$D4,$50,$90,$10,$94,$1C
                      db $81,$54,$00,$5C,$81,$14,$81,$1C
                      db $00,$54,$81,$14,$82,$1C,$00,$58
                      db $82,$1C,$83,$14,$00,$54,$82,$14
                      db $82,$10,$03,$50,$1C,$D4,$50,$81
                      db $90,$01,$10,$94,$81,$54,$81,$5C
                      db $81,$14,$01,$1C,$54,$82,$14,$01
                      db $1C,$14,$9C,$1C,$81,$14,$83,$1C
                      db $01,$54,$10,$81,$18,$03,$10,$50
                      db $54,$10,$81,$18,$02,$10,$50,$54
                      db $84,$10,$00,$54,$84,$10,$00,$54
                      db $84,$10,$00,$14,$84,$10,$01,$54
                      db $10,$81,$18,$03,$10,$50,$54,$10
                      db $81,$18,$02,$10,$50,$54,$84,$10
                      db $00,$54,$84,$10,$00,$54,$84,$10
                      db $00,$14,$84,$10,$01,$54,$10,$81
                      db $18,$03,$10,$50,$54,$10,$81,$18
                      db $02,$10,$50,$54,$84,$10,$00,$54
                      db $84,$10,$00,$54,$84,$10,$00,$14
                      db $84,$10,$8C,$58,$83,$14,$00,$58
                      db $85,$14,$81,$94,$81,$D4,$81,$94
                      db $00,$58,$83,$94,$8D,$58,$83,$14
                      db $04,$58,$14,$54,$14,$54,$81,$14
                      db $00,$94,$83,$D4,$01,$94,$58,$83
                      db $94,$8D,$58,$83,$14,$07,$58,$14
                      db $10,$94,$D4,$50,$14,$94,$81,$14
                      db $03,$54,$14,$94,$58,$83,$94,$8D
                      db $58,$83,$14,$01,$58,$14,$81,$10
                      db $81,$50,$04,$14,$94,$D4,$94,$D4
                      db $81,$94,$00,$58,$83,$94,$8C,$58
                      db $00,$14,$81,$10,$81,$50,$81,$14
                      db $81,$10,$81,$50,$05,$14,$94,$54
                      db $D4,$94,$14,$86,$94,$8B,$58,$00
                      db $14,$81,$10,$81,$50,$00,$14,$82
                      db $10,$82,$50,$8B,$94,$86,$58,$81
                      db $10,$81,$50,$00,$58,$82,$10,$82
                      db $50,$82,$10,$82,$50,$00,$54,$81
                      db $10,$81,$50,$00,$14,$85,$94,$00
                      db $58,$81,$10,$81,$50,$00,$58,$82
                      db $10,$82,$50,$82,$10,$82,$50,$82
                      db $10,$82,$50,$82,$10,$82,$50,$85
                      db $94,$00,$58,$81,$10,$81,$50,$00
                      db $58,$82,$10,$82,$50,$82,$10,$82
                      db $50,$82,$10,$82,$50,$82,$10,$82
                      db $50,$82,$10,$82,$50,$00,$58,$81
                      db $10,$81,$50,$00,$58,$82,$10,$82
                      db $50,$82,$10,$82,$50,$82,$10,$82
                      db $50,$81,$10,$81,$14,$81,$50,$82
                      db $10,$82,$50,$86,$58,$81,$10,$81
                      db $50,$00,$58,$82,$10,$82,$50,$82
                      db $10,$82,$50,$82,$10,$82,$50,$82
                      db $10,$82,$50,$01,$18,$54,$83,$50
                      db $81,$18,$81,$58,$81,$50,$83,$18
                      db $03,$58,$50,$18,$58,$83,$18,$01
                      db $98,$D8,$81,$98,$00,$D8,$85,$18
                      db $00,$D8,$85,$1C,$83,$10,$81,$1C
                      db $83,$10,$81,$1C,$83,$10,$81,$1C
                      db $83,$10,$01,$50,$5C,$83,$10,$81
                      db $50,$86,$1C,$82,$10,$00,$50,$81
                      db $1C,$81,$10,$81,$50,$01,$1C,$14
                      db $81,$90,$04,$D0,$50,$14,$1C,$50
                      db $81,$90,$00,$10,$81,$1C,$81,$58
                      db $81,$18,$8D,$1C,$82,$10,$02,$50
                      db $1C,$14,$81,$10,$81,$50,$01,$14
                      db $1C,$81,$90,$01,$D0,$50,$81,$1C
                      db $00,$50,$81,$90,$00,$10,$92,$1C
                      db $00,$14,$82,$10,$02,$50,$14,$1C
                      db $81,$10,$81,$50,$81,$1C,$81,$90
                      db $02,$D0,$50,$1C,$85,$14,$9D,$1C
                      db $82,$10,$81,$50,$00,$14,$82,$10
                      db $81,$50,$01,$1C,$50,$82,$90,$01
                      db $10,$1C,$81,$58,$00,$50,$81,$18
                      db $04,$94,$54,$58,$50,$18,$81,$14
                      db $00,$54,$83,$14,$00,$1C,$82,$10
                      db $81,$50,$00,$1C,$82,$10,$81,$50
                      db $00,$14,$82,$10,$81,$50,$00,$1C
                      db $82,$90,$03,$D0,$50,$1C,$50,$82
                      db $90,$05,$10,$94,$54,$58,$50,$18
                      db $81,$14,$82,$10,$81,$50,$00,$1C
                      db $82,$10,$81,$50,$00,$1C,$82,$10
                      db $81,$50,$00,$14,$82,$90,$03,$D0
                      db $50,$1C,$50,$82,$90,$01,$10,$1C
                      db $81,$58,$00,$50,$81,$18,$00,$94
                      db $82,$10,$81,$50,$00,$1C,$82,$10
                      db $81,$50,$00,$1C,$82,$10,$81,$50
                      db $00,$1C,$82,$90,$03,$D0,$50,$14
                      db $50,$82,$90,$01,$10,$1C,$81,$58
                      db $00,$50,$81,$18,$00,$1C,$82,$10
                      db $81,$50,$00,$1C,$82,$10,$81,$50
                      db $00,$1C,$82,$10,$81,$50,$00,$1C
                      db $82,$90,$03,$D0,$50,$1C,$50,$82
                      db $90,$01,$10,$14,$81,$58,$00,$50
                      db $81,$18,$02,$1C,$10,$50,$82,$18
                      db $02,$10,$18,$50,$82,$18,$02,$10
                      db $90,$50,$82,$1C,$02,$50,$90,$10
                      db $82,$58,$02,$54,$18,$14,$83,$58
                      db $00,$14,$84,$58,$FF,$0C,$FF,$0C
                      db $FF,$0C,$FF,$0C,$FF,$0C,$FF,$0C
                      db $FF,$0C,$CB,$0C,$02,$10,$50,$10
                      db $81,$50,$01,$90,$10,$81,$50,$81
                      db $90,$05,$18,$50,$90,$50,$90,$50
                      db $81,$90,$81,$10,$81,$50,$06,$90
                      db $50,$10,$D0,$90,$10,$50,$81,$90
                      db $00,$50,$81,$90,$81,$10,$81,$50
                      db $08,$90,$10,$50,$90,$D0,$10,$50
                      db $10,$50,$81,$10,$00,$D0,$82,$10
                      db $00,$50,$81,$10,$02,$50,$10,$50
                      db $81,$10,$08,$D0,$90,$D0,$10,$50
                      db $D0,$10,$50,$D0,$81,$10,$03,$50
                      db $10,$50,$54,$81,$58,$04,$14,$58
                      db $14,$54,$58,$81,$10,$01,$50,$18
                      db $87,$14,$81,$10,$82,$90,$01,$10
                      db $D0,$81,$10,$01,$50,$D0,$82,$10
                      db $00,$90,$81,$10,$81,$18,$00,$90
                      db $87,$10,$01,$18,$58,$81,$14,$08
                      db $90,$D0,$90,$D0,$58,$14,$94,$58
                      db $14,$81,$58,$02,$94,$10,$50,$81
                      db $10,$03,$50,$10,$D0,$90,$81,$10
                      db $81,$90,$81,$50,$81,$D0,$03,$90
                      db $D0,$18,$58,$84,$18,$0A,$58,$98
                      db $D8,$54,$18,$14,$18,$10,$50,$90
                      db $D0,$81,$18,$02,$D8,$98,$D8,$81
                      db $18,$04,$D8,$18,$58,$D8,$18,$85
                      db $10,$00,$90,$81,$D0,$81,$10,$05
                      db $D0,$10,$50,$90,$10,$90,$81,$10
                      db $04,$50,$90,$D0,$10,$50,$81,$10
                      db $02,$50,$90,$50,$81,$90,$88,$10
                      db $01,$90,$D0,$85,$10,$00,$50,$81
                      db $18,$82,$10,$00,$50,$84,$10,$02
                      db $50,$10,$D0,$81,$10,$81,$50,$09
                      db $10,$54,$14,$18,$54,$14,$18,$54
                      db $18,$54,$8A,$18,$02,$50,$10,$D0
                      db $85,$10,$81,$50,$0C,$D0,$18,$58
                      db $18,$58,$10,$50,$10,$50,$10,$50
                      db $D0,$90,$81,$18,$81,$98,$06,$18
                      db $58,$98,$D8,$18,$58,$D8,$82,$18
                      db $03,$D8,$10,$18,$58,$81,$10,$82
                      db $18,$00,$98,$8B,$18,$14,$D4,$94
                      db $54,$14,$D0,$10,$90,$D0,$50,$10
                      db $D0,$90,$94,$D4,$10,$50,$54,$10
                      db $D4,$90,$D0,$81,$10,$01,$D0,$90
                      db $81,$D0,$00,$10,$82,$50,$00,$90
                      db $82,$50,$01,$90,$50,$81,$90,$01
                      db $D0,$50,$81,$90,$84,$10,$00,$50
                      db $81,$10,$01,$50,$10,$81,$50,$01
                      db $90,$50,$81,$90,$00,$10,$81,$50
                      db $01,$10,$D0,$83,$10,$81,$18,$81
                      db $98,$81,$18,$81,$98,$07,$18,$58
                      db $98,$D8,$18,$58,$18,$58,$81,$18
                      db $04,$98,$10,$18,$58,$18,$81,$58
                      db $02,$18,$D8,$98,$94,$18,$08,$58
                      db $18,$58,$54,$1C,$14,$1C,$14,$94
                      db $81,$1C,$01,$14,$58,$81,$1C,$81
                      db $14,$81,$94,$83,$58,$03,$10,$50
                      db $10,$D0,$81,$10,$00,$D0,$82,$10
                      db $02,$D0,$10,$90,$81,$10,$05,$D0
                      db $10,$50,$90,$D0,$10,$81,$50,$01
                      db $90,$10,$81,$50,$00,$90,$81,$10
                      db $00,$50,$81,$90,$00,$10,$82,$50
                      db $01,$10,$D0,$81,$10,$00,$50,$81
                      db $10,$81,$50,$81,$90,$07,$14,$54
                      db $94,$D4,$10,$50,$14,$54,$88,$10
                      db $0A,$50,$90,$D0,$10,$50,$90,$D0
                      db $58,$10,$D8,$90,$81,$58,$81,$D8
                      db $83,$18,$02,$50,$18,$50,$81,$18
                      db $0A,$10,$18,$10,$9C,$DC,$1C,$5C
                      db $18,$58,$98,$10,$81,$18,$81,$10
                      db $FF,$0C,$FF,$0C,$FF,$0C,$BB,$0C
                      db $FF,$FF,$FF,$FF,$FF,$FF,$FF,$FF
                      db $FF,$FF,$FF,$FF,$FF,$FF,$FF,$FF
                      db $FF,$FF,$FF,$FF,$FF,$FF,$FF,$FF
EraseAllStripe:       db $20,$00,$5F,$FE,$F8,$00,$30,$00
                      db $5F,$FE,$FA,$08,$FF

CODE_0C938D:          PHB                                       ; Wrapper
                      PHK
                      PLB
                      SEP #$30                                  ; AXY->8
                      JSR CODE_0C9F6A
                      JSR CODE_0CAB1F
                      PLB
                      RTL

CODE_0C939A:          PHB                                       ; Wrapper
                      PHK
                      PLB
                      JSR CODE_0CA0E3
                      JSR CODE_0CA1D4
                      PLB
                      RTL

CODE_0C93A5:          PHB                                       ; Wrapper
                      PHK
                      PLB
                      JSR CODE_0CAEAD
                      PLB
                      RTL

CODE_0C93AD:          PHB
                      PHK
                      PLB
                      DEC.W Layer3ScrollYSpeed+1
                      BNE +
                      LDA.B #$23
                      STA.W GameMode
                      LDA.B #$FF
                      STA.W CreditsScreenNumber
                    + PLB
                      RTL


DATA_0C93C1:          dw DATA_0CF45A
                      dw DATA_0CE7C0
                      dw DATA_0CEC82
                      dw DATA_0CDD44
                      dw DATA_0CD900
                      dw DATA_0CDF59
                      dw DATA_0CDE54

DATA_0C93CF:          dw $FFFF
                      dw $FEFF
                      dw $FEFF
                      dw $FEFF
                      dw $FFFF
                      dw $FEFF
                      dw $FEFF

CODE_0C93DD:          REP #$30                                  ; AXY->16
                      STZ.W LevelLoadObject
CODE_0C93E2:          LDA.W LevelLoadObject
                      AND.W #$00FF
                      ASL A
                      TAX
                      LDA.L DATA_0C93C1,X
                      STA.B Layer2DataPtr
                      LDA.L DATA_0C93CF,X
                      STA.B Layer1DataPtr
                      SEP #$20                                  ; A->8
                      LDY.W #$0000
                      LDX.B Layer2DataPtr
                      CPX.W #DATA_0CE8FE
                      BCC +
                      LDY.W #$0001
                    + LDX.W #$0000
                      TYA
                    - STA.L Layer2TilemapHigh,X
                      STA.L Layer2TilemapHigh+$200,X
                      INX
                      CPX.W #$0200
                      BNE -
                      LDA.B #$0C
                      STA.B Layer2DataPtr+2
                      LDX.W #$B900
                      STX.B _D
                      REP #$20                                  ; A->16
                      JSR CODE_0C944C
                      JSR CODE_0C94C0
                      INC.W LevelLoadObject
                      LDA.W LevelLoadObject
                      CMP.W #$0007
                      BNE CODE_0C93E2
                      LDA.W #$5840
                      STA.B Layer1DataPtr
                      SEP #$30                                  ; AXY->8
                      STZ.B Layer1DataPtr+2
                      STZ.W LevelLoadObject
                      JSL CODE_0C9567
                      JSR CODE_0CA051
                      LDA.B #!BGM_STAFFCREDITS
                      STA.W SPCIO2                              ; / Change music
                      RTL

CODE_0C944C:          REP #$30                                  ; AXY->16
                      LDY.W #$0000
                      STY.B _3
                      STY.B _5
                      SEP #$20                                  ; A->8
                      LDA.B #$7E
                      STA.B _F
CODE_0C945B:          LDY.B _3
                      LDA.B [Layer2DataPtr],Y
                      STA.B _7
                      INY
                      STY.B _3
                      AND.B #$80
                      BEQ CODE_0C9480
                      LDA.B _7
                      AND.B #$7F
                      STA.B _7
                      LDA.B [Layer2DataPtr],Y
                      INY
                      STY.B _3
                      LDY.B _5
                    - STA.B [_D],Y
                      INY
                      DEC.B _7
                      BPL -
                      STY.B _5
                      BRA CODE_0C9492

CODE_0C9480:          LDY.B _3
                      LDA.B [Layer2DataPtr],Y
                      INY
                      STY.B _3
                      LDY.B _5
                      STA.B [_D],Y
                      INY
                      STY.B _5
                      DEC.B _7
                      BPL CODE_0C9480
CODE_0C9492:          LDY.B _3
                      LDA.B [Layer2DataPtr],Y
                      CMP.B #$FF
                      BNE CODE_0C945B
                      INY
                      LDA.B [Layer2DataPtr],Y
                      CMP.B #$FF
                      BNE CODE_0C945B
                      REP #$20                                  ; A->16
                      LDA.W #Map16BGTiles
                      STA.B _0
                      LDX.W #$0000
                    - LDA.B _0
                      STA.W Map16Pointers,X
                      LDA.B _0
                      CLC
                      ADC.W #$0008
                      STA.B _0
                      INX
                      INX
                      CPX.W #$0400
                      BNE -
                      RTS

CODE_0C94C0:          SEP #$20                                  ; A->8
                      REP #$10                                  ; XY->16
                      LDX.W #$B900
                      STX.B Map16LowPtr
                      LDA.B #$7E
                      STA.B Map16LowPtr+2
                      STA.B Map16HighPtr+2
                      LDX.W #$BD00
                      STX.B Map16HighPtr
                      LDA.B #$0D
                      STA.B Layer2DataPtr+2
                      LDY.W #$00F0
                      STY.B _4
                      LDA.W LevelLoadObject
                      XBA
                      AND.B #$00
                      TAX
                      STX.B _0
                      LDX.W #$007F
                      STX.B _8
CODE_0C94EB:          SEP #$20                                  ; A->8
                      LDY.B _4
                      LDA.B [Map16LowPtr],Y
                      STA.B _2
                      LDA.B [Map16HighPtr],Y
                      STA.B _3
                      REP #$20                                  ; A->16
                      LDA.B _2
                      ASL A
                      TAX
                      LDA.W Map16Pointers,X
                      STA.B Layer2DataPtr
                      LDY.W #$0000
                      LDA.B _0
                      ASL A
                      ASL A
                      PHA
                      AND.W #$003F
                      STA.B _6
                      PLA
                      AND.W #$FFC0
                      ASL A
                      ORA.B _6
                      TAX
                      LDA.B [Layer2DataPtr],Y
                      AND.B Layer1DataPtr
                      STA.L OWLayer2Tilemap,X
                      INY
                      INY
                      LDA.B [Layer2DataPtr],Y
                      AND.B Layer1DataPtr
                      STA.L OWLayer2Tilemap+$40,X
                      INY
                      INY
                      LDA.B [Layer2DataPtr],Y
                      AND.B Layer1DataPtr
                      STA.L OWLayer2Tilemap+2,X
                      INY
                      INY
                      LDA.B [Layer2DataPtr],Y
                      AND.B Layer1DataPtr
                      STA.L OWLayer2Tilemap+$42,X
                      INC.B _0
                      INC.B _4
                      DEC.B _8
                      BPL CODE_0C94EB
                      LDA.W #$007F
                      STA.B _8
                      LDA.B _4
                      CMP.W #$0170
                      BNE Return0C9558
                      LDA.W #$02A0
                      STA.B _4
                      BRA CODE_0C94EB

Return0C9558:         RTS


DATA_0C9559:          db $01,$18
                      dl OWLayer2Tilemap
                      dw $0400

DATA_0C9560:          db $01,$18
                      dl OWLayer2Tilemap+$400
                      dw $0400

CODE_0C9567:          SEP #$30                                  ; AXY->8
                      PHB
                      PHK
                      PLB
                      LDA.B #$80
                      STA.W HW_VMAINC
                      LDA.B #$C0
                      STA.W HW_VMADD
                      LDA.B #$30
                      STA.W HW_VMADD+1
                      LDY.B #$06
                    - LDA.W DATA_0C9559,Y
                      STA.W HW_DMAPARAM+$10,Y
                      DEY
                      BPL -
                      LDA.W LevelLoadObject
                      ASL A
                      ASL A
                      ASL A
                      ORA.W HW_DMAADDR+$11
                      STA.W HW_DMAADDR+$11
                      LDA.B #$02
                      STA.W HW_MDMAEN
                      LDA.B #$80
                      STA.W HW_VMAINC
                      LDA.B #$C0
                      STA.W HW_VMADD
                      LDA.B #$34
                      STA.W HW_VMADD+1
                      LDY.B #$06
                    - LDA.W DATA_0C9560,Y
                      STA.W HW_DMAPARAM+$10,Y
                      DEY
                      BPL -
                      LDA.W LevelLoadObject
                      ASL A
                      ASL A
                      ASL A
                      ORA.W HW_DMAADDR+$11
                      STA.W HW_DMAADDR+$11
                      LDA.B #$02
                      STA.W HW_MDMAEN
                      STZ.W CreditsUpdateBG
                      PLB
                      RTL


DATA_0C95C7:          db $07,$23,$0E,$3C,$21,$3C,$0E,$3C
                      db $0C,$3C,$1E,$3C,$1D,$3C,$12,$3C
                      db $1F,$3C,$0E,$3C,$FC,$00,$19,$3C
                      db $1B,$3C,$18,$3C,$0D,$3C,$1E,$3C
                      db $0C,$3C,$0E,$3C,$1B,$3C,$FF

                   if ver_is_japanese(!_VER)                    ;\======================= J =====================
                      db $08,$1F,$C7,$38,$C8,$38,$E1,$38        ;!
                      db $CE,$38,$E2,$38,$C7,$38,$C8,$38        ;!
                      db $FC,$00,$E8,$38,$C0,$38,$CC,$38        ;!
                      db $C0,$38,$E4,$38,$C2,$38,$C7,$38        ;!
                      db $C8,$38,$08,$1F,$D7,$38,$D8,$38        ;!
                      db $F1,$38,$DE,$38,$F2,$38,$D7,$38        ;!
                      db $D8,$38,$FC,$00,$F8,$38,$D0,$38        ;!
                      db $DC,$38,$D0,$38,$F4,$38,$D2,$38        ;!
                      db $D7,$38,$D8,$38,$0C,$0F,$19,$28        ;!
                      db $1B,$28,$18,$28,$0D,$28,$1E,$28        ;!
                      db $0C,$28,$0E,$28,$1B,$28,$08,$1F        ;!
                      db $E2,$38,$C7,$38,$C8,$38,$C6,$38        ;!
                      db $C4,$38,$E1,$38,$E4,$38,$FC,$00        ;!
                      db $CC,$38,$C8,$38,$E8,$38,$C0,$38        ;!
                      db $CC,$38,$CE,$38,$E3,$38,$CE,$38        ;!
                      db $08,$1F,$F2,$38,$D7,$38,$D8,$38        ;!
                      db $D6,$38,$D4,$38,$F1,$38,$F4,$38        ;!
                      db $FC,$00,$DC,$38,$D8,$38,$F8,$38        ;!
                      db $D0,$38,$DC,$38,$DE,$38,$F3,$38        ;!
                      db $DE,$38,$09,$1B,$1D,$28,$18,$28        ;!
                      db $1D,$28,$0A,$28,$15,$28,$FC,$00        ;!
                      db $0D,$28,$12,$28,$1B,$28,$0E,$28        ;!
                      db $0C,$28,$1D,$28,$18,$28,$1B,$28        ;!
                      db $09,$1B,$E3,$38,$C0,$38,$CA,$38        ;!
                      db $C0,$38,$E2,$38,$C7,$38,$C8,$38        ;!
                      db $FC,$00,$E3,$38,$C4,$38,$E9,$38        ;!
                      db $E4,$38,$CA,$38,$C0,$38,$09,$1B        ;!
                      db $F3,$38,$D0,$38,$DA,$38,$D0,$38        ;!
                      db $F2,$38,$D7,$38,$D8,$38,$FC,$00        ;!
                      db $F3,$38,$D4,$38,$F9,$38,$F4,$38        ;!
                      db $DA,$38,$D0,$38,$09,$19,$16,$3C        ;!
                      db $0A,$3C,$19,$3C,$FC,$00,$FC,$00        ;!
                      db $0D,$3C,$12,$3C,$1B,$3C,$0E,$3C        ;!
                      db $0C,$3C,$1D,$3C,$18,$3C,$1B,$3C        ;!
                      db $09,$19,$C7,$38,$C8,$38,$C3,$38        ;!
                      db $C4,$38,$E0,$38,$E4,$38,$C8,$38        ;!
                      db $FC,$00,$CA,$38,$CE,$38,$CD,$38        ;!
                      db $CD,$38,$CE,$38,$09,$19,$D7,$38        ;!
                      db $D8,$38,$D3,$38,$D4,$38,$F0,$38        ;!
                      db $F4,$38,$D8,$38,$FC,$00,$DA,$38        ;!
                      db $DE,$38,$DD,$38,$DD,$38,$DE,$38        ;!
                      db $08,$1D,$0C,$2C,$18,$2C,$1E,$2C        ;!
                      db $1B,$2C,$1C,$2C,$0E,$2C,$FC,$00        ;!
                      db $0D,$2C,$12,$2C,$1B,$2C,$0E,$2C        ;!
                      db $0C,$2C,$1D,$2C,$18,$2C,$1B,$2C        ;!
                      db $09,$1B,$CA,$38,$C0,$38,$E3,$38        ;!
                      db $E2,$38,$E4,$38,$E8,$38,$C0,$38        ;!
                      db $FC,$00,$C4,$38,$C6,$38,$E4,$38        ;!
                      db $C2,$38,$C7,$38,$C8,$38,$09,$1B        ;!
                      db $DA,$38,$D0,$38,$F3,$38,$F2,$38        ;!
                      db $F4,$38,$F8,$38,$D0,$38,$FC,$00        ;!
                      db $D4,$38,$D6,$38,$F4,$38,$D2,$38        ;!
                      db $D7,$38,$D8,$38,$08,$1F,$19,$28        ;!
                      db $1B,$28,$18,$28,$10,$28,$1B,$28        ;!
                      db $0A,$28,$16,$28,$FC,$00,$0D,$28        ;!
                      db $12,$28,$1B,$28,$0E,$28,$0C,$28        ;!
                      db $1D,$28,$18,$28,$1B,$28,$08,$1F        ;!
                      db $E3,$38,$CE,$38,$E2,$38,$C7,$38        ;!
                      db $C8,$38,$C7,$38,$C8,$38,$CA,$38        ;!
                      db $CE,$38,$FC,$00,$CD,$38,$C0,$38        ;!
                      db $CA,$38,$C0,$38,$C6,$38,$CE,$38        ;!
                      db $08,$1F,$F3,$38,$DE,$38,$F2,$38        ;!
                      db $D7,$38,$D8,$38,$D7,$38,$D8,$38        ;!
                      db $DA,$38,$DE,$38,$FC,$00,$DD,$38        ;!
                      db $D0,$38,$DA,$38,$D0,$38,$D6,$38        ;!
                      db $DE,$38,$03,$35,$19,$3C,$15,$3C        ;!
                      db $0A,$3C,$22,$3C,$0E,$3C,$1B,$3C        ;!
                      db $FC,$00,$0A,$3C,$17,$3C,$0D,$3C        ;!
                      db $FC,$00,$1C,$3C,$22,$3C,$1C,$3C        ;!
                      db $1D,$3C,$0E,$3C,$16,$3C,$FC,$00        ;!
                      db $19,$3C,$1B,$3C,$18,$3C,$10,$3C        ;!
                      db $1B,$3C,$0A,$3C,$16,$3C,$0E,$3C        ;!
                      db $1B,$3C,$09,$1B,$E3,$38,$CE,$38        ;!
                      db $E2,$38,$C7,$38,$C8,$38,$CE,$38        ;!
                      db $FC,$00,$C8,$38,$E6,$38,$C0,$38        ;!
                      db $E6,$38,$C0,$38,$CA,$38,$C8,$38        ;!
                      db $09,$1B,$F3,$38,$DE,$38,$F2,$38        ;!
                      db $D7,$38,$D8,$38,$DE,$38,$FC,$00        ;!
                      db $D8,$38,$F6,$38,$D0,$38,$F6,$38        ;!
                      db $D0,$38,$DA,$38,$D8,$38,$08,$1F        ;!
                      db $18,$2C,$0B,$2C,$13,$2C,$0E,$2C        ;!
                      db $0C,$2C,$1D,$2C,$FC,$00,$19,$2C        ;!
                      db $1B,$2C,$18,$2C,$10,$2C,$1B,$2C        ;!
                      db $0A,$2C,$16,$2C,$0E,$2C,$1B,$2C        ;!
                      db $09,$1B,$CA,$38,$C0,$38,$E9,$38        ;!
                      db $E4,$38,$C0,$38,$CA,$38,$C8,$38        ;!
                      db $FC,$00,$CC,$38,$CE,$38,$E1,$38        ;!
                      db $C8,$38,$E3,$38,$C0,$38,$09,$1B        ;!
                      db $DA,$38,$D0,$38,$F9,$38,$F4,$38        ;!
                      db $D0,$38,$DA,$38,$D8,$38,$FC,$00        ;!
                      db $DC,$38,$DE,$38,$F1,$38,$D8,$38        ;!
                      db $F3,$38,$D0,$38,$05,$29,$0B,$28        ;!
                      db $0A,$28,$0C,$28,$14,$28,$FC,$00        ;!
                      db $10,$28,$1B,$28,$18,$28,$1E,$28        ;!
                      db $17,$28,$0D,$28,$FC,$00,$19,$28        ;!
                      db $1B,$28,$18,$28,$10,$28,$1B,$28        ;!
                      db $0A,$28,$16,$28,$0E,$28,$1B,$28        ;!
                      db $07,$25,$E2,$38,$C7,$38,$C8,$38        ;!
                      db $C6,$38,$C4,$38,$C7,$38,$C8,$38        ;!
                      db $E1,$38,$CE,$38,$FC,$00,$CA,$38        ;!
                      db $C0,$38,$E2,$38,$C0,$38,$CC,$38        ;!
                      db $C0,$38,$E3,$38,$E2,$38,$E4,$38        ;!
                      db $07,$25,$F2,$38,$D7,$38,$D8,$38        ;!
                      db $D6,$38,$D4,$38,$D7,$38,$D8,$38        ;!
                      db $F1,$38,$DE,$38,$FC,$00,$DA,$38        ;!
                      db $D0,$38,$F2,$38,$D0,$38,$DC,$38        ;!
                      db $D0,$38,$F3,$38,$F2,$38,$F4,$38        ;!
                      db $07,$23,$E3,$38,$C0,$38,$E3,$38        ;!
                      db $E2,$38,$E4,$38,$CD,$38,$CE,$38        ;!
                      db $E1,$38,$C8,$38,$FC,$00,$E3,$38        ;!
                      db $C0,$38,$CA,$38,$C0,$38,$CA,$38        ;!
                      db $E4,$38,$E1,$38,$C0,$38,$07,$23        ;!
                      db $F3,$38,$D0,$38,$F3,$38,$F2,$38        ;!
                      db $F4,$38,$DD,$38,$DE,$38,$F1,$38        ;!
                      db $D8,$38,$FC,$00,$F3,$38,$D0,$38        ;!
                      db $DA,$38,$D0,$38,$DA,$38,$F4,$38        ;!
                      db $F1,$38,$D0,$38,$09,$19,$16,$3C        ;!
                      db $0A,$3C,$19,$3C,$FC,$00,$19,$3C        ;!
                      db $1B,$3C,$18,$3C,$10,$3C,$1B,$3C        ;!
                      db $0A,$3C,$16,$3C,$0E,$3C,$1B,$3C        ;!
                      db $08,$1F,$E3,$38,$C0,$38,$E3,$38        ;!
                      db $E2,$38,$E4,$38,$CE,$38,$FC,$00        ;!
                      db $CD,$38,$C8,$38,$E2,$38,$C7,$38        ;!
                      db $C8,$38,$E8,$38,$C0,$38,$CC,$38        ;!
                      db $C0,$38,$08,$1F,$F3,$38,$D0,$38        ;!
                      db $F3,$38,$F2,$38,$F4,$38,$DE,$38        ;!
                      db $FC,$00,$DD,$38,$D8,$38,$F2,$38        ;!
                      db $D7,$38,$D8,$38,$F8,$38,$D0,$38        ;!
                      db $DC,$38,$D0,$38,$09,$19,$0C,$2C        ;!
                      db $18,$2C,$1E,$2C,$1B,$2C,$1C,$2C        ;!
                      db $0E,$2C,$FC,$00,$0E,$2C,$0D,$2C        ;!
                      db $12,$2C,$1D,$2C,$18,$2C,$1B,$2C        ;!
                      db $08,$1F,$E8,$38,$CE,$38,$E2,$38        ;!
                      db $C7,$38,$C8,$38,$C7,$38,$C8,$38        ;!
                      db $E1,$38,$CE,$38,$FC,$00,$CD,$38        ;!
                      db $CE,$38,$CC,$38,$CE,$38,$E3,$38        ;!
                      db $CE,$38,$08,$1F,$F8,$38,$DE,$38        ;!
                      db $F2,$38,$D7,$38,$D8,$38,$D7,$38        ;!
                      db $D8,$38,$F1,$38,$DE,$38,$FC,$00        ;!
                      db $DD,$38,$DE,$38,$DC,$38,$DE,$38        ;!
                      db $F3,$38,$DE,$38,$0B,$11,$C4,$38        ;!
                      db $C8,$38,$C9,$38,$C8,$38,$FC,$00        ;!
                      db $CD,$38,$CE,$38,$E3,$38,$CE,$38        ;!
                      db $0B,$11,$D4,$38,$D8,$38,$D9,$38        ;!
                      db $D8,$38,$FC,$00,$DD,$38,$DE,$38        ;!
                      db $F3,$38,$DE,$38,$08,$1D,$E2,$38        ;!
                      db $C0,$38,$E3,$38,$CE,$38,$E1,$38        ;!
                      db $E4,$38,$FC,$00,$E3,$38,$C0,$38        ;!
                      db $CA,$38,$C0,$38,$C7,$38,$C0,$38        ;!
                      db $E3,$38,$C0,$38,$08,$1D,$F2,$38        ;!
                      db $D0,$38,$F3,$38,$DE,$38,$F1,$38        ;!
                      db $F4,$38,$FC,$00,$F3,$38,$D0,$38        ;!
                      db $DA,$38,$D0,$38,$D7,$38,$D0,$38        ;!
                      db $F3,$38,$D0,$38,$0A,$17,$0C,$28        ;!
                      db $24,$28,$10,$28,$24,$28,$0D,$28        ;!
                      db $0E,$28,$1C,$28,$12,$28,$10,$28        ;!
                      db $17,$28,$0E,$28,$1B,$28,$09,$1B        ;!
                      db $E2,$38,$C7,$38,$C8,$38,$C6,$38        ;!
                      db $C4,$38,$C5,$38,$E4,$38,$CC,$38        ;!
                      db $C8,$38,$FC,$00,$C7,$38,$C8,$38        ;!
                      db $CD,$38,$CE,$38,$09,$1B,$F2,$38        ;!
                      db $D7,$38,$D8,$38,$D6,$38,$D4,$38        ;!
                      db $D5,$38,$F4,$38,$DC,$38,$D8,$38        ;!
                      db $FC,$00,$D7,$38,$D8,$38,$DD,$38        ;!
                      db $DE,$38,$09,$1B,$1C,$3C,$18,$3C        ;!
                      db $1E,$3C,$17,$3C,$0D,$3C,$FC,$00        ;!
                      db $0C,$3C,$18,$3C,$16,$3C,$19,$3C        ;!
                      db $18,$3C,$1C,$3C,$0E,$3C,$1B,$3C        ;!
                      db $0B,$13,$CA,$38,$CE,$38,$C9,$38        ;!
                      db $C8,$38,$FC,$00,$CA,$38,$CE,$38        ;!
                      db $CD,$38,$C3,$38,$CE,$38,$0B,$13        ;!
                      db $DA,$38,$DE,$38,$D9,$38,$D8,$38        ;!
                      db $FC,$00,$DA,$38,$DE,$38,$DD,$38        ;!
                      db $D3,$38,$DE,$38,$08,$21,$1C,$2C        ;!
                      db $19,$2C,$0E,$2C,$0C,$2C,$12,$2C        ;!
                      db $0A,$2C,$15,$2C,$FC,$00,$1D,$2C        ;!
                      db $11,$2C,$0A,$2C,$17,$2C,$14,$2C        ;!
                      db $1C,$2C,$FC,$00,$1D,$2C,$18,$2C        ;!
                      db $09,$19,$E8,$38,$CE,$38,$C8,$38        ;!
                      db $C2,$38,$C7,$38,$C8,$38,$FC,$00        ;!
                      db $CA,$38,$CE,$38,$E3,$38,$C0,$38        ;!
                      db $C1,$38,$C4,$38,$09,$19,$F8,$38        ;!
                      db $DE,$38,$D8,$38,$D2,$38,$D7,$38        ;!
                      db $D8,$38,$FC,$00,$DA,$38,$DE,$38        ;!
                      db $F3,$38,$D0,$38,$D1,$38,$D4,$38        ;!
                      db $09,$1B,$E8,$38,$C0,$38,$E2,$38        ;!
                      db $E4,$38,$C7,$38,$C8,$38,$E1,$38        ;!
                      db $CE,$38,$FC,$00,$E2,$38,$C0,$38        ;!
                      db $CA,$38,$C0,$38,$C8,$38,$09,$1B        ;!
                      db $F8,$38,$D0,$38,$F2,$38,$F4,$38        ;!
                      db $D7,$38,$D8,$38,$F1,$38,$DE,$38        ;!
                      db $FC,$00,$F2,$38,$D0,$38,$DA,$38        ;!
                      db $D0,$38,$D8,$38,$09,$19,$CC,$38        ;!
                      db $C8,$38,$C4,$38,$FC,$00,$E8,$38        ;!
                      db $CE,$38,$E2,$38,$C7,$38,$C8,$38        ;!
                      db $CC,$38,$E4,$38,$E1,$38,$C0,$38        ;!
                      db $09,$19,$DC,$38,$D8,$38,$D4,$38        ;!
                      db $FC,$00,$F8,$38,$DE,$38,$F2,$38        ;!
                      db $D7,$38,$D8,$38,$DC,$38,$F4,$38        ;!
                      db $F1,$38,$D0,$38,$09,$1B,$C7,$38        ;!
                      db $C8,$38,$E1,$38,$CE,$38,$CD,$38        ;!
                      db $CE,$38,$C1,$38,$E4,$38,$FC,$00        ;!
                      db $CA,$38,$C0,$38,$CA,$38,$E4,$38        ;!
                      db $C8,$38,$09,$1B,$D7,$38,$D8,$38        ;!
                      db $F1,$38,$DE,$38,$DD,$38,$DE,$38        ;!
                      db $D1,$38,$F4,$38,$FC,$00,$DA,$38        ;!
                      db $D0,$38,$DA,$38,$F4,$38,$D8,$38        ;!
                      db $0B,$13,$CA,$38,$C4,$38,$C8,$38        ;!
                      db $E9,$38,$CE,$38,$FC,$00,$CA,$38        ;!
                      db $C0,$38,$E3,$38,$CE,$38,$0B,$13        ;!
                      db $DA,$38,$D4,$38,$D8,$38,$F9,$38        ;!
                      db $DE,$38,$FC,$00,$DA,$38,$D0,$38        ;!
                      db $F3,$38,$DE,$38,$09,$19,$E3,$38        ;!
                      db $C0,$38,$CA,$38,$C0,$38,$CE,$38        ;!
                      db $FC,$00,$E2,$38,$C7,$38,$C8,$38        ;!
                      db $CC,$38,$C8,$38,$E9,$38,$E4,$38        ;!
                      db $09,$19,$F3,$38,$D0,$38,$DA,$38        ;!
                      db $D0,$38,$DE,$38,$FC,$00,$F2,$38        ;!
                      db $D7,$38,$D8,$38,$DC,$38,$D8,$38        ;!
                      db $F9,$38,$F4,$38                        ;!
																;!
DATA_0C9D18:          dw $00C1,$0026,$00DF,$00FD                ;!
                      dw $0026,$0026,$0026,$011B                ;!
                      dw $0026,$0137,$0153,$0026                ;!
                      dw $0026,$0026,$016F,$0026                ;!
                      dw $018F,$01AD,$0026,$0026                ;!
                      dw $0026,$01CB,$0026,$01ED                ;!
                      dw $020F,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0231,$0026,$0269                ;!
                      dw $0287,$0026,$0026,$0026                ;!
                      dw $02A5,$0026,$02C7,$02E5                ;!
                      dw $0026,$0026,$0026,$0303                ;!
                      dw $0026,$032F,$0357,$0026                ;!
                      dw $037F,$03A5,$0026,$03CB                ;!
                      dw $0026,$03E7,$0409,$0026                ;!
                      dw $0026,$0026,$042B,$0026                ;!
                      dw $0447,$0469,$0026,$048B                ;!
                      dw $049F,$0026,$04B3,$04D3                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $04F3,$0026,$050D,$052B                ;!
                      dw $0026,$0026,$0026,$0549                ;!
                      dw $0026,$0567,$057D,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0593                ;!
                      dw $0026,$05B7,$05D3,$0026                ;!
                      dw $05EF,$060D,$0026,$062B                ;!
                      dw $0647,$0026,$0663,$0681                ;!
                      dw $0026,$069F,$06B5,$0026                ;!
                      dw $06CB,$06E7,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$006B,$0026                ;!
                      dw $007D,$009F,$0026,$0026                ;!
                      dw $0026,$0000,$0026,$0027                ;!
                      dw $0049,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026                            ;!
                   else                                         ;<================== U, SS, E0, & E1 ==============
                      db $09,$1F,$C7,$38,$C8,$38,$E1,$38        ;!
                      db $CE,$38,$E2,$38,$C7,$38,$C8,$38        ;!
                      db $FC,$00,$E8,$38,$C0,$38,$CC,$38        ;!
                      db $C0,$38,$E4,$38,$C2,$38,$C7,$38        ;!
                      db $C8,$38,$09,$1F,$D7,$38,$D8,$38        ;!
                      db $F1,$38,$DE,$38,$F2,$38,$D7,$38        ;!
                      db $D8,$38,$FC,$00,$F8,$38,$D0,$38        ;!
                      db $DC,$38,$D0,$38,$F4,$38,$D2,$38        ;!
                      db $D7,$38,$D8,$38,$0C,$0F,$19,$28        ;!
                      db $1B,$28,$18,$28,$0D,$28,$1E,$28        ;!
                      db $0C,$28,$0E,$28,$1B,$28,$08,$1F        ;!
                      db $E2,$38,$C7,$38,$C8,$38,$C6,$38        ;!
                      db $C4,$38,$E1,$38,$E4,$38,$FC,$00        ;!
                      db $CC,$38,$C8,$38,$E8,$38,$C0,$38        ;!
                      db $CC,$38,$CE,$38,$E3,$38,$CE,$38        ;!
                      db $08,$1F,$F2,$38,$D7,$38,$D8,$38        ;!
                      db $D6,$38,$D4,$38,$F1,$38,$F4,$38        ;!
                      db $FC,$00,$DC,$38,$D8,$38,$F8,$38        ;!
                      db $D0,$38,$DC,$38,$DE,$38,$F3,$38        ;!
                      db $DE,$38,$09,$19,$16,$28,$0A,$28        ;!
                      db $12,$28,$17,$28,$FC,$00,$0D,$28        ;!
                      db $12,$28,$1B,$28,$0E,$28,$0C,$28        ;!
                      db $1D,$28,$18,$28,$1B,$28,$09,$1B        ;!
                      db $E3,$38,$C0,$38,$CA,$38,$C0,$38        ;!
                      db $E2,$38,$C7,$38,$C8,$38,$FC,$00        ;!
                      db $E3,$38,$C4,$38,$E9,$38,$E4,$38        ;!
                      db $CA,$38,$C0,$38,$09,$1B,$F3,$38        ;!
                      db $D0,$38,$DA,$38,$D0,$38,$F2,$38        ;!
                      db $D7,$38,$D8,$38,$FC,$00,$F3,$38        ;!
                      db $D4,$38,$F9,$38,$F4,$38,$DA,$38        ;!
                      db $D0,$38,$0A,$17,$16,$3C,$0A,$3C        ;!
                      db $19,$3C,$FC,$00,$0D,$3C,$12,$3C        ;!
                      db $1B,$3C,$0E,$3C,$0C,$3C,$1D,$3C        ;!
                      db $18,$3C,$1B,$3C,$09,$19,$C7,$38        ;!
                      db $C8,$38,$C3,$38,$C4,$38,$E0,$38        ;!
                      db $E4,$38,$C8,$38,$FC,$00,$CA,$38        ;!
                      db $CE,$38,$CD,$38,$CD,$38,$CE,$38        ;!
                      db $09,$19,$D7,$38,$D8,$38,$D3,$38        ;!
                      db $D4,$38,$F0,$38,$F4,$38,$D8,$38        ;!
                      db $FC,$00,$DA,$38,$DE,$38,$DD,$38        ;!
                      db $DD,$38,$DE,$38,$09,$19,$0A,$2C        ;!
                      db $1B,$2C,$0E,$2C,$0A,$2C,$FC,$00        ;!
                      db $0D,$2C,$12,$2C,$1B,$2C,$0E,$2C        ;!
                      db $0C,$2C,$1D,$2C,$18,$2C,$1B,$2C        ;!
                      db $09,$1B,$CA,$38,$C0,$38,$E3,$38        ;!
                      db $E2,$38,$E4,$38,$E8,$38,$C0,$38        ;!
                      db $FC,$00,$C4,$38,$C6,$38,$E4,$38        ;!
                      db $C2,$38,$C7,$38,$C8,$38,$09,$1B        ;!
                      db $DA,$38,$D0,$38,$F3,$38,$F2,$38        ;!
                      db $F4,$38,$F8,$38,$D0,$38,$FC,$00        ;!
                      db $D4,$38,$D6,$38,$F4,$38,$D2,$38        ;!
                      db $D7,$38,$D8,$38,$08,$1F,$19,$28        ;!
                      db $1B,$28,$18,$28,$10,$28,$1B,$28        ;!
                      db $0A,$28,$16,$28,$FC,$00,$0D,$28        ;!
                      db $12,$28,$1B,$28,$0E,$28,$0C,$28        ;!
                      db $1D,$28,$18,$28,$1B,$28,$08,$1F        ;!
                      db $E3,$38,$CE,$38,$E2,$38,$C7,$38        ;!
                      db $C8,$38,$C7,$38,$C8,$38,$CA,$38        ;!
                      db $CE,$38,$FC,$00,$CD,$38,$C0,$38        ;!
                      db $CA,$38,$C0,$38,$C6,$38,$CE,$38        ;!
                      db $08,$1F,$F3,$38,$DE,$38,$F2,$38        ;!
                      db $D7,$38,$D8,$38,$D7,$38,$D8,$38        ;!
                      db $DA,$38,$DE,$38,$FC,$00,$DD,$38        ;!
                      db $D0,$38,$DA,$38,$D0,$38,$D6,$38        ;!
                      db $DE,$38,$02,$35,$16,$3C,$0A,$3C        ;!
                      db $1B,$3C,$12,$3C,$18,$3C,$FC,$00        ;!
                      db $0A,$3C,$17,$3C,$0D,$3C,$FC,$00        ;!
                      db $1C,$3C,$22,$3C,$1C,$3C,$1D,$3C        ;!
                      db $0E,$3C,$16,$3C,$FC,$00,$19,$3C        ;!
                      db $1B,$3C,$18,$3C,$10,$3C,$1B,$3C        ;!
                      db $0A,$3C,$16,$3C,$16,$3C,$0E,$3C        ;!
                      db $1B,$3C,$09,$1B,$E3,$38,$CE,$38        ;!
                      db $E2,$38,$C7,$38,$C8,$38,$CE,$38        ;!
                      db $FC,$00,$C8,$38,$E6,$38,$C0,$38        ;!
                      db $E6,$38,$C0,$38,$CA,$38,$C8,$38        ;!
                      db $09,$1B,$F3,$38,$DE,$38,$F2,$38        ;!
                      db $D7,$38,$D8,$38,$DE,$38,$FC,$00        ;!
                      db $D8,$38,$F6,$38,$D0,$38,$F6,$38        ;!
                      db $D0,$38,$DA,$38,$D8,$38,$08,$21        ;!
                      db $18,$2C,$0B,$2C,$13,$2C,$0E,$2C        ;!
                      db $0C,$2C,$1D,$2C,$FC,$00,$19,$2C        ;!
                      db $1B,$2C,$18,$2C,$10,$2C,$1B,$2C        ;!
                      db $0A,$2C,$16,$2C,$16,$2C,$0E,$2C        ;!
                      db $1B,$2C,$09,$1B,$CA,$38,$C0,$38        ;!
                      db $E9,$38,$E4,$38,$C0,$38,$CA,$38        ;!
                      db $C8,$38,$FC,$00,$CC,$38,$CE,$38        ;!
                      db $E1,$38,$C8,$38,$E3,$38,$C0,$38        ;!
                      db $09,$1B,$DA,$38,$D0,$38,$F9,$38        ;!
                      db $F4,$38,$D0,$38,$DA,$38,$D8,$38        ;!
                      db $FC,$00,$DC,$38,$DE,$38,$F1,$38        ;!
                      db $D8,$38,$F3,$38,$D0,$38,$06,$29        ;!
                      db $0B,$28,$0A,$28,$0C,$28,$14,$28        ;!
                      db $10,$28,$1B,$28,$18,$28,$1E,$28        ;!
                      db $17,$28,$0D,$28,$FC,$00,$19,$28        ;!
                      db $1B,$28,$18,$28,$10,$28,$1B,$28        ;!
                      db $0A,$28,$16,$28,$16,$28,$0E,$28        ;!
                      db $1B,$28,$07,$25,$E2,$38,$C7,$38        ;!
                      db $C8,$38,$C6,$38,$C4,$38,$C7,$38        ;!
                      db $C8,$38,$E1,$38,$CE,$38,$FC,$00        ;!
                      db $CA,$38,$C0,$38,$E2,$38,$C0,$38        ;!
                      db $CC,$38,$C0,$38,$E3,$38,$E2,$38        ;!
                      db $E4,$38,$07,$25,$F2,$38,$D7,$38        ;!
                      db $D8,$38,$D6,$38,$D4,$38,$D7,$38        ;!
                      db $D8,$38,$F1,$38,$DE,$38,$FC,$00        ;!
                      db $DA,$38,$D0,$38,$F2,$38,$D0,$38        ;!
                      db $DC,$38,$D0,$38,$F3,$38,$F2,$38        ;!
                      db $F4,$38,$07,$23,$E3,$38,$C0,$38        ;!
                      db $E3,$38,$E2,$38,$E4,$38,$CD,$38        ;!
                      db $CE,$38,$E1,$38,$C8,$38,$FC,$00        ;!
                      db $E3,$38,$C0,$38,$CA,$38,$C0,$38        ;!
                      db $CA,$38,$E4,$38,$E1,$38,$C0,$38        ;!
                      db $07,$23,$F3,$38,$D0,$38,$F3,$38        ;!
                      db $F2,$38,$F4,$38,$DD,$38,$DE,$38        ;!
                      db $F1,$38,$D8,$38,$FC,$00,$F3,$38        ;!
                      db $D0,$38,$DA,$38,$D0,$38,$DA,$38        ;!
                      db $F4,$38,$F1,$38,$D0,$38,$09,$1B        ;!
                      db $16,$3C,$0A,$3C,$19,$3C,$FC,$00        ;!
                      db $19,$3C,$1B,$3C,$18,$3C,$10,$3C        ;!
                      db $1B,$3C,$0A,$3C,$16,$3C,$16,$3C        ;!
                      db $0E,$3C,$1B,$3C,$08,$1F,$E3,$38        ;!
                      db $C0,$38,$E3,$38,$E2,$38,$E4,$38        ;!
                      db $CE,$38,$FC,$00,$CD,$38,$C8,$38        ;!
                      db $E2,$38,$C7,$38,$C8,$38,$E8,$38        ;!
                      db $C0,$38,$CC,$38,$C0,$38,$08,$1F        ;!
                      db $F3,$38,$D0,$38,$F3,$38,$F2,$38        ;!
                      db $F4,$38,$DE,$38,$FC,$00,$DD,$38        ;!
                      db $D8,$38,$F2,$38,$D7,$38,$D8,$38        ;!
                      db $F8,$38,$D0,$38,$DC,$38,$D0,$38        ;!
                      db $08,$1D,$0A,$2C,$1B,$2C,$0E,$2C        ;!
                      db $0A,$2C,$FC,$00,$0D,$2C,$0A,$2C        ;!
                      db $1D,$2C,$0A,$2C,$FC,$00,$12,$2C        ;!
                      db $17,$2C,$19,$2C,$1E,$2C,$1D,$2C        ;!
                      db $08,$1F,$E8,$38,$CE,$38,$E2,$38        ;!
                      db $C7,$38,$C8,$38,$C7,$38,$C8,$38        ;!
                      db $E1,$38,$CE,$38,$FC,$00,$CD,$38        ;!
                      db $CE,$38,$CC,$38,$CE,$38,$E3,$38        ;!
                      db $CE,$38,$08,$1F,$F8,$38,$DE,$38        ;!
                      db $F2,$38,$D7,$38,$D8,$38,$D7,$38        ;!
                      db $D8,$38,$F1,$38,$DE,$38,$FC,$00        ;!
                      db $DD,$38,$DE,$38,$DC,$38,$DE,$38        ;!
                      db $F3,$38,$DE,$38,$0B,$11,$C4,$38        ;!
                      db $C8,$38,$C9,$38,$C8,$38,$FC,$00        ;!
                      db $CD,$38,$CE,$38,$E3,$38,$CE,$38        ;!
                      db $0B,$11,$D4,$38,$D8,$38,$D9,$38        ;!
                      db $D8,$38,$FC,$00,$DD,$38,$DE,$38        ;!
                      db $F3,$38,$DE,$38,$08,$1D,$E2,$38        ;!
                      db $C0,$38,$E3,$38,$CE,$38,$E1,$38        ;!
                      db $E4,$38,$FC,$00,$E3,$38,$C0,$38        ;!
                      db $CA,$38,$C0,$38,$C7,$38,$C0,$38        ;!
                      db $E3,$38,$C0,$38,$08,$1D,$F2,$38        ;!
                      db $D0,$38,$F3,$38,$DE,$38,$F1,$38        ;!
                      db $F4,$38,$FC,$00,$F3,$38,$D0,$38        ;!
                      db $DA,$38,$D0,$38,$D7,$38,$D0,$38        ;!
                      db $F3,$38,$D0,$38,$03,$33,$0C,$28        ;!
                      db $11,$28,$0A,$28,$1B,$28,$0A,$28        ;!
                      db $0C,$28,$1D,$28,$0E,$28,$1B,$28        ;!
                      db $FC,$00,$10,$28,$1B,$28,$0A,$28        ;!
                      db $19,$28,$11,$28,$12,$28,$0C,$28        ;!
                      db $FC,$00,$0D,$28,$0E,$28,$1C,$28        ;!
                      db $12,$28,$10,$28,$17,$28,$0E,$28        ;!
                      db $1B,$28,$09,$1B,$E2,$38,$C7,$38        ;!
                      db $C8,$38,$C6,$38,$C4,$38,$C5,$38        ;!
                      db $E4,$38,$CC,$38,$C8,$38,$FC,$00        ;!
                      db $C7,$38,$C8,$38,$CD,$38,$CE,$38        ;!
                      db $09,$1B,$F2,$38,$D7,$38,$D8,$38        ;!
                      db $D6,$38,$D4,$38,$D5,$38,$F4,$38        ;!
                      db $DC,$38,$D8,$38,$FC,$00,$D7,$38        ;!
                      db $D8,$38,$DD,$38,$DE,$38,$09,$1B        ;!
                      db $1C,$3C,$18,$3C,$1E,$3C,$17,$3C        ;!
                      db $0D,$3C,$FC,$00,$0C,$3C,$18,$3C        ;!
                      db $16,$3C,$19,$3C,$18,$3C,$1C,$3C        ;!
                      db $0E,$3C,$1B,$3C,$0B,$13,$CA,$38        ;!
                      db $CE,$38,$C9,$38,$C8,$38,$FC,$00        ;!
                      db $CA,$38,$CE,$38,$CD,$38,$C3,$38        ;!
                      db $CE,$38,$0B,$13,$DA,$38,$DE,$38        ;!
                      db $D9,$38,$D8,$38,$FC,$00,$DA,$38        ;!
                      db $DE,$38,$DD,$38,$D3,$38,$DE,$38        ;!
                      db $07,$23,$1C,$2C,$19,$2C,$0E,$2C        ;!
                      db $0C,$2C,$12,$2C,$0A,$2C,$15,$2C        ;!
                      db $FC,$00,$1D,$2C,$11,$2C,$0A,$2C        ;!
                      db $17,$2C,$14,$2C,$1C,$2C,$FC,$00        ;!
                      db $1D,$2C,$18,$2C,$78,$2C,$09,$19        ;!
                      db $E8,$38,$CE,$38,$C8,$38,$C2,$38        ;!
                      db $C7,$38,$C8,$38,$FC,$00,$CA,$38        ;!
                      db $CE,$38,$E3,$38,$C0,$38,$C1,$38        ;!
                      db $C4,$38,$09,$19,$F8,$38,$DE,$38        ;!
                      db $D8,$38,$D2,$38,$D7,$38,$D8,$38        ;!
                      db $FC,$00,$DA,$38,$DE,$38,$F3,$38        ;!
                      db $D0,$38,$D1,$38,$D4,$38,$09,$1B        ;!
                      db $E8,$38,$C0,$38,$E2,$38,$E4,$38        ;!
                      db $C7,$38,$C8,$38,$E1,$38,$CE,$38        ;!
                      db $FC,$00,$E2,$38,$C0,$38,$CA,$38        ;!
                      db $C0,$38,$C8,$38,$09,$1B,$F8,$38        ;!
                      db $D0,$38,$F2,$38,$F4,$38,$D7,$38        ;!
                      db $D8,$38,$F1,$38,$DE,$38,$FC,$00        ;!
                      db $F2,$38,$D0,$38,$DA,$38,$D0,$38        ;!
                      db $D8,$38,$09,$19,$CC,$38,$C8,$38        ;!
                      db $C4,$38,$FC,$00,$E8,$38,$CE,$38        ;!
                      db $E2,$38,$C7,$38,$C8,$38,$CC,$38        ;!
                      db $E4,$38,$E1,$38,$C0,$38,$09,$19        ;!
                      db $DC,$38,$D8,$38,$D4,$38,$FC,$00        ;!
                      db $F8,$38,$DE,$38,$F2,$38,$D7,$38        ;!
                      db $D8,$38,$DC,$38,$F4,$38,$F1,$38        ;!
                      db $D0,$38,$09,$1B,$C7,$38,$C8,$38        ;!
                      db $E1,$38,$CE,$38,$CD,$38,$CE,$38        ;!
                      db $C1,$38,$E4,$38,$FC,$00,$CA,$38        ;!
                      db $C0,$38,$CA,$38,$E4,$38,$C8,$38        ;!
                      db $09,$1B,$D7,$38,$D8,$38,$F1,$38        ;!
                      db $DE,$38,$DD,$38,$DE,$38,$D1,$38        ;!
                      db $F4,$38,$FC,$00,$DA,$38,$D0,$38        ;!
                      db $DA,$38,$F4,$38,$D8,$38,$0B,$13        ;!
                      db $CA,$38,$C4,$38,$C8,$38,$E9,$38        ;!
                      db $CE,$38,$FC,$00,$CA,$38,$C0,$38        ;!
                      db $E3,$38,$CE,$38,$0B,$13,$DA,$38        ;!
                      db $D4,$38,$D8,$38,$F9,$38,$DE,$38        ;!
                      db $FC,$00,$DA,$38,$D0,$38,$F3,$38        ;!
                      db $DE,$38,$09,$19,$E3,$38,$C0,$38        ;!
                      db $CA,$38,$C0,$38,$CE,$38,$FC,$00        ;!
                      db $E2,$38,$C7,$38,$C8,$38,$CC,$38        ;!
                      db $C8,$38,$E9,$38,$E4,$38,$09,$19        ;!
                      db $F3,$38,$D0,$38,$DA,$38,$D0,$38        ;!
                      db $DE,$38,$FC,$00,$F2,$38,$D7,$38        ;!
                      db $D8,$38,$DC,$38,$D8,$38,$F9,$38        ;!
                      db $F4,$38,$0A,$15,$C3,$38,$C0,$38        ;!
                      db $E8,$38,$E5,$38,$FC,$00,$C1,$38        ;!
                      db $E1,$38,$CE,$38,$CE,$38,$CA,$38        ;!
                      db $E2,$38,$0A,$15,$D3,$38,$D0,$38        ;!
                      db $F8,$38,$F5,$38,$FC,$00,$D1,$38        ;!
                      db $F1,$38,$DE,$38,$DE,$38,$DA,$38        ;!
                      db $F2,$38                                ;!
																;!
DATA_0C9D18:          dw $00C1,$0026,$00DD,$00FB                ;!
                      dw $0026,$0026,$0026,$0119                ;!
                      dw $0026,$0133,$014F,$0026                ;!
                      dw $0026,$0026,$016B,$0026                ;!
                      dw $0187,$01A5,$0026,$0026                ;!
                      dw $0026,$01C3,$0026,$01E5                ;!
                      dw $0207,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0229,$0026,$0261,$027F                ;!
                      dw $0026,$0026,$0026,$029D                ;!
                      dw $0026,$02C1,$02DF,$0026                ;!
                      dw $0026,$0026,$02FD,$0026                ;!
                      dw $0329,$0351,$0026,$0379                ;!
                      dw $039F,$0026,$0026,$03C5                ;!
                      dw $0026,$03E3,$0405,$0026                ;!
                      dw $0026,$0026,$0427,$0026                ;!
                      dw $0447,$0469,$0026,$048B                ;!
                      dw $049F,$0026,$04B3,$04D3                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$04F3                ;!
                      dw $0026,$0529,$0547,$0026                ;!
                      dw $0026,$0026,$0565,$0026                ;!
                      dw $0583,$0599,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$05AF,$0026,$05D5                ;!
                      dw $05F1,$0026,$060D,$062B                ;!
                      dw $0026,$0649,$0665,$0026                ;!
                      dw $0681,$069F,$0026,$06BD                ;!
                      dw $06D3,$0026,$06E9,$0705                ;!
                      dw $0026,$0721,$0739,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$006B,$0026                ;!
                      dw $007D,$009F,$0026,$0026                ;!
                      dw $0026,$0000,$0026,$0027                ;!
                      dw $0049,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026,$0026,$0026                ;!
                      dw $0026,$0026                            ;!
                   endif                                        ;/===============================================

DATA_0C9EAC:          db $40,$3E,$FC,$00,$FF

CODE_0C9EB1:          REP #$30                                  ; AXY->16
                      LDA.L DynStripeImgSize
                      TAX
                      LDY.W #$0000
                      SEP #$20                                  ; A->8
                      LDA.B Layer1DataPtr+1
                      STA.L DynamicStripeImage,X
                      LDA.B Layer1DataPtr
                      STA.L DynamicStripeImage+1,X
                      INX
                      INX
                    - LDA.W DATA_0C9EAC,Y
                      STA.L DynamicStripeImage,X
                      INX
                      INY
                      CPY.W #$0005
                      BNE -
                      REP #$20                                  ; A->16
                      DEX
                      TXA
                      STA.L DynStripeImgSize
                      LDA.B Layer1DataPtr+2
                      AND.W #$00FF
                      ASL A
                      TAY
                      LDA.W DATA_0C9D18,Y
                      TAY
                      SEP #$20                                  ; A->8
                      INC.B Layer1DataPtr+2
                      LDA.W DATA_0C95C7,Y
                      CMP.B #$FF
                      BEQ CODE_0C9F43
                      LDA.W DATA_0C95C7,Y
                      STA.B _2
                      LDA.W DATA_0C95C7+1,Y
                      STA.B _0
                      STZ.B _1
                      INY
                      INY
                      LDA.B Layer1DataPtr+1
                      STA.L DynamicStripeImage,X
                      LDA.B Layer1DataPtr
                      CLC
                      ADC.B _2
                      STA.L DynamicStripeImage+1,X
                      INX
                      INX
                      LDA.B _1
                      STA.L DynamicStripeImage,X
                      LDA.B _0
                      STA.L DynamicStripeImage+1,X
                      INX
                      INX
                      REP #$20                                  ; A->16
                    - LDA.W DATA_0C95C7,Y
                      STA.L DynamicStripeImage,X
                      INX
                      INX
                      INY
                      INY
                      DEC.B _0
                      DEC.B _0
                      BPL -
                      LDA.W #$00FF
                      STA.L DynamicStripeImage,X
                      TXA
                      STA.L DynStripeImgSize
CODE_0C9F43:          REP #$20                                  ; A->16
                      SEP #$10                                  ; XY->8
                      LDA.B Layer1DataPtr
                      CLC
                      ADC.W #$0020
                      STA.B Layer1DataPtr
                      AND.W #$03FF
                      BNE +
                      LDA.B Layer1DataPtr
                      EOR.W #$0C00
                      STA.B Layer1DataPtr
                    + RTS


DATA_0C9F5C:          db $C0,$00,$80,$01,$40,$02,$00,$03
                      db $C0,$03,$80,$04,$59,$05

CODE_0C9F6A:          REP #$20                                  ; A->16
                      LDX.B #$00
                      LDA.W #$FF80
                      STA.W Layer1ScrollXSpeed,X
                      LDA.B Layer2XPos,X
                      STA.B _0,X
                      JSR CODE_0C9FCB
                      LDA.B _0,X
                      STA.B Layer2XPos,X
                      LDA.B Layer3YPos
                      CMP.W #$0559
                      BCS +
                      LDX.B #$02
                      LDA.W #con($0040,$0040,$0040,$004E,$004E)
                      STA.W Layer1ScrollXSpeed,X
                      LDA.B Layer3YPos
                      STA.B _0,X
                      STA.B _4
                      JSR CODE_0C9FCB
                      LDA.B _0,X
                      CMP.B _4
                      BEQ +
                      STA.B Layer3YPos
                      LDA.B Layer3YPos
                      AND.W #$0007
                      CMP.W #$0001
                      BNE +
                      JSR CODE_0C9EB1
                    + LDX.B #$0C
CODE_0C9FAE:          LDA.B Layer3YPos
                      %LorW_X(CMP,DATA_0C9F5C)
                      BEQ CODE_0C9FBB
                      DEX
                      DEX
                      BPL CODE_0C9FAE
                      BRA +

CODE_0C9FBB:          LDA.W MessageBoxExpand
                      AND.W #$00FF
                      BNE +
                      INC.W MessageBoxExpand
                    + SEP #$20                                  ; A->8
                      BRL CODE_0C9FEA
CODE_0C9FCB:          LDA.W Layer1ScrollXPosUpd,X
                      AND.W #$00FF
                      CLC
                      ADC.W Layer1ScrollXSpeed,X
                      STA.W Layer1ScrollXPosUpd,X
                      AND.W #$FF00
                      BPL +
                      ORA.W #$00FF
                    + XBA
                      CLC
                      ADC.B _0,X
                      STA.B _0,X
                      RTS


DATA_0C9FE7:          db $00,$FF,$00

CODE_0C9FEA:          REP #$20                                  ; A->16
                      LDA.W #$0038
                      STA.B PlayerXPosNext
                      LDA.W #$008F
                      STA.B PlayerYPosNext
                      SEP #$20                                  ; A->8
                      LDA.B #$01
                      STA.B Powerup
                      LDA.B #$08
                      STA.W PlayerXSpeed
                      JSR CODE_0CA75A
                      LDA.B #$52
                      STA.B SpriteXPosLow
                      STZ.W SpriteXPosHigh
                      LDA.B #$8F
                      STA.B SpriteYPosLow
                      STZ.W SpriteYPosHigh
                      LDA.B #$A0
                      STA.W SpriteOAMIndex
                      JSR CODE_0CA778
                      LDX.W SpriteMisc1602
                      LDA.B #$51
                      STA.B _0
                      STZ.B _1
                      LDA.W DATA_0C9FE7,X
                      CLC
                      ADC.B #$85
                      STA.B _2
                      TXA
                      ASL A
                      ASL A
                      TAY
                      LDX.B #$00
                      JSR CODE_0CA821
                      LDA.B #$A0
                      STA.B _E
                      JSR CODE_0CA8A3
                      LDA.B #$10
                      STA.B _2
                      JSR CODE_0CA8DF
                      RTS


DATA_0CA043:          db $63,$73,$83,$93,$A3,$B3,$C3

DATA_0CA04A:          db $A0,$9C,$A0,$9C,$A0,$9C,$A0

CODE_0CA051:          PHB
                      PHK
                      PLB
                      LDX.B #$0E
                    - STZ.W CreditsSprYPosSpx,X
                      STZ.W CreditsSprXPosSpx,X
                      DEX
                      BPL -
                      LDX.B #$0D
                      LDY.B #$06
                    - LDA.W DATA_0CA04A,Y
                      STA.W CreditsSprYPosLow,X
                      STZ.W CreditsSprYPosHigh,X
                      LDA.W DATA_0CA043,Y
                      STA.W CreditsSprXPosLow,X
                      STZ.W CreditsSprXPosHigh,X
                      LDA.W DATA_0CA3C2,Y
                      STA.W CreditsSprYSpeed,X
                      LDA.B #$01
                      STA.W GfxDecompOWAni,X
                      DEY
                      DEX
                      CPX.B #$06
                      BNE -
                      STZ.B PlayerDirection
                      LDA.B #con($E2,$E2,$E2,$E2,$FE)
                      STA.W MessageBoxTimer
                      PLB
                      RTS


DATA_0CA08F:          db $FF,$02,$04,$06,$08,$06,$08,$06
                      db $08,$06,$08,$04,$02,$FF,$20,$22
                      db $24,$FF,$FF,$FF,$FF,$FF,$FF,$FF
                      db $FF,$24,$22,$20,$40,$42,$FF,$FF
                      db $FF,$FF,$FF,$FF,$FF,$FF,$FF,$FF
                      db $42,$40

DATA_0CA0B9:          db $FF,$37,$37,$35,$35,$37,$37,$39
                      db $39,$3B,$3B,$77,$77,$FF,$37,$37
                      db $37,$FF,$FF,$FF,$FF,$FF,$FF,$FF
                      db $FF,$77,$77,$77,$37,$37,$FF,$FF
                      db $FF,$FF,$FF,$FF,$FF,$FF,$FF,$FF
                      db $77,$77

CODE_0CA0E3:          LDA.B #$10
                      STA.B _0
                      STA.B _2
                      LDA.B #$2F
                      STA.B _1
                      LDX.B #$90
                      LDY.B #$00
CODE_0CA0F1:          LDA.W DATA_0CA08F,Y
                      CMP.B #$FF
                      BEQ +
                      STA.W OAMTileNo+$100,X
                      LDA.W DATA_0CA0B9,Y
                      STA.W OAMTileAttr+$100,X
                      LDA.B _0
                      STA.W OAMTileXPos+$100,X
                      LDA.B _1
                      STA.W OAMTileYPos+$100,X
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.B #$02
                      STA.W OAMTileSize+$40,X
                      PLX
                      INX
                      INX
                      INX
                      INX
                    + LDA.B _0
                      CLC
                      ADC.B #$10
                      STA.B _0
                      CMP.B #$F0
                      BNE +
                      LDA.B _2
                      STA.B _0
                      LDA.B _1
                      CLC
                      ADC.B #$10
                      STA.B _1
                    + INY
                      CPY.B #$2A
                      BNE CODE_0CA0F1
                      RTS


DATA_0CA136:          db $26,$28,$2A,$2C,$46,$48,$4A,$4C
                      db $60,$62,$64,$66,$6A,$6C,$6E,$0A

CODE_0CA146:          REP #$20                                  ; A->16
                      LDA.W #$003F
                      STA.B _8
                      STA.B _A
                      STZ.B _0
                      STZ.B _2
                      STZ.B _4
                      LDY.B #$00
                      LDX.B #$50
CODE_0CA159:          REP #$20                                  ; A->16
                      LDA.B _0
                      CLC
                      ADC.W #$0080
                      STA.B _0
                      STA.B _2
                      SEP #$20                                  ; A->8
                      JSR CODE_0CA183
                      REP #$20                                  ; A->16
                      LDA.W #$0080
                      SEC
                      SBC.B _4
                      SEC
                      SBC.W #$0040
                      STA.B _0
                      STA.B _2
                      LDA.W #$003F
                      STA.B _8
                      STA.B _A
                      SEP #$20                                  ; A->8
CODE_0CA183:          LDA.B _1
                      BNE +
                      LDA.W DATA_0CA136,Y
                      STA.W OAMTileNo+$100,X
                      LDA.B #$35
                      STA.W OAMTileAttr+$100,X
                      LDA.B _0
                      STA.W OAMTileXPos+$100,X
                      LDA.B _8
                      STA.W OAMTileYPos+$100,X
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.B #$02
                      STA.W OAMTileSize+$40,X
                      PLX
                      INX
                      INX
                      INX
                      INX
                    + REP #$20                                  ; A->16
                      LDA.B _0
                      CLC
                      ADC.W #$0010
                      STA.B _0
                      SEP #$20                                  ; A->8
                      TYA
                      AND.B #$07
                      CMP.B #$03
                      BNE +
                      LDA.B _2
                      STA.B _0
                      LDA.B _3
                      STA.B _1
                      LDA.B _8
                      CLC
                      ADC.B #$10
                      STA.B _8
                    + INY
                      TYA
                      AND.B #$07
                      BNE CODE_0CA183
                      RTS

CODE_0CA1D4:          JSR CODE_0CAB1F
                      LDA.W Layer1ScrollType
                      JSL ExecutePtrLong

                      dl CODE_0CA1ED
                      dl CODE_0CA43C
                      dl CODE_0CA532
                      dl CODE_0CA65B
                      dl CODE_0CA6B0

CODE_0CA1ED:          JSR CODE_0CA315
                      LDA.W MessageBoxTimer
                      BEQ CODE_0CA1F6
                      RTS

CODE_0CA1F6:          LDA.B #$60
                      STA.B PlayerYPosNext
                      LDA.B #$01
                      STA.B PlayerYPosNext+1
                      LDX.B #$00
                      LDA.W CreditsSprXPosLow,X
                      CMP.B #$70
                      BNE CODE_0CA24F
                      STZ.W YoshiIsLoose
                      LDA.B #$0F                                ; \ Mario's image = Going Down Pipe
                      STA.W PlayerPose                          ; /
                      JSR CODE_0CA764
                      JSR CODE_0CA7B4
                      LDA.W Layer1ScrollType
                      BNE +
                      LDA.B SpriteXPosLow
                      STA.B _0
                      LDA.B #$9F
                      STA.B _2
                      LDY.B #$00
                      LDX.B #$B4
                      LDA.B #$01
                      STA.B _E
                      JSR CODE_0CAA15
                    + STZ.B _1
                      LDA.W CreditsSprXPosLow
                      CLC
                      ADC.B #$10
                      STA.B _0
                      LDA.W CreditsSprXPosHigh
                      ADC.B #$00
                      STA.B _1
                      LDA.B #$9F
                      STA.B _2
                      LDA.B #$03
                      ASL A
                      ASL A
                      TAY
                      LDX.B #$A4
                      JSR CODE_0CA821
                      BRL CODE_0CA2C3
CODE_0CA24F:          LDA.B #$F8
                      STA.W CreditsSprXSpeed,X
                      JSR CODE_0CA74F
                      LDA.W CreditsSprXPosLow,X
                      STA.B PlayerXPosNext
                      LDA.W CreditsSprXPosHigh,X
                      STA.B PlayerXPosNext+1
                      LDA.B #$01
                      STA.B Powerup
                      LDA.B #$08
                      STA.W PlayerXSpeed
                      JSR CODE_0CA75A
                      LDA.W CreditsSprXPosLow
                      CLC
                      ADC.B #$30
                      STA.B SpriteXPosLow
                      LDA.W CreditsSprXPosHigh
                      ADC.B #$00
                      STA.W SpriteXPosHigh
                      LDA.B #$60
                      STA.B SpriteYPosLow
                      LDA.B #$01
                      STA.W SpriteYPosHigh
                      LDA.B #$30
                      STA.W SpriteOAMIndex
                      JSR CODE_0CA778
                      STZ.B _1
                      LDA.W CreditsSprXPosLow
                      CLC
                      ADC.B #$10
                      STA.B _0
                      LDA.W CreditsSprXPosHigh
                      ADC.B #$00
                      STA.B _1
                      LDA.B #$9F
                      STA.B _2
                      DEC.W Layer1ScrollXPosUpd
                      BPL +
                      LDA.B #$06
                      STA.W Layer1ScrollXPosUpd
                      LDA.W Layer1ScrollXPosUpd+1
                      EOR.B #$01
                      STA.W Layer1ScrollXPosUpd+1
                    + LDA.W Layer1ScrollXPosUpd+1
                      CLC
                      ADC.B #$03
                      ASL A
                      ASL A
                      TAY
                      LDX.B #$A4
                      JSR CODE_0CA821
CODE_0CA2C3:          LDA.W CreditsSprXPosLow+7
                      STA.B _0
                      LDA.W CreditsSprXPosHigh+7
                      STA.B _1
                      REP #$20                                  ; A->16
                      LDA.B _0
                      CMP.W #$0030
                      BNE CODE_0CA2E0
                      LDA.W Layer1ScrollType
                      BNE CODE_0CA2FC
                      INC.W Layer1ScrollType
                      BRA CODE_0CA2FC

CODE_0CA2E0:          SEP #$20                                  ; A->8
                      LDX.B #$0D
CODE_0CA2E4:          CPX.B #$0B
                      BCC CODE_0CA2EF
                      LDA.W CreditsSprXPosLow+$0B
                      CMP.B #$98
                      BEQ +
CODE_0CA2EF:          LDA.B #$F8
                      STA.W CreditsSprXSpeed,X
                      JSR CODE_0CA74F
                    + DEX
                      CPX.B #$06
                      BNE CODE_0CA2E4
CODE_0CA2FC:          SEP #$20                                  ; A->8
                      LDA.B #$AF
                      STA.B _E
                      JSR CODE_0CA8A3
                      LDA.B #$88
                      STA.B _2
                      JSR CODE_0CA8DF
                      RTS


DATA_0CA30D:          db $00,$01,$02,$01

DATA_0CA311:          db $00,$01,$01,$01

CODE_0CA315:          LDX.B #$01
                      LDA.W CreditsSprYSpeed,X
                      CLC
                      ADC.B #$01
                      STA.W CreditsSprYSpeed,X
                      JSR CODE_0CA721
                      LDA.W CreditsSprYPosLow,X
                      CMP.B #$9F
                      BCC +
                      LDA.B #$F0
                      STA.W CreditsSprYSpeed,X
                      LDA.B #$9F
                      STA.W CreditsSprYPosLow,X
                    + LDY.B #$00
                      LDA.W CreditsSprYSpeed+1
                      BPL CODE_0CA349
                      CMP.B #$F4
                      BCC CODE_0CA355
                      LDY.B #$01
                      CMP.B #$F8
                      BCC CODE_0CA355
                      LDY.B #$02
                      BRA CODE_0CA349

CODE_0CA349:          CMP.B #$0C
                      BCS CODE_0CA355
                      LDY.B #$01
                      CMP.B #$08
                      BCS CODE_0CA355
                      LDY.B #$02
CODE_0CA355:          STY.W Layer3ScrollXSpeed
                      LDA.W CreditsJumpingYoshi
                      BEQ +
                      CMP.B #$01
                      BNE +
                      LDA.B #$18
                      STA.B _0
                      STZ.B _1
                      LDA.W CreditsSprYPosLow+1
                      STA.B _2
                      LDY.W Layer3ScrollXSpeed
                      LDA.W DATA_0CA30D,Y
                      CLC
                      ADC.B #$07
                      ASL A
                      ASL A
                      TAY
                      LDX.B #$D0
                      JSR CODE_0CA821
                      LDA.B #$D4
                      STA.B _0
                      STZ.B _1
                      LDA.W CreditsSprYPosLow+1
                      STA.B _2
                      LDY.W Layer3ScrollXSpeed
                      LDA.W DATA_0CA30D,Y
                      CLC
                      ADC.B #$0A
                      ASL A
                      ASL A
                      TAY
                      LDX.B #$F0
                      JSR CODE_0CA821
                      LDY.W Layer3ScrollXSpeed
                      LDA.W DATA_0CA311,Y
                      ASL A
                      TAY
                      LDA.B #$50
                      STA.B _0
                      LDA.W CreditsSprYPosLow+1
                      STA.B _2
                      LDX.B #$F0
                      LDA.B #$00
                      STA.B _E
                      JSR CODE_0CAA15
                    + RTS


DATA_0CA3B4:          db $40,$50,$60,$70,$80,$90,$A0

DATA_0CA3BB:          db $AF,$AB,$AF,$AB,$AF,$AB,$AF

DATA_0CA3C2:          db $F6,$00,$F6,$00,$F6,$00,$F6

CODE_0CA3C9:          PHB
                      PHK
                      PLB
                      LDA.B #$01
                      STA.W CreditsJumpingYoshi
                      STZ.B PlayerDirection
                      STZ.W Layer1ScrollType
                      LDA.B #$00
                      STA.W CreditsSprXPosLow
                      LDA.B #$01
                      STA.W CreditsSprXPosHigh
                      STZ.W CreditsSprXPosSpx
                      STZ.W CreditsSprXPosSpx+1
                      STZ.W CreditsSprXPosSpx+7
                      LDA.B #$06
                      STA.W Layer1ScrollXPosUpd
                      STZ.W Layer1ScrollXPosUpd+1
                      STZ.W Layer3ScrollXSpeed
                      LDA.B #$06
                      STA.W Layer3ScrollXSpeed+1
                      LDA.B #$F0
                      STA.W CreditsSprYSpeed+1
                      LDA.B #$9F
                      STA.W CreditsSprYPosLow+1
                      LDA.B #con($E2,$E2,$E2,$E2,$FE)
                      STA.W MessageBoxTimer
                      STA.W LevelLoadObject
                      LDA.B #!BGM_CREDITSYOSHISHOUSE
                      STA.W SPCIO2                              ; / Change music
                      LDX.B #$0D
                      LDY.B #$06
                    - LDA.W DATA_0CA3BB,Y
                      STA.W CreditsSprYPosLow,X
                      STZ.W CreditsSprYPosHigh,X
                      LDA.W DATA_0CA3B4,Y
                      STA.W CreditsSprXPosLow,X
                      LDA.W DATA_0CA3C2,Y
                      STA.W CreditsSprYSpeed,X
                      LDA.B #$01
                      STA.W GfxDecompOWAni,X
                      STA.W CreditsSprXPosHigh,X
                      DEY
                      DEX
                      CPX.B #$06
                      BNE -
                      PLB
                      RTL


DATA_0CA439:          db $00,$02,$02

CODE_0CA43C:          LDA.W Layer3ScrollXSpeed
                      CMP.B #$08
                      BCS CODE_0CA45C
                      JSR CODE_0CA315
                      LDA.W Layer3ScrollXSpeed
                      BNE +
                      LDA.W CreditsSprYPosLow+1
                      CMP.B #$9F
                      BNE +
                      LDA.B #$08
                      STA.W Layer3ScrollXSpeed
                    + LDY.B #$00
                      BRL CODE_0CA510
CODE_0CA45C:          DEC.W Layer3ScrollXSpeed+1
                      BPL CODE_0CA478
                      LDA.B #$06
                      STA.W Layer3ScrollXSpeed+1
                      INC.W Layer3ScrollXSpeed
                      LDA.W Layer3ScrollXSpeed
                      CMP.B #$0B
                      BNE CODE_0CA478
                      LDA.B #$0A
                      STA.W Layer3ScrollXSpeed
                      INC.W Layer1ScrollType
CODE_0CA478:          LDA.W Layer3ScrollXSpeed
                      AND.B #$03
                      BNE CODE_0CA4B6
                      LDA.B #$18
                      STA.B _0
                      STZ.B _1
                      LDA.B #$9F
                      STA.B _2
                      LDA.W Layer3ScrollXSpeed
                      AND.B #$03
                      CLC
                      ADC.B #$07
                      ASL A
                      ASL A
                      TAY
                      LDX.B #$D0
                      JSR CODE_0CA821
                      LDA.B #$D4
                      STA.B _0
                      STZ.B _1
                      LDA.B #$9F
                      STA.B _2
                      LDA.W Layer3ScrollXSpeed
                      AND.B #$03
                      CLC
                      ADC.B #$0A
                      ASL A
                      ASL A
                      TAY
                      LDX.B #$F0
                      JSR CODE_0CA821
                      BRL +
CODE_0CA4B6:          LDA.B #$18
                      STA.B _0
                      STZ.B _1
                      LDA.B #$A7
                      STA.B _2
                      LDA.W Layer3ScrollXSpeed
                      AND.B #$03
                      DEC A
                      ASL A
                      ASL A
                      TAY
                      LDX.B #$C0
                      JSR CODE_0CA99E
                      LDA.B #$CC
                      STA.B _0
                      STZ.B _1
                      LDA.B #$A7
                      STA.B _2
                      LDA.W Layer3ScrollXSpeed
                      AND.B #$03
                      DEC A
                      CLC
                      ADC.B #$02
                      ASL A
                      ASL A
                      TAY
                      LDX.B #$D8
                      JSR CODE_0CA99E
                    + LDA.W Layer3ScrollXSpeed
                      AND.B #$03
                      TAY
                      LDA.W DATA_0CA439,Y
                      ASL A
                      TAY
                      LDA.B #$50
                      STA.B _0
                      LDA.B #$9F
                      STA.B _2
                      LDX.B #$F0
                      LDA.B #$00
                      STA.B _E
                      JSR CODE_0CAA15
                      LDA.W Layer3ScrollXSpeed
                      AND.B #$03
                      TAY
                      LDA.W DATA_0CA439,Y
                      ASL A
                      TAY
CODE_0CA510:          LDA.B SpriteXPosLow
                      STA.B _0
                      LDA.B #$9F
                      STA.B _2
                      LDX.B #$B4
                      LDA.B #$01
                      STA.B _E
                      JSR CODE_0CAA15
                      BRL CODE_0CA1F6

DATA_0CA524:          db $20,$01,$10,$40,$08,$02,$04

DATA_0CA52B:          db $10,$60,$20,$00,$30,$50,$40

CODE_0CA532:          LDA.W Layer3ScrollXPosUpd
                      BEQ CODE_0CA53A
                      BRL CODE_0CA5CB
CODE_0CA53A:          LDA.B #$98
                      STA.W CreditsSprYSpeed+3
                      STA.W CreditsSprYSpeed+4
                      LDA.B #$D4
                      STA.W CreditsSprYSpeed+5
                      STA.W CreditsSprYSpeed+6
                      LDY.W Layer3ScrollYSpeed+1
                      LDA.W DATA_0CA52B,Y
                      LSR A
                      LSR A
                      LSR A
                      LSR A
                      TAX
                      INC A
                      STA.W Layer3ScrollXPosUpd
                      LDA.B #$C0
                      STA.W CreditsSprYSpeed+7,X
                      LDA.B #$04
                      STA.W CreditsSprTimer,X                   ;0\A560;
                      LDA.W CreditsSprYPosLow+7,X
                      STA.W CreditsSprYPosLow+3
                      STA.W CreditsSprYPosLow+4
                      CLC
                      ADC.B #$08
                      STA.W CreditsSprYPosLow+5
                      STA.W CreditsSprYPosLow+6
                      STZ.W CreditsSprYPosHigh+3
                      STZ.W CreditsSprYPosHigh+4
                      STZ.W CreditsSprYPosHigh+5
                      STZ.W CreditsSprYPosHigh+6
                      LDA.W CreditsSprXPosLow+7,X
                      STA.W CreditsSprXPosLow+3
                      STA.W CreditsSprXPosLow+5
                      CLC
                      ADC.B #$08
                      STA.W CreditsSprXPosLow+4
                      STA.W CreditsSprXPosLow+6
                      STZ.W CreditsSprXPosHigh+3
                      STZ.W CreditsSprXPosHigh+4
                      STZ.W CreditsSprXPosHigh+5
                      STZ.W CreditsSprXPosHigh+6
                      LDA.W DATA_0CA524,Y
                      TSB.W Layer3ScrollYSpeed
                      INC.W Layer3ScrollYSpeed+1
                      LDA.W Layer3ScrollYSpeed+1
                      CMP.B #$08
                      BEQ CODE_0CA5B6
                      LDA.B #!SFX_EGGHATCH
                      STA.W SPCIO3                              ; / Play sound effect
                      BRA CODE_0CA5CB

CODE_0CA5B6:          STZ.W CreditsSprXPosHigh+2
                      LDA.B #$80
                      STA.W CreditsSprXPosLow+2
                      STZ.W CreditsSprXSpeed+2
                      LDA.B #!BGM_CREDITSTHANKYOU
                      STA.W SPCIO2                              ; / Change music
                      INC.W Layer1ScrollType
                      BRA +

CODE_0CA5CB:          JSR CODE_0CA5DE
                    + JSR CODE_0CA4B6
                      RTS


DATA_0CA5D2:          db $C8,$C8,$D8,$D8

DATA_0CA5D6:          db $26,$66,$26,$66

DATA_0CA5DA:          db $E8,$18,$F4,$0C

CODE_0CA5DE:          LDA.W Layer3ScrollXPosUpd
                      BEQ Return0CA65A
                      LDX.B #$06
CODE_0CA5E5:          LDA.W CreditsSprYSpeed,X
                      CLC
                      ADC.B #$06
                      CMP.B #$70
                      BEQ +
                      STA.W CreditsSprYSpeed,X
                    + TXA
                      SEC
                      SBC.B #$03
                      TAY
                      LDA.W DATA_0CA5DA,Y
                      STA.W CreditsSprXSpeed,X
                      JSR CODE_0CA721
                      JSR CODE_0CA74F
                      DEX
                      CPX.B #$02
                      BNE CODE_0CA5E5
                      LDA.W CreditsSprYPosLow+3
                      AND.B #$F0
                      CMP.B #$F0
                      BNE +
                      STZ.W Layer3ScrollXPosUpd
                    + LDX.B #$00
                      LDY.B #$06
CODE_0CA618:          LDA.W CreditsSprXPosHigh,Y
                      BNE +
                      LDA.W CreditsSprYPosHigh,Y
                      BNE +
                      LDA.W CreditsSprYPosLow,Y
                      CMP.B #$F0
                      BCS +
                      LDA.W CreditsSprXPosLow,Y
                      STA.W OAMTileXPos,X
                      LDA.W CreditsSprYPosLow,Y
                      STA.W OAMTileYPos,X
                      PHY
                      TYA
                      SEC
                      SBC.B #$03
                      TAY
                      LDA.W DATA_0CA5D2,Y
                      STA.W OAMTileNo,X
                      LDA.W DATA_0CA5D6,Y
                      STA.W OAMTileAttr,X
                      PLY
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      STZ.W OAMTileSize,X
                      PLX
                      INX
                      INX
                      INX
                      INX
                    + DEY
                      CPY.B #$02
                      BNE CODE_0CA618
Return0CA65A:         RTS

CODE_0CA65B:          LDX.B #$02
                      LDA.W CreditsSprXSpeed,X
                      SEC
                      SBC.B #$04
                      STA.W CreditsSprXSpeed,X
                      JSR CODE_0CA74F
                      LDA.W CreditsSprXPosLow,X
                      CMP.B #$F8
                      BCC +
                      INC.W Layer1ScrollType
                      LDA.B #$F0
                      STA.W Layer3ScrollXPosUpd+1
                      LDA.B #$00
                    + STA.B _0
                      STA.B _2
                      STA.B _4
                      STZ.B _1
                      STZ.B _3
                      STZ.B _5
                      LDA.B #$3F
                      STA.B _8
                      STA.B _A
                      LDY.B #$00
                      LDX.B #$50
                      JSR CODE_0CA159
                      DEC.W Layer3ScrollXSpeed+1
                      BPL +
                      LDA.B #$06
                      STA.W Layer3ScrollXSpeed+1
                      DEC.W Layer3ScrollXSpeed
                      LDA.W Layer3ScrollXSpeed
                      CMP.B #$07
                      BNE +
                      LDA.B #$08
                      STA.W Layer3ScrollXSpeed
                    + JSR CODE_0CA478
                      RTS

CODE_0CA6B0:          JSR CODE_0CA146
                      JSR CODE_0CA315
                      JSR CODE_0CA2FC
                      LDA.B #$60
                      STA.B PlayerYPosNext
                      LDA.B #$01
                      STA.B PlayerYPosNext+1
                      STZ.W YoshiIsLoose
                      LDA.B #$26                                ; \ Mario's image = Peace Sign
                      STA.W PlayerPose                          ; /
                      JSR CODE_0CA764
                      JSR CODE_0CA7B4
                      LDY.W Layer3ScrollXSpeed
                      LDA.W DATA_0CA311,Y
                      ASL A
                      TAY
                      LDA.B SpriteXPosLow
                      STA.B _0
                      LDA.W CreditsSprYPosLow+1
                      STA.B _2
                      LDX.B #$B4
                      LDA.B #$01
                      STA.B _E
                      JSR CODE_0CAA15
                      STZ.B _1
                      LDA.W CreditsSprXPosLow
                      CLC
                      ADC.B #$10
                      STA.B _0
                      LDA.W CreditsSprXPosHigh
                      ADC.B #$00
                      STA.B _1
                      LDA.B #$9F
                      STA.B _2
                      LDA.B TrueFrame
                      AND.B #$08
                      LSR A
                      LSR A
                      LSR A
                      CLC
                      ADC.B #$05
                      ASL A
                      ASL A
                      TAY
                      LDX.B #$A4
                      JSR CODE_0CA821
                      DEC.W Layer3ScrollXPosUpd+1
                      LDA.W Layer3ScrollXPosUpd+1
                      BNE +
                      INC.W GameMode
                      LDA.B #$40
                      STA.W Layer3ScrollYSpeed+1
                    + RTS

CODE_0CA721:          LDA.W CreditsSprYSpeed,X
                      ASL A
                      ASL A
                      ASL A
                      ASL A
                      CLC
                      ADC.W CreditsSprYPosSpx,X
                      STA.W CreditsSprYPosSpx,X
                      PHP
                      LDA.W CreditsSprYSpeed,X
                      LSR A
                      LSR A
                      LSR A
                      LSR A
                      CMP.B #$08
                      LDY.B #$00
                      BCC +
                      ORA.B #$F0
                      DEY
                    + PLP
                      ADC.W CreditsSprYPosLow,X
                      STA.W CreditsSprYPosLow,X
                      TYA
                      ADC.W CreditsSprYPosHigh,X
                      STA.W CreditsSprYPosHigh,X
                      RTS

CODE_0CA74F:          PHX
                      TXA
                      CLC
                      ADC.B #$0F
                      TAX
                      JSR CODE_0CA721
                      PLX
                      RTS

CODE_0CA75A:          PHB
                      LDA.B #$00
                      PHA
                      PLB
                      JSL CODE_00CEB1
                      PLB
CODE_0CA764:          STZ.W PlayerBehindNet
                      LDA.W PlayerAniTimer
                      BEQ +
                      DEC.W PlayerAniTimer
                    + LDA.W CapeAniTimer
                      BEQ +
                      DEC.W CapeAniTimer
                    + RTS

CODE_0CA778:          LDX.B #$00
                      STX.W CurSpriteProcess
                      LDA.B #$35
                      STA.B SpriteNumber
                      LDA.W SpriteMisc1602
                      PHA
                      JSL InitSpriteTables
                      PLA
                      STA.W SpriteMisc1602
                      LDA.B #$02
                      STA.B SpriteTableC2
                      LDA.B #$01
                      STA.W SpriteMisc157C
                      LDA.W ScrollLayerIndex
                      CLC
                      ADC.B #$38
                      STA.W ScrollLayerIndex
                      BCC +
                      LDA.W SpriteMisc1602
                      INC A
                      STA.W SpriteMisc1602
                      CMP.B #$03
                      BCC +
                      STZ.W SpriteMisc1602
                    + LDA.B #$01
                      STA.W YoshiIsLoose
CODE_0CA7B4:          JSL DrawMarioAndYoshi
                      RTS


DATA_0CA7B9:          db $63

DATA_0CA7BA:          db $64,$68,$69,$63,$64,$68,$69,$4B
                      db $4C,$6B,$6C,$8A,$8B,$AA,$68,$8D
                      db $8E,$AD,$AE,$8A,$00,$AA,$44,$8A
                      db $0E,$AA,$2E,$81,$80,$A1,$A0,$84
                      db $83,$A4,$A3,$87,$86,$A7,$A6,$80
                      db $81,$A0,$A1,$83,$84,$A3,$A4,$86
                      db $87,$A6,$A7

DATA_0CA7ED:          db $21

DATA_0CA7EE:          db $21,$21,$21,$21,$21,$21,$21,$21
                      db $21,$21,$21,$21,$21,$21,$21,$20
                      db $20,$20,$20,$21,$21,$21,$21,$21
                      db $21,$21,$21,$78,$78,$78,$78,$78
                      db $78,$78,$78,$78,$78,$78,$78,$34
                      db $34,$34,$34,$34,$34,$34,$34,$34
                      db $34,$34,$34

CODE_0CA821:          REP #$30                                  ; AXY->16
                      TXA
                      AND.W #$00FF
                      TAX
                      TYA
                      AND.W #$00FF
                      CMP.W #$0028
                      BCC +
                      TXA
                      ORA.W #$0100
                      TAX
                    + SEP #$20                                  ; A->8
                      LDA.B #$01
                      STA.B _4
CODE_0CA83C:          LDA.B _1
                      BNE CODE_0CA8A0
                      LDA.B #$02
                      STA.B _A
                      LDA.B _0
                      STA.W OAMTileXPos+4,X
                      CLC
                      ADC.B #$08
                      STA.W OAMTileXPos,X
                      BCC +
                      LDA.B #$01
                      STA.B _A
                    + LDA.B _2
                      STA.W OAMTileYPos,X
                      STA.W OAMTileYPos+4,X
                      LDA.W DATA_0CA7B9,Y
                      STA.W OAMTileNo+4,X
                      LDA.W DATA_0CA7BA,Y
                      STA.W OAMTileNo,X
                      LDA.W DATA_0CA7ED,Y
                      STA.W OAMTileAttr+4,X
                      LDA.W DATA_0CA7EE,Y
                      STA.W OAMTileAttr,X
                      LDA.B _2
                      CLC
                      ADC.B #$10
                      STA.B _2
                      PHX
                      REP #$20                                  ; A->16
                      TXA
                      LSR A
                      LSR A
                      TAX
                      SEP #$20                                  ; A->8
                      LDA.B #$02
                      STA.W OAMTileSize+1,X
                      LDA.B _A
                      STA.W OAMTileSize,X
                      PLX
                      REP #$20                                  ; A->16
                      TXA
                      CLC
                      ADC.W #$0008
                      TAX
                      SEP #$20                                  ; A->8
                      INY
                      INY
                      DEC.B _4
                      BPL CODE_0CA83C
CODE_0CA8A0:          SEP #$30                                  ; AXY->8
                      RTS

CODE_0CA8A3:          LDX.B #$0D
CODE_0CA8A5:          LDA.W CreditsSprYSpeed,X
                      CLC
                      ADC.W GfxDecompOWAni,X
                      STA.W CreditsSprYSpeed,X
                      JSR CODE_0CA721
                      LDA.W CreditsSprYPosLow,X
                      CMP.B _E
                      BCC +
                      STZ.W CreditsSprYPosSpx,X
                      LDA.B #$F6
                      STA.W CreditsSprYSpeed,X
                      LDA.B #$01
                      STA.W GfxDecompOWAni,X
                      LDA.B _E
                      STA.W CreditsSprYPosLow,X
                    + DEX
                      CPX.B #$06
                      BNE CODE_0CA8A5
                      RTS


                      db $00,$10,$20,$30,$40,$50,$60

DATA_0CA8D8:          db $68,$26,$24,$6A,$28,$64,$26

CODE_0CA8DF:          LDA.W Layer3ScrollYSpeed
                      STA.B _E
                      LDY.B #$0D
CODE_0CA8E6:          LDA.W CreditsSprXPosHigh,Y
                      BNE CODE_0CA934
                      LDA.W CreditsSprXPosLow,Y
                      STA.B _0
                      LDA.W CreditsSprYPosLow,Y
                      STA.B _1
                      LDX.B _2
                      LDA.B _0
                      STA.W OAMTileXPos,X
                      LDA.B _1
                      STA.W OAMTileYPos,X
                      LSR.B _E
                      BCS CODE_0CA911
                      LDA.B #$86
                      STA.W OAMTileNo,X
                      LDA.B #$21
                      STA.W OAMTileAttr,X
                      BRA +

CODE_0CA911:          PHY
                      TYA
                      SEC
                      SBC.B #$07
                      TAY
                      LDA.B #$EA
                      STA.W OAMTileNo,X
                      LDA.W DATA_0CA8D8,Y
                      STA.W OAMTileAttr,X
                      PLY
                    + TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.B #$02
                      STA.W OAMTileSize,X
                      INC.B _2
                      INC.B _2
                      INC.B _2
                      INC.B _2
CODE_0CA934:          DEY
                      CPY.B #$06
                      BNE CODE_0CA8E6
                      RTS


DATA_0CA93A:          db $BB

DATA_0CA93B:          db $B9,$DC,$DB,$DA,$D9,$8B,$89,$AC
                      db $AB,$AA,$A9,$B9,$BB,$D9,$DA,$DB
                      db $DC,$89,$8B,$A9,$AA,$AB,$AC

DATA_0CA952:          db $78

DATA_0CA953:          db $78,$78,$78,$78,$78,$78,$78,$78
                      db $78,$78,$78,$34,$34,$34,$34,$34
                      db $34,$34,$34,$34,$34,$34,$34

DATA_0CA96A:          db $00

DATA_0CA96B:          db $10,$00,$08,$10,$18,$00,$10,$00
                      db $08,$10,$18,$00,$10,$00,$08,$10
                      db $18,$00,$10,$00,$08,$10,$18

DATA_0CA982:          db $00

DATA_0CA983:          db $00,$10,$10,$10,$10,$00,$00,$10
                      db $10,$10,$10,$00,$00,$10,$10,$10
                      db $10,$00,$00,$10,$10,$10,$10

DATA_0CA99A:          db $00,$06,$0C,$12

CODE_0CA99E:          TYA
                      LSR A
                      LSR A
                      TAY
                      LDA.W DATA_0CA99A,Y
                      TAY
                      LDA.B #$02
                      STA.B _4
CODE_0CA9AA:          LDA.W DATA_0CA96A,Y
                      CLC
                      ADC.B _0
                      STA.W OAMTileXPos,X
                      LDA.W DATA_0CA96B,Y
                      CLC
                      ADC.B _0
                      STA.W OAMTileXPos+4,X
                      LDA.W DATA_0CA982,Y
                      CLC
                      ADC.B _2
                      STA.W OAMTileYPos,X
                      LDA.W DATA_0CA983,Y
                      CLC
                      ADC.B _2
                      STA.W OAMTileYPos+4,X
                      LDA.W DATA_0CA93A,Y
                      STA.W OAMTileNo,X
                      LDA.W DATA_0CA93B,Y
                      STA.W OAMTileNo+4,X
                      LDA.W DATA_0CA952,Y
                      STA.W OAMTileAttr,X
                      LDA.W DATA_0CA953,Y
                      STA.W OAMTileAttr+4,X
                      PHY
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDY.B #$02
                      LDA.B _4
                      AND.B #$02
                      BNE +
                      LDY.B #$00
                    + TYA
                      STA.W OAMTileSize,X
                      STA.W OAMTileSize+1,X
                      PLX
                      PLY
                      TXA
                      CLC
                      ADC.B #$08
                      TAX
                      INY
                      INY
                      DEC.B _4
                      BPL CODE_0CA9AA
                      RTS


DATA_0CAA0B:          db $C4

DATA_0CAA0C:          db $E4,$E6,$E8,$CE,$EE

DATA_0CAA11:          db $36

DATA_0CAA12:          db $36,$3A,$3A

CODE_0CAA15:          PHY
                      LDA.B _E
                      ASL A
                      TAY
                      LDA.W DATA_0CAA11,Y
                      STA.W OAMTileAttr,X
                      LDA.W DATA_0CAA12,Y
                      STA.W OAMTileAttr+4,X
                      PLY
                      LDA.B _0
                      STA.W OAMTileXPos,X
                      STA.W OAMTileXPos+4,X
                      LDA.B _2
                      STA.W OAMTileYPos,X
                      CLC
                      ADC.B #$10
                      STA.W OAMTileYPos+4,X
                      LDA.W DATA_0CAA0B,Y
                      STA.W OAMTileNo,X
                      LDA.W DATA_0CAA0C,Y
                      STA.W OAMTileNo+4,X
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.B #$02
                      STA.W OAMTileSize,X
                      STA.W OAMTileSize+1,X
                      RTS


DATA_0CAA53:          db $53

DATA_0CAA54:          db $6A

DATA_0CAA55:          db $80

DATA_0CAA56:          db $3C,$63,$6A,$82,$3C,$53,$7A,$A0
                      db $3C,$63,$7A,$A2,$3C,$53,$8A,$84
                      db $3C,$63,$8A,$86,$3C,$53,$9A,$A4
                      db $3C,$63,$9A,$A6,$3C,$53,$AA,$88
                      db $3C,$63,$AA,$8A,$3C,$8D,$5A,$A8
                      db $3A,$9D,$5A,$AA,$3A,$8D,$6A,$8C
                      db $3A,$9D,$6A,$8E,$3A,$8D,$7A,$AC
                      db $3A,$9D,$7A,$AE,$3A,$8D,$8A,$63
                      db $3A,$9D,$8A,$65,$3A,$8D,$9A,$48
                      db $3A,$9D,$9A,$68,$3A,$8D,$AA,$6B
                      db $3A,$9D,$AA,$6D,$3A,$78,$58,$4D
                      db $3E,$70,$68,$E0,$3F,$80,$68,$C4
                      db $3F,$70,$78,$4A,$3F,$80,$78,$4C
                      db $3F,$70,$88,$6A,$3F,$80,$88,$6C
                      db $3F,$68,$98,$AB,$3F,$78,$98,$C8
                      db $3F,$88,$98,$E6,$3F,$68,$A8,$E8
                      db $3F,$78,$A8,$EA,$3F,$88,$A8,$EC
                      db $3F

CODE_0CAADF:          PHB
                      PHK
                      PLB
                      LDX.B #$00
                      TXY
                    - LDA.W DATA_0CAA53,Y
                      STA.W OAMTileXPos,X
                      LDA.W DATA_0CAA54,Y
                      STA.W OAMTileYPos,X
                      LDA.W DATA_0CAA55,Y
                      STA.W OAMTileNo,X
                      LDA.W DATA_0CAA56,Y
                      STA.W OAMTileAttr,X
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.B #$02
                      STA.W OAMTileSize,X
                      PLX
                      INX
                      INX
                      INX
                      INX
                      TXY
                      CPY.B #$8C
                      BNE -
                      PLB
                      RTL

CODE_0CAB13:          PHB                                       ; Wrapper
                      PHK
                      PLB
                      JSR CODE_0CAB1F
                      PLB
                      RTL


DATA_0CAB1B:          db $FE,$02

                   if ver_is_lores(!_VER)                       ;\================ J, U, SS, & E0 ===============
DATA_0CAB1D:          db $00,$E0                                ;!
                   else                                         ;<====================== E1 =====================
DATA_0CAB1D:          db $00,$FE                                ;!
                   endif                                        ;/===============================================

CODE_0CAB1F:          LDX.W MessageBoxExpand
                      LDA.B #$33
                      STA.B Layer12Window
                      LDY.W LevelLoadObject
                      BNE CODE_0CAB2F
                      CPX.B #$00
                      BEQ CODE_0CAB3B
CODE_0CAB2F:          CPY.B #$06
                      BCC CODE_0CAB39
                      BNE CODE_0CAB3B
                      CPX.B #$00
                      BNE CODE_0CAB3B
CODE_0CAB39:          LDA.B #$30
CODE_0CAB3B:          STA.B OBJCWWindow
                      LDA.W MessageBoxTimer
                      CMP.W DATA_0CAB1D,X
                      BNE CODE_0CAB64
                      CPX.B #$00
                      BEQ CODE_0CAB6B
                      LDX.B #$00
                      STX.W MessageBoxExpand
                      CPY.B #$06
                      BCC CODE_0CAB57
                      INC.W GameMode
                      BRA CODE_0CAB6B

CODE_0CAB57:          PHA
                      PHX
                      INC.W CreditsUpdateBG
                      INC.W LevelLoadObject
                      JSR CODE_0CABB2
                      PLX
                      PLA
CODE_0CAB64:          CLC
                      ADC.W DATA_0CAB1B,X
                      STA.W MessageBoxTimer
                   if ver_is_lores(!_VER)                       ;\================ J, U, SS, & E0 ===============
CODE_0CAB6B:          REP #$20                                  ;! A->16
                      LDX.B #$00                                ;!
                      LDY.B #$E0                                ;!
                      LDA.W #$00FF                              ;!
CODE_0CAB74:          CPX.W MessageBoxTimer                     ;!
                      BCC +                                     ;!
                      LDA.W #$FF00                              ;!
                    + STA.W WindowTable,X                       ;!
                      STA.W WindowTable+$DE,Y                   ;!
                      INX                                       ;!
                      INX                                       ;!
                      DEY                                       ;!
                      DEY                                       ;!
                      BNE CODE_0CAB74                           ;!
                      SEP #$20                                  ;! A->8
                   else                                         ;<======================= E1 ====================
CODE_0CAB6B:          REP #$30                                  ;! AXY->16
                      LDA.W MessageBoxTimer                     ;!
                      AND.W #$00FF                              ;!
                      STA.W CreditsLetterbox                    ;!
                      LDX.W #$0000                              ;!
                      LDY.W #$00FE                              ;!
                      LDA.W #$00FF                              ;!
                    - CPX.W CreditsLetterbox                    ;!
                      BCC +                                     ;!
                      BEQ +                                     ;!
                      LDA.W #$FF00                              ;!
                    + STA.W WindowTable+$DE,Y                   ;!
                      CPX.W #$001E                              ;!
                      BMI +                                     ;!
                      STA.W WindowTable-$1E,X                   ;!
                    + INX                                       ;!
                      INX                                       ;!
                      DEY                                       ;!
                      DEY                                       ;!
                      BNE -                                     ;!
                      SEP #$30                                  ;! AXY->8
                   endif                                        ;/===============================================
                      LDA.B #$13
                      STA.W HW_TMW
                      STA.W HW_TSW
                      LDA.B #$22
                      STA.B ColorAddition
                      LDA.B #$80
                      STA.W HDMAEnable
                      RTS


DATA_0CAB9C:          db $00,$18,$30,$48,$60,$78,$90,$A8
DATA_0CABA4:          db $06,$00,$00,$02,$05,$06,$00

DATA_0CABAB:          db $03,$03,$07,$00,$01,$02,$00

CODE_0CABB2:          SEP #$30                                  ; AXY->8
                      LDA.B #$0C
                      STA.W DynPaletteTable
                      STA.W DynPaletteTable+$0E
                      LDA.B #$02
                      STA.W DynPaletteTable+1
                      LDA.B #$12
                      STA.W DynPaletteTable+$0F
                      REP #$30                                  ; AXY->16
                      LDA.W LevelLoadObject
                      AND.W #$00FF
                      TAY
                      LDA.W DATA_0CABA4,Y
                      AND.W #$000F
                      ASL A
                      TAX
                      LDA.L BackAreaColors,X
                      STA.W BackgroundColor
                      LDA.W #BackgroundPalettes
                      STA.B _0
                      LDA.W DATA_0CABAB,Y
                      AND.W #$000F
                      TAX
                      %LorW_X(LDA,DATA_0CAB9C)
                      AND.W #$00FF
                      CLC
                      ADC.B _0
                      STA.B _0
                      STZ.B _2
                      LDA.W #$0000
                      STA.B _4
                      LDA.W #$0001
                      STA.B _8
CODE_0CAC01:          LDX.B _4
                      LDY.W #$0005
                    - LDA.B [_0]
                      STA.W DynPaletteTable+2,X
                      INC.B _0
                      INC.B _0
                      INX
                      INX
                      DEY
                      BPL -
                      LDA.B _4
                      CLC
                      ADC.W #$000E
                      STA.B _4
                      DEC.B _8
                      BPL CODE_0CAC01
                      LDA.W #$0000
                      STA.W DynPaletteTable+2,X
                      SEP #$30                                  ; AXY->8
                      RTS


DATA_0CAC29:          db $E0,$00,$00,$00,$00,$26,$2B,$41
                      db $0A,$F4,$10,$26,$3B,$51,$2B,$37
                      db $00,$2C,$40,$3F,$FF

DATA_0CAC3E:          db $00,$20,$05
                      db $01,$00,$26,$3A,$57,$26,$4B,$54
                      db $0C,$F3,$30,$2C,$38,$33,$2C,$49
                      db $36,$FF

DATA_0CAC53:          db $40,$40,$05,$00,$00,$04
                      db $92,$02,$26,$4B,$94,$28,$49,$71
                      db $26,$3A,$1B,$27,$40,$53,$27,$34
                      db $58,$28,$38,$57,$2B,$16,$10,$2D
                      db $40,$27,$2D,$38,$25,$FF

DATA_0CAC77:          db $20,$C0
                      db $08,$07,$07,$05,$96,$01,$23,$2C
                      db $21,$23,$3C,$C1,$26,$3A,$60,$07
                      db $6B,$90,$2B,$32,$47,$2B,$43,$40
                      db $2B,$34,$48,$2D,$38,$27,$2D,$49
                      db $21,$2D,$3B,$28,$FF

DATA_0CAC9E:          db $00,$20,$0A
                      db $01,$00,$23,$2C,$21,$23,$3C,$81
                      db $26,$3A,$50,$07,$6B,$40,$25,$23
                      db $10,$25,$34,$11,$27,$31,$40,$08
                      db $62,$31,$27,$34,$47,$27,$45,$40
                      db $27,$36,$48,$2C,$40,$3F,$FF

DATA_0CACC8:          db $C0
                      db $60,$03,$22,$03,$21,$26,$46,$6C
                      db $A6,$39,$64,$61,$12,$6A,$60,$55
                      db $FF

DATA_0CACDA:          db $80,$40,$04,$1A,$09,$60,$68
                      db $24,$65,$68,$14,$08,$0B,$7F,$09
                      db $08,$7F,$6C,$60,$33,$FF

DATA_0CACEF:          db $60,$40
                      db $07,$2C,$05,$65,$83,$09,$6A,$8A
                      db $02,$66,$94,$91,$66,$99,$91,$6B
                      db $9B,$71,$FF

DATA_0CAD04:          db $C0,$80,$01,$0B,$01
                      db $60,$C8,$22,$63,$CD,$42,$64,$C1
                      db $24,$65,$C6,$21,$65,$CA,$22,$69
                      db $C3,$11,$6B,$CA,$24,$FF

DATA_0CAD1F:          db $60,$A0
                      db $01,$0B,$01,$60,$C3,$22,$64,$C9
                      db $23,$69,$C1,$42,$2A,$84,$3B,$FF

DATA_0CAD31:          db $C0,$20,$01,$13,$01,$29,$C0,$0F
                      db $2C,$A0,$3F,$FF

DATA_0CAD3D:          db $A0,$60,$0C,$0B
                      db $01,$28,$40,$76,$28,$37,$75,$29
                      db $A8,$61,$28,$3A,$73,$28,$4B,$74
                      db $FF

DATA_0CAD52:          db $01,$60,$00,$20,$01,$FF

DATA_0CAD58:          dw DATA_0CAC29
                      dw DATA_0CAC3E
                      dw DATA_0CAC53
                      dw DATA_0CAC77
                      dw DATA_0CAC9E
                      dw DATA_0CACC8
                      dw DATA_0CACDA
                      dw DATA_0CACEF
                      dw DATA_0CAD04
                      dw DATA_0CAD1F
                      dw DATA_0CAD31
                      dw DATA_0CAD3D
                      dw DATA_0CAD52

DATA_0CAD72:          dw DATA_0CEC82
                      dw DATA_0CDD44
                      dw DATA_0CDF59
                      dw DATA_0CD900
                      dw DATA_0CDE54
                      dw DATA_0CE8FE
                      dw DATA_0CDAB9
                      dw DATA_0CEF80
                      dw DATA_0CF45A
                      dw DATA_0CE103
                      dw DATA_0CF45A
                      dw DATA_0CE103
                      dw DATA_0CAD52

CODE_0CAD8C:          PHB
                      PHK
                      PLB
                      INC.W CreditsScreenNumber
                      LDX.B #$FF
                      LDA.W CreditsScreenNumber
                      CMP.B #$0C
                      BNE +
                      LDX.B #$0C
                    + ASL A
                      TAY
                      LDA.B #$0C
                      STA.B Layer1DataPtr+2
                      STX.B Layer2DataPtr+2
                      REP #$20                                  ; A->16
                      LDA.W DATA_0CAD58,Y
                      STA.B Layer1DataPtr
                      LDA.W DATA_0CAD72,Y
                      STA.B Layer2DataPtr
                      SEP #$20                                  ; A->8
                      PLB
                      RTL


DATA_0CADB5:          db $28,$28,$44,$28,$38,$20,$28,$20
                      db $08,$28,$7C,$68,$28

DATA_0CADC2:          db $00,$00,$88,$00,$E0,$00,$C0,$00
                      db $E8,$00,$00,$00,$A0,$00,$50,$00
                      db $B0,$00,$E0,$00,$18,$00,$E0,$00
                      db $00,$00

                   if ver_is_english(!_VER)                     ;\=============== U, SS, E0, & E1 ===============
DATA_0CADDC:          dw SpecEneNameStripe00                    ;!
                      dw SpecEneNameStripe01                    ;!
                      dw SpecEneNameStripe02                    ;!
                      dw SpecEneNameStripe03                    ;!
                      dw SpecEneNameStripe04                    ;!
                      dw SpecEneNameStripe05                    ;!
                      dw SpecEneNameStripe06                    ;!
                      dw SpecEneNameStripe07                    ;!
                      dw SpecEneNameStripe08                    ;!
                      dw SpecEneNameStripe09                    ;!
                      dw SpecEneNameStripe0A                    ;!
                      dw SpecEneNameStripe0B                    ;!
                      dw SpecEneNameStripe0C                    ;!
                   endif                                        ;/===============================================

CODE_0CADF6:          PHB
                      PHK
                      PLB
                      LDA.W CreditsScreenNumber
                      TAY
                      ASL A
                      TAX
                      CLC
                      ADC.W CreditsScreenNumber
                      CLC
                      ADC.B #OtherStripes-StripeImages+$0F
                      STA.B StripeImage
                   if ver_is_english(!_VER)                     ;\================ U, SS, E0, & E1 ==============
                      PHY                                       ;!
                      PHX                                       ;!
                      JSL CODE_0084C8                           ;!
                      LDA.W OWLevelTileSettings+$49             ;!
                      BPL CODE_0CAE48                           ;!
                      LDA.W CreditsScreenNumber                 ;!
                      ASL A                                     ;!
                      TAY                                       ;!
                      LDA.W DATA_0CADDC,Y                       ;!
                      STA.W _0                                  ;!
                      LDA.W DATA_0CADDC+1,Y                     ;!
                      STA.W _1                                  ;!
                      LDA.B #$0D                                ;!
                      STA.W _2                                  ;!
                      LDY.B #$00                                ;!
                      LDA.L DynStripeImgSize                    ;!
                      TAX                                       ;!
                    - REP #$20                                  ;! A->16
                      LDA.B [_0],Y                              ;!
                      STA.L DynamicStripeImage,X                ;!
                      INY                                       ;!
                      INY                                       ;!
                      INX                                       ;!
                      INX                                       ;!
                      CMP.W #$FFFF                              ;!
                      BNE -                                     ;!
                      TXA                                       ;!
                      STA.L DynStripeImgSize                    ;!
                      SEP #$20                                  ;! A->8
CODE_0CAE48:          PLX                                       ;!
                      PLY                                       ;!
                   endif                                        ;/===============================================
                      LDA.W DATA_0CADB5,Y
                      STA.W CreditsL1HDMATable+3
                      STA.W CreditsL2HDMATable+3
                      STA.W CreditsL3HDMATable+3
                      LDA.B #$88
                      SEC
                      SBC.W DATA_0CADB5,Y
                      STA.W CreditsL1HDMATable+6
                      STA.W CreditsL2HDMATable+6
                      STA.W CreditsL3HDMATable+6
                      REP #$20                                  ; A->16
                      %LorW_X(LDA,DATA_0CADC2)
                      STA.B Layer2YPos
                      LDA.W CreditsScreenNumber
                      AND.W #$00FF
                      CMP.W #$000C
                      BNE CODE_0CAE88
                      STZ.B Layer1XPos
                      STZ.B Layer2XPos
                      STZ.B Layer3XPos
                      STZ.W NextLayer1XPos
                      STZ.W NextLayer2XPos
                      STZ.W Layer3HorizOffset
                      BRA +

CODE_0CAE88:          LDA.W #$FF00
                      STA.B Layer1XPos
                      STA.B Layer3XPos
                      LDA.W #$0100
                      STA.W NextLayer1XPos
                      STA.W Layer3HorizOffset
                      LDA.W #$FF80
                      STA.B Layer2XPos
                      LDA.W #$0080
                      STA.W NextLayer2XPos
                    + LDA.W #con($00FF,$00FF,$00FF,$00A0,$00A0)
                      STA.W Layer3ScrollYSpeed+1
                      SEP #$20                                  ; A->8
                      PLB
                      RTL

CODE_0CAEAD:          SEP #$20                                  ; A->8
                      LDA.W CreditsScreenNumber
                      CMP.B #$0C
                      BNE +
                      DEC.W Layer3ScrollYSpeed+1
                      JMP CODE_0CAEF8

                    + REP #$20                                  ; A->16
                      LDA.B Layer1XPos
                      CMP.W NextLayer1XPos
                      BNE +
                      LDA.W Layer3ScrollYSpeed+1
                      BEQ +
                      DEC.W Layer3ScrollYSpeed+1
                      JMP CODE_0CAF0C

                    + LDA.B Layer1XPos
                      CLC
                      ADC.W #$0002
                      STA.B Layer1XPos
                      STA.B Layer3XPos
                      LDA.W NextLayer1XPos
                      SEC
                      SBC.W #$0002
                      STA.W NextLayer1XPos
                      STA.W Layer3HorizOffset
                      INC.B Layer2XPos
                      DEC.W NextLayer2XPos
                      LDA.B Layer1XPos
                      AND.W NextLayer1XPos
                      AND.W #$00FF
                      SEP #$20                                  ; A->8
                      BNE CODE_0CAF0C
CODE_0CAEF8:          LDA.W Layer3ScrollYSpeed+1
                      BNE CODE_0CAF0C
                      INC.W GameMode
                      LDA.W CreditsScreenNumber
                      CMP.B #$0C
                      BEQ CODE_0CAF0C
                      LDA.B #$22
                      STA.W GameMode
CODE_0CAF0C:          SEP #$20                                  ; A->8
                      JMP CODE_0CB5BC


DATA_0CAF11:          db $29

DATA_0CAF12:          db $3C

DATA_0CAF13:          db $C2

DATA_0CAF14:          db $5F,$39,$3C,$C0,$5F,$5B,$34,$88
                      db $59,$5B,$40,$8A,$55,$29,$4C,$E2
                      db $5F,$39,$4C,$E0,$5F,$5B,$4C,$8C
                      db $55,$5B,$58,$8E,$55,$5C,$60,$C6
                      db $1F,$60,$A0,$82,$34,$60,$B0,$A2
                      db $34,$A0,$B0,$A4,$38,$80,$A0,$82
                      db $36,$C0,$A0,$82,$78,$80,$B0,$A0
                      db $36,$C0,$B0,$A2,$7A,$A0,$A0,$84
                      db $38,$FF,$92,$A3,$A2,$35,$A2,$A3
                      db $A4,$35,$80,$A9,$D7,$25,$86,$AC
                      db $D6,$25,$B5,$AF,$C7,$25,$BD,$B2
                      db $C6,$25,$92,$B3,$E6,$35,$A2,$B3
                      db $C8,$35,$B2,$B3,$A6,$35,$63,$B4
                      db $E0,$33,$73,$B4,$E0,$33,$83,$B4
                      db $E0,$33,$93,$B4,$E0,$33,$17,$2F
                      db $8C,$19,$37,$37,$8E,$19,$3F,$47
                      db $AA,$19,$1F,$5F,$EC,$99,$38,$8F
                      db $80,$3D,$50,$8F,$82,$7D,$90,$3C
                      db $AE,$1B,$A6,$45,$AE,$1B,$78,$54
                      db $C0,$1B,$FF,$43,$1E,$82,$5D,$30
                      db $22,$64,$5D,$72,$23,$0A,$15,$27
                      db $30,$66,$5D,$AD,$40,$AA,$1D,$84
                      db $6A,$8E,$33,$8C,$6A,$8E,$73,$84
                      db $7A,$AE,$33,$8C,$7A,$AE,$73,$BC
                      db $92,$A2,$23,$C4,$92,$A2,$63,$BC
                      db $9A,$B2,$23,$C4,$9A,$B2,$63,$48
                      db $93,$0C,$35,$58,$93,$0E,$35,$50
                      db $9A,$09,$25,$48,$A3,$0E,$F5,$58
                      db $A3,$0C,$F5,$FF,$6B,$67,$41,$5B
                      db $73,$67,$40,$5B,$A6,$67,$40,$1B
                      db $AE,$67,$41,$1B,$57,$6E,$72,$0B
                      db $66,$6E,$71,$4B,$BB,$6E,$70,$0B
                      db $42,$6F,$00,$1B,$4A,$6F,$01,$1B
                      db $6B,$6F,$51,$5B,$AE,$6F,$0A,$1B
                      db $7B,$77,$60,$4B,$A6,$77,$60,$0B
                      db $FF,$20,$37,$6D,$57,$38,$37,$5A
                      db $07,$40,$37,$4A,$07,$30,$3F,$46
                      db $17,$40,$3F,$48,$17,$20,$47,$C6
                      db $56,$50,$47,$C6,$16,$30,$4F,$2E
                      db $12,$40,$4F,$2E,$12,$90,$9C,$B6
                      db $29,$99,$9C,$B6,$29,$86,$A0,$B6
                      db $29,$A3,$A0,$B6,$29,$CB,$A4,$06
                      db $77,$C0,$A7,$1D,$67,$C8,$A7,$1C
                      db $67,$88,$AF,$CE,$3B,$90,$AF,$E2
                      db $29,$98,$AF,$E3,$29,$98,$AF,$CE
                      db $7B,$C0,$AF,$21,$77,$C8,$AF,$20
                      db $77,$38,$8F,$AE,$38,$38,$9F,$C5
                      db $2A,$40,$9F,$C5,$6A,$8C,$1F,$E0
                      db $1B,$94,$22,$E4,$09,$9C,$22,$E5
                      db $09,$9C,$27,$F2,$0B,$B4,$27,$E0
                      db $1B,$BC,$2A,$F4,$05,$C4,$2A,$F5
                      db $05,$C4,$2F,$F2,$09,$94,$3F,$E0
                      db $1B,$9C,$42,$E4,$09,$A4,$42,$E5
                      db $09,$A4,$47,$F2,$0B,$B4,$4F,$E2
                      db $18,$BC,$4F,$C9,$05,$BC,$57,$D9
                      db $05,$C4,$57,$C0,$05,$FF,$2C,$27
                      db $EE,$15,$28,$2F,$C5,$15,$30,$2F
                      db $C6,$15,$20,$3F,$2E,$10,$30,$3F
                      db $2E,$10,$40,$3F,$2E,$10,$30,$4F
                      db $CC,$15,$20,$5F,$CC,$15,$30,$5F
                      db $EC,$15,$40,$5F,$CC,$15,$89,$3F
                      db $8A,$15,$BA,$43,$86,$11,$CB,$47
                      db $3D,$00,$AE,$48,$3D,$00,$8A,$4F
                      db $E8,$15,$8A,$4F,$E8,$15,$D6,$4F
                      db $84,$51,$B7,$54,$3D,$00,$C8,$55
                      db $3D,$00,$89,$5F,$E8,$15,$89,$5F
                      db $E8,$15,$8A,$6F,$E8,$15,$74,$AF
                      db $62,$30,$84,$AF,$A6,$72,$FF,$80
                      db $8F,$80,$33,$90,$8F,$82,$33,$A0
                      db $8F,$84,$33,$B0,$8F,$86,$33,$80
                      db $9F,$A0,$33,$90,$9F,$88,$33,$A0
                      db $9F,$CE,$33,$B0,$9F,$EE,$33,$80
                      db $AF,$C0,$33,$90,$AF,$C2,$33,$A0
                      db $AF,$CE,$33,$B0,$AF,$EE,$33,$80
                      db $BF,$8E,$33,$90,$BF,$AE,$33,$A0
                      db $BF,$84,$B3,$B0,$BF,$86,$B3,$6C
                      db $2F,$8A,$57,$67,$3F,$AA,$57,$A4
                      db $4F,$66,$00,$A0,$53,$64,$00,$8C
                      db $57,$A8,$17,$9C,$57,$62,$00,$30
                      db $8F,$CC,$71,$40,$8F,$CA,$71,$30
                      db $9F,$EC,$71,$40,$9F,$EA,$71,$FF
                      db $30,$27,$EC,$1B,$28,$37,$EC,$1B
                      db $50,$40,$C0,$5D,$60,$40,$86,$5D
                      db $30,$47,$EC,$1B,$50,$50,$C2,$5D
                      db $60,$50,$A6,$5D,$98,$2F,$C8,$17
                      db $A8,$2F,$C8,$57,$A0,$37,$CA,$17
                      db $98,$3F,$C8,$97,$A8,$3F,$C8,$D7
                      db $44,$8F,$AC,$37,$22,$94,$F1,$2B
                      db $20,$9C,$F0,$2B,$25,$A4,$E1,$2B
                      db $4C,$A7,$AE,$77,$24,$AB,$E0,$2B
                      db $24,$AF,$8C,$37,$B2,$A7,$A0,$77
                      db $C2,$A7,$80,$77,$BA,$9E,$84,$77
                      db $88,$AF,$A4,$77,$88,$BF,$66,$76
                      db $90,$BF,$64,$76,$98,$BF,$62,$76
                      db $A8,$BF,$82,$77,$B8,$BF,$80,$77
                      db $FF,$28,$26,$E6,$57,$20,$36,$C2
                      db $51,$48,$36,$E6,$17,$51,$45,$E8
                      db $1B,$A8,$4A,$60,$14,$B0,$4A,$60
                      db $14,$AC,$3E,$A8,$1B,$AA,$4E,$60
                      db $14,$AE,$4E,$60,$14,$AF,$51,$4D
                      db $09,$9F,$33,$84,$07,$A7,$33,$84
                      db $47,$9F,$3B,$84,$87,$A7,$3B,$84
                      db $C7,$B7,$2B,$60,$54,$BF,$2B,$60
                      db $54,$BB,$1F,$EC,$5B,$C9,$21,$AA
                      db $55,$B9,$2F,$60,$54,$BD,$2F,$60
                      db $54,$C0,$32,$4D,$49,$D9,$21,$89
                      db $49,$D9,$29,$89,$49,$D9,$31,$89
                      db $49,$D9,$39,$89,$49,$D9,$41,$89
                      db $49,$D9,$49,$89,$49,$D9,$51,$89
                      db $49,$D9,$59,$89,$49,$D1,$61,$24
                      db $5A,$48,$8F,$82,$79,$7F,$AE,$CC
                      db $35,$B8,$AF,$C6,$35,$B0,$AF,$C4
                      db $35,$A8,$AE,$C8,$35,$A0,$AF,$C6
                      db $35,$98,$AF,$8C,$35,$9E,$A7,$98
                      db $2B,$FF,$41,$56,$8C,$1F,$46,$46
                      db $AE,$1F,$50,$36,$88,$1F,$5C,$29
                      db $A8,$1F,$AB,$50,$60,$54,$B5,$50
                      db $60,$54,$B0,$45,$64,$5D,$BE,$48
                      db $8A,$59,$AD,$54,$60,$54,$B3,$54
                      db $60,$54,$C6,$58,$AC,$5D,$C6,$68
                      db $AC,$5D,$C6,$7C,$AC,$7D,$C6,$8A
                      db $CE,$77,$B2,$90,$6A,$3D,$A0,$8A
                      db $ED,$3D,$8E,$8F,$6A,$3D,$22,$95
                      db $86,$7D,$32,$95,$84,$7D,$42,$95
                      db $82,$7D,$52,$95,$80,$7D,$22,$A5
                      db $A6,$7D,$32,$A5,$A4,$7D,$42,$A5
                      db $A2,$7D,$59,$A5,$E8,$7D,$52,$A5
                      db $A0,$7D,$22,$B5,$C6,$7D,$32,$B5
                      db $C4,$7D,$42,$B5,$A2,$7D,$52,$B5
                      db $A0,$FD,$22,$C5,$E6,$7D,$32,$C5
                      db $E4,$7D,$42,$C5,$82,$FD,$52,$C5
                      db $80,$FD,$30,$A6,$E8,$3D,$4B,$A3
                      db $C0,$7D,$4B,$B3,$E0,$7D,$FF,$5C
                      db $47,$EA,$13,$6C,$47,$EA,$53,$5C
                      db $57,$EA,$93,$6C,$57,$EA,$D3,$54
                      db $3F,$E8,$13,$49,$34,$E8,$13,$3E
                      db $29,$E8,$13,$7B,$2E,$6C,$13,$8B
                      db $2E,$6C,$53,$7B,$3E,$6C,$93,$8B
                      db $3E,$6C,$D3,$70,$A7,$A8,$3D,$80
                      db $A7,$A3,$2D,$80,$AF,$A3,$AD,$A8
                      db $AC,$A6,$3D,$B8,$AC,$A3,$2D,$B8
                      db $B4,$A3,$AD,$50,$AF,$A6,$3D,$60
                      db $AF,$A3,$2D,$60,$B7,$A3,$AD,$86
                      db $AF,$A6,$3D,$96,$AF,$B3,$2D,$96
                      db $B7,$B3,$AD,$78,$BF,$A6,$3D,$88
                      db $BF,$B3,$2D,$88,$C7,$B3,$AD,$FF
                      db $70,$1E,$44,$1F,$80,$1E,$46,$1F
                      db $60,$2E,$26,$D5,$70,$2E,$64,$1F
                      db $80,$2E,$66,$1F,$70,$3E,$4E,$11
                      db $80,$3E,$4E,$51,$48,$3F,$40,$1F
                      db $58,$3F,$42,$1F,$98,$3F,$42,$5F
                      db $A8,$3F,$40,$5F,$48,$4F,$60,$1F
                      db $58,$4F,$62,$1F,$98,$4F,$62,$5F
                      db $A8,$4F,$60,$5F,$48,$5F,$4E,$11
                      db $58,$5F,$4E,$51,$98,$5F,$4E,$11
                      db $A8,$5F,$4E,$51,$70,$66,$46,$5F
                      db $80,$66,$44,$5F,$70,$76,$66,$5F
                      db $80,$76,$64,$5F,$70,$86,$4E,$11
                      db $80,$86,$4E,$51,$A0,$8E,$26,$15
                      db $FF,$6C,$1A,$48,$10,$7C,$1A,$4A
                      db $10,$8C,$22,$1C,$00,$6C,$2A,$68
                      db $10,$7C,$2A,$6A,$10,$8C,$2A,$2C
                      db $10,$64,$3A,$08,$00,$6C,$3A,$09
                      db $00,$74,$3A,$0A,$00,$7C,$3A,$0B
                      db $00,$84,$3A,$0C,$00,$8C,$3A,$0D
                      db $00,$94,$3A,$0E,$00,$9C,$3A,$0F
                      db $00,$64,$42,$40,$10,$74,$42,$41
                      db $40,$7C,$42,$45,$00,$84,$42,$41
                      db $00,$8C,$42,$41,$40,$94,$42,$46
                      db $10,$74,$4A,$51,$40,$7C,$4A,$62
                      db $00,$84,$4A,$51,$00,$8C,$4A,$51
                      db $40,$64,$52,$60,$10,$74,$52,$62
                      db $00,$7C,$52,$62,$00,$84,$52,$62
                      db $00,$8C,$52,$65,$00,$94,$52,$66
                      db $10,$74,$5A,$72,$00,$7C,$5A,$73
                      db $00,$84,$5A,$74,$00,$8C,$5A,$75
                      db $00,$64,$62,$20,$00,$6C,$62,$21
                      db $00,$74,$62,$22,$10,$84,$62,$24
                      db $10,$94,$62,$26,$00,$9C,$62,$27
                      db $00,$6C,$6A,$31,$00,$94,$6A,$36
                      db $00,$6C,$72,$37,$00,$74,$72,$38
                      db $00,$7C,$72,$39,$00,$84,$72,$3A
                      db $00,$8C,$72,$3B,$00,$94,$72,$2B
                      db $00,$31,$2C,$83,$4C,$39,$2C,$DA
                      db $4C,$41,$2C,$D9,$4C,$31,$34,$86
                      db $4C,$39,$34,$EA,$4C,$41,$34,$E9
                      db $4C,$31,$3C,$8D,$4C,$39,$3C,$8C
                      db $4C,$41,$3C,$87,$4C,$B2,$2C,$B2
                      db $0E,$BA,$2C,$B3,$0E,$C2,$2C,$91
                      db $0E,$B2,$34,$92,$0E,$BA,$34,$93
                      db $0E,$C2,$34,$94,$0E,$BA,$3C,$95
                      db $0E,$C2,$3C,$96,$0E,$1A,$6C,$A8
                      db $15,$12,$7C,$EC,$1B,$22,$7C,$EE
                      db $1B,$15,$5C,$92,$05,$1D,$5C,$92
                      db $45,$15,$64,$84,$15,$CD,$6C,$A0
                      db $19,$DD,$6C,$A0,$59,$CD,$7C,$EC
                      db $1B,$DD,$7C,$EE,$1B,$D5,$5F,$9F
                      db $49,$DD,$5F,$9E,$49,$D5,$67,$CC
                      db $59,$34,$AA,$24,$57,$3C,$9A,$1B
                      db $47,$3C,$A2,$08,$57,$70,$A9,$CC
                      db $02,$78,$A9,$CD,$02,$80,$A9,$CD
                      db $42,$88,$A9,$CC,$42,$70,$B1,$99
                      db $02,$78,$B1,$9A,$02,$80,$B1,$9A
                      db $42,$88,$B1,$99,$42,$70,$B9,$9B
                      db $02,$78,$B9,$9C,$02,$80,$B9,$9C
                      db $42,$88,$B9,$9B,$42,$70,$C1,$9D
                      db $02,$78,$C1,$9E,$02,$80,$C1,$9E
                      db $42,$88,$C1,$9D,$42,$BD,$9E,$0C
                      db $0B,$B4,$A6,$00,$1B,$BD,$AB,$02
                      db $1B,$B5,$B4,$0A,$0B,$FF

DATA_0CB5A2:          db $FC,$02,$1C,$01,$C1,$01,$22,$02
                      db $00,$00,$45,$00,$8B,$02,$95,$03
                      db $9E,$00,$2A,$04,$93,$04,$E7,$00
                      db $FC,$04

CODE_0CB5BC:          LDA.B #$00
                      XBA
                      LDY.B #$20
                      LDA.W CreditsScreenNumber
                      CMP.B #$05
                      BNE +
                      LDY.B #$30
                    + STY.B _0
                      ASL A
                      REP #$10                                  ; XY->16
                      TAY
                      LDX.W DATA_0CB5A2,Y
                      LDY.W #$007F
                      STY.B _1
                      LDY.W #$01FC
CODE_0CB5DB:          PHY
                      LDY.B Layer1XPos
                      LDA.W DATA_0CAF14,X
                      AND.B #$20
                      BEQ +
                      LDY.W NextLayer1XPos
                    + STY.B _3
                      PLY
                      LDA.W DATA_0CAF11,X
                      CMP.B #$FF
                      BEQ CODE_0CB633
                      SEC
                      SBC.B _3
                      STA.W OAMTileXPos,Y
                      LDA.B #$00
                      SBC.B _4
                      AND.B #$01
                      STA.B _4
                      LDA.W DATA_0CAF14,X
                      AND.B #$10
                      LSR A
                      LSR A
                      LSR A
                      ORA.B _4
                      PHY
                      LDY.B _1
                      STA.W OAMTileSize,Y
                      PLY
                      LDA.W DATA_0CAF12,X
                      STA.W OAMTileYPos,Y
                      LDA.W DATA_0CAF13,X
                      STA.W OAMTileNo,Y
                      LDA.W DATA_0CAF14,X
                      AND.B #$CF
                      ORA.B _0
                      STA.W OAMTileAttr,Y
                      DEY
                      DEY
                      DEY
                      DEY
                      INX
                      INX
                      INX
                      INX
                      DEC.B _1
                      BRA CODE_0CB5DB

CODE_0CB633:          SEP #$10                                  ; XY->8
                      RTS


TheEndStripe:         db $50,$EA,$00,$17,$E3,$20,$FC,$20
                      db $C7,$20,$FC,$20,$C4,$20,$FC,$20
                      db $FC,$20,$C4,$20,$FC,$20,$CD,$20
                      db $FC,$20,$C3,$20,$51,$0A,$00,$17
                      db $F3,$20,$FC,$20,$D7,$20,$FC,$20
                      db $D4,$20,$FC,$20,$FC,$20,$D4,$20
                      db $FC,$20,$DD,$20,$FC,$20,$D3,$20
                      db $FF

                      %insert_empty($638,$191,$191,$191,$17F)

LudwigCutBGStripe:    db $30,$00,$5F,$FE,$FA,$28,$30,$64
                      db $40,$2E,$F8,$00,$30,$76,$00,$03
                      db $B6,$11,$B6,$51,$30,$84,$40,$2E
                      db $F8,$00,$30,$84,$00,$03,$B6,$11
                      db $B6,$51,$30,$A4,$40,$2E,$F8,$00
                      db $30,$AF,$00,$03,$B6,$11,$B6,$51
                      db $30,$C4,$40,$2E,$F8,$00,$30,$DB
                      db $00,$01,$F1,$11,$30,$E4,$40,$2E
                      db $F8,$00,$30,$F8,$00,$07,$F1,$11
                      db $F1,$51,$F1,$11,$F2,$11,$31,$04
                      db $40,$2E,$F8,$00,$31,$06,$00,$03
                      db $B6,$11,$B6,$51,$31,$17,$00,$09
                      db $F1,$11,$F2,$11,$F2,$51,$F2,$11
                      db $F9,$08,$31,$24,$40,$2E,$F8,$00
                      db $31,$2A,$00,$03,$F1,$11,$F1,$51
                      db $31,$36,$00,$0B,$F1,$11,$F2,$11
                      db $F9,$08,$F9,$08,$F0,$11,$F0,$51
                      db $31,$44,$40,$2E,$F8,$00,$31,$49
                      db $00,$07,$F1,$11,$F2,$11,$F2,$51
                      db $F1,$51,$31,$52,$00,$03,$B6,$11
                      db $B6,$51,$31,$56,$00,$0B,$F1,$91
                      db $F2,$91,$F9,$08,$F0,$11,$F2,$11
                      db $F2,$51,$31,$64,$00,$2F,$F1,$11
                      db $F1,$51,$F8,$08,$F8,$08,$F1,$11
                      db $F2,$11,$F9,$08,$F2,$D1,$F1,$D1
                      db $AA,$0D,$AB,$0D,$AC,$0D,$AD,$0D
                      db $AE,$0D,$AF,$0D,$F8,$08,$F8,$08
                      db $F8,$08,$F1,$11,$F2,$11,$F9,$08
                      db $F0,$91,$F2,$91,$F9,$08,$31,$84
                      db $00,$2F,$F2,$11,$F2,$51,$F1,$51
                      db $F1,$11,$F2,$11,$F9,$08,$F9,$08
                      db $F2,$51,$F1,$51,$BA,$0D,$BB,$0D
                      db $BC,$0D,$BD,$0D,$DC,$0D,$BF,$0D
                      db $F8,$08,$F8,$08,$F1,$11,$F2,$11
                      db $F0,$11,$F0,$51,$F0,$11,$F2,$11
                      db $F9,$08,$31,$A4,$00,$2F,$86,$1D
                      db $87,$1D,$86,$1D,$87,$1D,$86,$1D
                      db $87,$1D,$86,$1D,$87,$1D,$86,$1D
                      db $CA,$0D,$CB,$0D,$DC,$0D,$DB,$0D
                      db $DC,$0D,$CF,$0D,$86,$1D,$87,$1D
                      db $86,$1D,$87,$1D,$86,$1D,$87,$1D
                      db $86,$1D,$87,$1D,$86,$1D,$31,$C4
                      db $00,$2F,$9E,$1D,$9F,$1D,$9E,$1D
                      db $9F,$1D,$9E,$1D,$9F,$1D,$9E,$1D
                      db $9F,$1D,$9E,$1D,$EA,$0D,$DB,$0D
                      db $DC,$0D,$DB,$0D,$DC,$0D,$DF,$0D
                      db $9E,$1D,$9F,$1D,$9E,$1D,$9F,$1D
                      db $9E,$1D,$9F,$1D,$9E,$1D,$9F,$1D
                      db $9E,$1D,$31,$E4,$40,$2E,$8A,$35
                      db $31,$E4,$00,$01,$B8,$35,$31,$E9
                      db $00,$01,$B8,$35,$31,$F2,$00,$01
                      db $B8,$35,$31,$FB,$00,$01,$B8,$35
                      db $32,$04,$40,$2E,$9A,$35,$32,$07
                      db $00,$01,$B7,$35,$32,$0E,$00,$01
                      db $B7,$35,$32,$15,$00,$01,$B7,$35
                      db $32,$19,$00,$01,$B7,$35,$FF

CutsceneCastleStripe: db $20,$00,$5F,$FE,$F8,$00,$20,$F4
                      db $00,$0B,$80,$01,$81,$01,$82,$01
                      db $83,$01,$81,$41,$85,$01,$21,$14
                      db $00,$0B,$90,$01,$91,$01,$92,$01
                      db $93,$01,$94,$01,$95,$01,$21,$34
                      db $00,$0B,$A0,$01,$A1,$01,$A2,$01
                      db $A3,$01,$A4,$01,$A5,$01,$21,$54
                      db $00,$0B,$B0,$01,$91,$01,$92,$01
                      db $93,$01,$94,$01,$B5,$01,$21,$74
                      db $00,$0B,$A0,$01,$A1,$01,$A2,$01
                      db $A3,$01,$A4,$01,$A5,$01,$21,$94
                      db $00,$0B,$90,$01,$B1,$01,$B2,$01
                      db $B3,$01,$B4,$01,$95,$01,$21,$B4
                      db $00,$0B,$A0,$01,$C6,$01,$C7,$01
                      db $C6,$41,$94,$01,$A5,$01,$21,$D4
                      db $00,$0B,$B0,$01,$D6,$01,$C7,$81
                      db $D6,$41,$B4,$41,$B5,$01,$21,$F4
                      db $00,$0B,$D0,$89,$D0,$C9,$E0,$09
                      db $E1,$09,$D0,$C9,$E2,$09,$FF

LemmyCutBGStripe:     db $30,$00,$5F,$FE,$FA,$28,$30,$64
                      db $40,$2E,$FA,$08,$30,$84,$40,$2E
                      db $FA,$08,$30,$A4,$40,$2E,$FA,$08
                      db $30,$C4,$40,$2E,$FA,$08,$30,$E4
                      db $40,$2E,$FA,$08,$31,$04,$40,$2E
                      db $FA,$08,$31,$24,$40,$2E,$FA,$08
                      db $31,$44,$40,$2E,$FA,$08,$31,$64
                      db $40,$2E,$FA,$08,$31,$84,$40,$2E
                      db $FA,$08,$31,$A4,$40,$2E,$FA,$08
                      db $31,$C4,$40,$2E,$FA,$08,$30,$64
                      db $80,$05,$F8,$08,$F8,$08,$8C,$09
                      db $30,$65,$00,$01,$8C,$09,$30,$68
                      db $00,$01,$8C,$09,$30,$69,$80,$03
                      db $F8,$08,$8C,$09,$30,$6A,$80,$05
                      db $F8,$08,$F8,$08,$8C,$09,$30,$6B
                      db $00,$01,$8C,$09,$30,$6C,$80,$03
                      db $F8,$08,$8C,$09,$30,$6D,$C0,$06
                      db $F8,$08,$30,$ED,$00,$01,$8C,$09
                      db $30,$6E,$80,$05,$F8,$08,$F8,$08
                      db $8C,$09,$30,$6F,$C0,$04,$F8,$08
                      db $30,$CF,$00,$01,$8C,$09,$30,$71
                      db $80,$05,$F8,$08,$F8,$08,$8C,$09
                      db $30,$74,$00,$01,$8C,$09,$30,$75
                      db $80,$03,$F8,$08,$8C,$09,$30,$77
                      db $00,$01,$8C,$09,$30,$7A,$80,$05
                      db $F8,$08,$F8,$08,$8C,$09,$30,$7B
                      db $C0,$04,$F8,$08,$30,$DB,$00,$01
                      db $8C,$09,$31,$84,$80,$05,$8C,$89
                      db $F8,$08,$F8,$08,$31,$45,$00,$01
                      db $8C,$89,$31,$65,$C0,$06,$F8,$08
                      db $31,$C7,$00,$01,$8C,$89,$31,$A9
                      db $80,$03,$8C,$89,$F8,$08,$31,$6B
                      db $00,$01,$8C,$89,$31,$8B,$C0,$04
                      db $F8,$08,$31,$8C,$80,$05,$8C,$89
                      db $F8,$08,$F8,$08,$31,$CD,$00,$01
                      db $8C,$89,$31,$AF,$80,$03,$8C,$89
                      db $F8,$08,$31,$91,$80,$05,$8C,$89
                      db $F8,$08,$F8,$08,$31,$D2,$00,$01
                      db $8C,$89,$31,$B6,$80,$03,$8C,$89
                      db $F8,$08,$31,$D7,$00,$01,$8C,$89
                      db $31,$99,$80,$05,$8C,$89,$F8,$08
                      db $F8,$08,$31,$7B,$00,$01,$8C,$89
                      db $31,$9B,$C0,$04,$F8,$08,$31,$E4
                      db $40,$2E,$8A,$25,$32,$04,$40,$2E
                      db $9A,$25,$FF

WendyCutBGStripe:     db $30,$00,$5F,$FE,$FA,$28,$30,$64
                      db $40,$2E,$F8,$08,$30,$76,$00,$03
                      db $B6,$11,$B6,$51,$30,$84,$40,$2E
                      db $F8,$08,$30,$84,$00,$03,$B6,$11
                      db $B6,$51,$30,$A4,$40,$2E,$F8,$08
                      db $30,$AF,$00,$03,$B6,$11,$B6,$51
                      db $30,$C4,$40,$2E,$F8,$08,$30,$E4
                      db $40,$2E,$F8,$08,$30,$FA,$00,$03
                      db $B6,$11,$B6,$51,$31,$04,$40,$2E
                      db $F8,$08,$31,$06,$00,$03,$B6,$11
                      db $B6,$51,$31,$24,$40,$2E,$F8,$08
                      db $31,$44,$40,$2E,$F8,$08,$31,$4A
                      db $00,$05,$8F,$09,$FE,$08,$8D,$49
                      db $31,$4F,$00,$09,$8F,$09,$8E,$49
                      db $F8,$08,$B6,$11,$B6,$51,$31,$64
                      db $40,$2E,$F8,$08,$31,$69,$00,$0F
                      db $8F,$09,$FF,$08,$FE,$08,$FD,$08
                      db $F8,$08,$8F,$09,$FF,$08,$9B,$C9
                      db $31,$75,$00,$05,$8F,$09,$FE,$08
                      db $9B,$C9,$31,$84,$40,$2E,$F8,$08
                      db $31,$86,$00,$19,$8F,$09,$9C,$C9
                      db $9B,$C9,$FF,$08,$FF,$08,$FE,$08
                      db $9B,$49,$8D,$49,$FF,$08,$9C,$C9
                      db $FE,$08,$FE,$08,$9B,$C9,$31,$95
                      db $00,$05,$FF,$08,$FE,$08,$FD,$08
                      db $31,$9A,$00,$03,$8F,$09,$8E,$49
                      db $31,$A4,$40,$2E,$F8,$08,$31,$A5
                      db $00,$1B,$8F,$09,$9C,$C9,$FE,$08
                      db $FD,$08,$FF,$08,$9C,$C9,$FE,$08
                      db $9B,$C9,$FD,$08,$FF,$08,$9C,$C9
                      db $FE,$08,$9B,$C9,$8D,$C9,$31,$B5
                      db $00,$0D,$FF,$08,$9C,$C9,$FD,$08
                      db $8D,$49,$8F,$09,$9C,$C9,$9B,$C9
                      db $31,$C4,$00,$2F,$8F,$09,$9C,$C9
                      db $FE,$08,$9B,$C9,$FD,$08,$9B,$89
                      db $FE,$08,$9B,$C9,$FD,$08,$9D,$09
                      db $9C,$C9,$FE,$08,$9B,$C9,$FD,$08
                      db $8D,$49,$F8,$08,$8F,$09,$9C,$C9
                      db $FE,$08,$FD,$08,$9D,$09,$9C,$C9
                      db $9B,$C9,$FD,$08,$31,$E4,$40,$2E
                      db $8A,$25,$32,$04,$40,$2E,$9A,$25
                      db $FF

IggyCutBGStripe:      db $30,$00,$5F,$FE,$FA,$28,$30,$64
                      db $40,$2E,$F8,$00,$30,$76,$00,$03
                      db $B6,$11,$B6,$51,$30,$84,$40,$2E
                      db $F8,$00,$30,$84,$00,$03,$B6,$11
                      db $B6,$51,$30,$A4,$40,$2E,$F8,$00
                      db $30,$AF,$00,$03,$B6,$11,$B6,$51
                      db $30,$C4,$40,$2E,$F8,$00,$30,$E4
                      db $40,$2E,$F8,$00,$30,$FA,$00,$03
                      db $B6,$11,$B6,$51,$31,$04,$40,$2E
                      db $F8,$00,$31,$06,$00,$03,$B6,$11
                      db $B6,$51,$31,$0F,$00,$01,$89,$0D
                      db $31,$24,$40,$2E,$F8,$00,$31,$2E
                      db $00,$05,$88,$0D,$FC,$0C,$88,$4D
                      db $31,$44,$40,$2E,$F8,$00,$31,$46
                      db $00,$01,$89,$0D,$31,$4D,$00,$0D
                      db $88,$0D,$FC,$0C,$FC,$0C,$FC,$0C
                      db $88,$4D,$B6,$11,$B6,$51,$31,$64
                      db $40,$2E,$F8,$00,$31,$65,$00,$05
                      db $88,$0D,$FC,$0C,$88,$4D,$31,$6C
                      db $00,$01,$88,$0D,$31,$6D,$40,$08
                      db $FC,$0C,$31,$72,$00,$01,$88,$4D
                      db $31,$7B,$00,$01,$88,$0D,$31,$84
                      db $40,$2E,$F8,$00,$31,$84,$00,$09
                      db $88,$0D,$FC,$0C,$FC,$0C,$FC,$0C
                      db $88,$4D,$31,$8B,$00,$01,$88,$0D
                      db $31,$8C,$40,$0C,$FC,$0C,$31,$93
                      db $00,$01,$88,$4D,$31,$9A,$00,$03
                      db $88,$0D,$FC,$0C,$31,$A4,$00,$21
                      db $96,$0D,$97,$0D,$98,$0D,$99,$0D
                      db $FC,$0C,$88,$4D,$88,$0D,$FC,$0C
                      db $96,$0D,$97,$0D,$98,$0D,$99,$0D
                      db $FC,$0C,$FC,$0C,$FC,$0C,$FC,$0C
                      db $88,$4D,$31,$B5,$40,$06,$F8,$00
                      db $31,$B9,$00,$05,$88,$0D,$96,$0D
                      db $97,$0D,$31,$C4,$00,$2F,$A6,$0D
                      db $A7,$0D,$A8,$0D,$A9,$0D,$FC,$0C
                      db $FC,$0C,$FC,$0C,$FC,$0C,$A6,$0D
                      db $A7,$0D,$A8,$0D,$A9,$0D,$FC,$0C
                      db $FC,$0C,$FC,$0C,$FC,$0C,$FC,$0C
                      db $88,$4D,$F8,$00,$F8,$00,$88,$0D
                      db $FC,$0C,$A6,$0D,$A7,$0D,$31,$E4
                      db $40,$2E,$8A,$35,$31,$E4,$00,$01
                      db $B8,$35,$31,$E9,$00,$01,$B8,$35
                      db $31,$F2,$00,$01,$B8,$35,$31,$FB
                      db $00,$01,$B8,$35,$32,$04,$40,$2E
                      db $9A,$35,$32,$07,$00,$01,$B7,$35
                      db $32,$0E,$00,$01,$B7,$35,$32,$15
                      db $00,$01,$B7,$35,$32,$19,$00,$01
                      db $B7,$35,$FF

                   if ver_is_japanese(!_VER)                    ;\======================== J ====================
C1Message1Stripe:     db $52,$50,$00,$01,$59,$39,$52,$5A        ;!
                      db $00,$01,$5B,$39,$52,$64,$00,$2F        ;!
                      db $47,$39,$5A,$39,$4A,$39,$4B,$39        ;!
                      db $5A,$39,$11,$39,$02,$39,$14,$39        ;!
                      db $5D,$39,$50,$39,$09,$39,$5C,$39        ;!
                      db $10,$39,$5D,$39,$08,$39,$01,$39        ;!
                      db $09,$39,$84,$39,$14,$39,$6C,$39        ;!
                      db $5F,$39,$65,$39,$61,$39,$77,$39        ;!
                      db $FF                                    ;!
																;!
C1Message2Stripe:     db $52,$92,$00,$01,$5B,$39,$52,$97        ;!
                      db $00,$01,$59,$39,$52,$9B,$00,$01        ;!
                      db $59,$39,$52,$A4,$00,$2F,$0D,$39        ;!
                      db $50,$39,$09,$39,$0D,$39,$5D,$39        ;!
                      db $67,$39,$60,$39,$6A,$39,$0D,$39        ;!
                      db $0E,$39,$15,$39,$5D,$39,$5F,$39        ;!
                      db $65,$39,$61,$39,$14,$39,$19,$39        ;!
                      db $54,$39,$02,$39,$10,$39,$5D,$39        ;!
                      db $0D,$39,$19,$39,$07,$39,$FF            ;!
																;!
C1Message3Stripe:     db $52,$D0,$00,$01,$59,$39,$52,$D3        ;!
                      db $00,$01,$59,$39,$52,$E4,$00,$2D        ;!
                      db $12,$39,$08,$39,$57,$39,$0D,$39        ;!
                      db $5D,$39,$52,$39,$04,$39,$19,$39        ;!
                      db $77,$39,$0D,$39,$0A,$39,$06,$39        ;!
                      db $0D,$39,$09,$39,$5D,$39,$64,$39        ;!
                      db $5A,$39,$46,$39,$41,$39,$18,$39        ;!
                      db $01,$39,$1E,$39,$18,$39,$FF            ;!
																;!
C1Message4Stripe:     db $53,$05,$00,$01,$59,$39,$53,$06        ;!
                      db $00,$01,$59,$39,$53,$09,$00,$01        ;!
                      db $59,$39,$53,$24,$00,$17,$0D,$39        ;!
                      db $16,$39,$0D,$39,$0F,$39,$14,$39        ;!
                      db $10,$39,$00,$39,$55,$39,$19,$39        ;!
                      db $09,$39,$0D,$39,$78,$39,$FF            ;!
																;!
C2Message1Stripe:     db $52,$4C,$00,$01,$59,$39,$52,$53        ;!
                      db $00,$01,$5B,$39,$52,$64,$00,$2F        ;!
                      db $67,$39,$60,$39,$6A,$39,$0D,$39        ;!
                      db $0E,$39,$15,$39,$5D,$39,$12,$39        ;!
                      db $15,$39,$21,$39,$1C,$39,$14,$39        ;!
                      db $6C,$39,$5F,$39,$65,$39,$61,$39        ;!
                      db $1D,$39,$5D,$39,$1E,$39,$7C,$39        ;!
                      db $0F,$39,$06,$39,$10,$39,$5D,$39        ;!
                      db $FF                                    ;!
																;!
C2Message2Stripe:     db $52,$84,$00,$01,$59,$39,$52,$96        ;!
                      db $00,$01,$59,$39,$52,$99,$00,$01        ;!
                      db $59,$39,$52,$A4,$00,$2F,$64,$39        ;!
                      db $5A,$39,$46,$39,$41,$39,$18,$39        ;!
                      db $01,$39,$1E,$39,$04,$39,$20,$39        ;!
                      db $5D,$39,$0E,$39,$04,$39,$14,$39        ;!
                      db $0B,$39,$04,$39,$01,$39,$14,$39        ;!
                      db $5D,$39,$61,$39,$40,$39,$63,$39        ;!
                      db $64,$39,$5A,$39,$43,$39,$FF            ;!
																;!
C2Message3Stripe:     db $52,$C9,$00,$01,$59,$39,$52,$D2        ;!
                      db $00,$01,$59,$39,$52,$D7,$00,$01        ;!
                      db $59,$39,$52,$E4,$00,$2F,$18,$39        ;!
                      db $11,$39,$0A,$39,$0A,$39,$21,$39        ;!
                      db $10,$39,$01,$39,$05,$39,$3F,$39        ;!
                      db $07,$39,$14,$39,$08,$39,$51,$39        ;!
                      db $5D,$39,$11,$39,$21,$39,$52,$39        ;!
                      db $A7,$39,$46,$39,$04,$39,$19,$39        ;!
                      db $0E,$39,$04,$39,$19,$39,$FF            ;!
																;!
C2Message4Stripe:     db $53,$0B,$00,$01,$5B,$39,$53,$24        ;!
                      db $00,$2F,$03,$39,$10,$39,$01,$39        ;!
                      db $56,$39,$04,$39,$3D,$39,$5D,$39        ;!
                      db $79,$39,$5A,$39,$7B,$39,$16,$39        ;!
                      db $1C,$39,$14,$39,$02,$39,$21,$39        ;!
                      db $1C,$39,$01,$39,$1E,$39,$5D,$39        ;!
                      db $01,$39,$04,$39,$12,$39,$3F,$39        ;!
                      db $3D,$39,$FF                            ;!
																;!
C3Message1Stripe:     db $52,$4D,$00,$01,$59,$39,$52,$54        ;!
                      db $00,$01,$5B,$39,$52,$64,$00,$2F        ;!
                      db $67,$39,$60,$39,$6A,$39,$0D,$39        ;!
                      db $0E,$39,$15,$39,$5D,$39,$08,$39        ;!
                      db $21,$39,$15,$39,$21,$39,$1C,$39        ;!
                      db $14,$39,$6C,$39,$5F,$39,$65,$39        ;!
                      db $61,$39,$1D,$39,$5D,$39,$1E,$39        ;!
                      db $7C,$39,$0F,$39,$06,$39,$10,$39        ;!
                      db $FF                                    ;!
																;!
C3Message2Stripe:     db $52,$A4,$00,$2F,$54,$39,$7C,$39        ;!
                      db $11,$39,$5D,$39,$16,$39,$11,$39        ;!
                      db $01,$39,$51,$39,$78,$39,$5D,$39        ;!
                      db $09,$39,$04,$39,$09,$39,$6C,$39        ;!
                      db $5A,$39,$4A,$39,$15,$39,$5D,$39        ;!
                      db $06,$39,$58,$39,$09,$39,$05,$39        ;!
                      db $52,$39,$56,$39,$FF                    ;!
																;!
C3Message3Stripe:     db $52,$C9,$00,$01,$59,$39,$52,$E4        ;!
                      db $00,$2F,$1D,$39,$09,$39,$1D,$39        ;!
                      db $5D,$39,$1A,$39,$11,$39,$55,$39        ;!
                      db $1E,$39,$00,$39,$04,$39,$14,$39        ;!
                      db $4A,$39,$48,$39,$65,$39,$7B,$39        ;!
                      db $77,$39,$5D,$39,$11,$39,$7C,$39        ;!
                      db $10,$39,$01,$39,$52,$39,$01,$39        ;!
                      db $11,$39,$FF                            ;!
																;!
C3Message4Stripe:     db $53,$08,$00,$01,$59,$39,$53,$0C        ;!
                      db $00,$01,$59,$39,$53,$12,$00,$01        ;!
                      db $59,$39,$53,$24,$00,$27,$09,$39        ;!
                      db $0D,$39,$20,$39,$5D,$39,$11,$39        ;!
                      db $21,$39,$52,$39,$0D,$39,$16,$39        ;!
                      db $12,$39,$5D,$39,$52,$39,$56,$39        ;!
                      db $14,$39,$10,$39,$00,$39,$5C,$39        ;!
                      db $02,$39,$04,$39,$3F,$39,$FF            ;!
																;!
C4Message1Stripe:     db $52,$46,$00,$01,$59,$39,$52,$4D        ;!
                      db $00,$01,$5B,$39,$52,$64,$00,$2F        ;!
                      db $1F,$39,$21,$39,$15,$39,$21,$39        ;!
                      db $1C,$39,$14,$39,$6C,$39,$5F,$39        ;!
                      db $65,$39,$61,$39,$1D,$39,$5D,$39        ;!
                      db $52,$39,$21,$39,$11,$39,$04,$39        ;!
                      db $5F,$39,$60,$39,$62,$39,$5A,$39        ;!
                      db $5D,$39,$67,$39,$60,$39,$6A,$39        ;!
                      db $FF                                    ;!
																;!
C4Message2Stripe:     db $52,$A4,$00,$2F,$0D,$39,$0E,$39        ;!
                      db $15,$39,$5D,$39,$07,$39,$57,$39        ;!
                      db $04,$39,$20,$39,$5D,$39,$19,$39        ;!
                      db $1F,$39,$01,$39,$14,$39,$1D,$39        ;!
                      db $55,$39,$12,$39,$15,$39,$01,$39        ;!
                      db $7C,$39,$10,$39,$01,$39,$05,$39        ;!
                      db $3F,$39,$3D,$39,$FF                    ;!
																;!
C4Message3Stripe:     db $52,$C5,$00,$01,$59,$39,$52,$CD        ;!
                      db $00,$01,$59,$39,$52,$D1,$00,$01        ;!
                      db $59,$39,$52,$D2,$00,$01,$59,$39        ;!
                      db $52,$E4,$00,$2F,$52,$39,$0C,$39        ;!
                      db $77,$39,$11,$39,$04,$39,$52,$39        ;!
                      db $01,$39,$11,$39,$5D,$39,$10,$39        ;!
                      db $56,$39,$07,$39,$11,$39,$04,$39        ;!
                      db $10,$39,$51,$39,$52,$39,$01,$39        ;!
                      db $11,$39,$01,$39,$02,$39,$5D,$39        ;!
                      db $17,$39,$09,$39,$FF                    ;!
																;!
C4Message4Stripe:     db $53,$04,$00,$01,$59,$39,$53,$15        ;!
                      db $00,$01,$59,$39,$53,$16,$00,$01        ;!
                      db $59,$39,$53,$24,$00,$2F,$51,$39        ;!
                      db $14,$39,$1D,$39,$55,$39,$3F,$39        ;!
                      db $15,$39,$0D,$39,$09,$39,$10,$39        ;!
                      db $1D,$39,$55,$39,$77,$39,$53,$39        ;!
                      db $06,$39,$56,$39,$07,$39,$11,$39        ;!
                      db $04,$39,$10,$39,$51,$39,$56,$39        ;!
                      db $14,$39,$04,$39,$3D,$39,$FF            ;!
																;!
C5Message1Stripe:     db $52,$4B,$00,$01,$59,$39,$52,$4C        ;!
                      db $00,$01,$59,$39,$52,$53,$00,$01        ;!
                      db $5B,$39,$52,$64,$00,$2F,$67,$39        ;!
                      db $60,$39,$6A,$39,$0D,$39,$0E,$39        ;!
                      db $15,$39,$5D,$39,$07,$39,$15,$39        ;!
                      db $21,$39,$1C,$39,$14,$39,$6C,$39        ;!
                      db $5F,$39,$65,$39,$61,$39,$77,$39        ;!
                      db $1E,$39,$7C,$39,$0F,$39,$06,$39        ;!
                      db $5D,$39,$1E,$39,$7C,$39,$FF            ;!
																;!
C5Message2Stripe:     db $52,$8E,$00,$01,$59,$39,$52,$8F        ;!
                      db $00,$01,$59,$39,$52,$94,$00,$01        ;!
                      db $59,$39,$52,$95,$00,$01,$59,$39        ;!
                      db $52,$A4,$00,$2F,$11,$39,$5D,$39        ;!
                      db $1D,$39,$55,$39,$77,$39,$53,$39        ;!
                      db $06,$39,$56,$39,$07,$39,$11,$39        ;!
                      db $04,$39,$10,$39,$51,$39,$0D,$39        ;!
                      db $78,$39,$5D,$39,$0D,$39,$04,$39        ;!
                      db $5D,$39,$07,$39,$14,$39,$08,$39        ;!
                      db $51,$39,$14,$39,$FF                    ;!
																;!
C5Message3Stripe:     db $52,$CE,$00,$01,$59,$39,$52,$D5        ;!
                      db $00,$01,$59,$39,$52,$D7,$00,$01        ;!
                      db $59,$39,$52,$D9,$00,$01,$59,$39        ;!
                      db $52,$E4,$00,$2F,$7B,$39,$44,$39        ;!
                      db $6C,$39,$4E,$39,$5A,$39,$11,$39        ;!
                      db $02,$39,$15,$39,$5D,$39,$52,$39        ;!
                      db $0C,$39,$14,$39,$6C,$39,$5A,$39        ;!
                      db $4A,$39,$11,$39,$5D,$39,$64,$39        ;!
                      db $63,$39,$6C,$39,$4C,$39,$10,$39        ;!
                      db $01,$39,$7C,$39,$FF                    ;!
																;!
C5Message4Stripe:     db $53,$04,$00,$01,$5B,$39,$53,$09        ;!
                      db $00,$01,$59,$39,$53,$13,$00,$01        ;!
                      db $59,$39,$53,$17,$00,$01,$59,$39        ;!
                      db $53,$24,$00,$2F,$15,$39,$01,$39        ;!
                      db $78,$39,$5D,$39,$0F,$39,$51,$39        ;!
                      db $52,$39,$56,$39,$0D,$39,$0D,$39        ;!
                      db $04,$39,$01,$39,$14,$39,$5D,$39        ;!
                      db $15,$39,$09,$39,$19,$39,$55,$39        ;!
                      db $15,$39,$09,$39,$19,$39,$55,$39        ;!
                      db $5A,$39,$3F,$39,$FF                    ;!
																;!
C6Message1Stripe:     db $52,$47,$00,$01,$59,$39,$52,$4E        ;!
                      db $00,$01,$5B,$39,$52,$64,$00,$2B        ;!
                      db $5D,$39,$5C,$39,$05,$39,$15,$39        ;!
                      db $21,$39,$1C,$39,$14,$39,$6C,$39        ;!
                      db $5F,$39,$65,$39,$61,$39,$77,$39        ;!
                      db $0D,$39,$50,$39,$09,$39,$0D,$39        ;!
                      db $67,$39,$60,$39,$6A,$39,$0D,$39        ;!
                      db $0E,$39,$3F,$39,$FF                    ;!
																;!
C6Message2Stripe:     db $52,$8D,$00,$01,$59,$39,$52,$96        ;!
                      db $00,$01,$5B,$39,$52,$A4,$00,$2D        ;!
                      db $5D,$39,$07,$39,$14,$39,$08,$39        ;!
                      db $51,$39,$14,$39,$5D,$39,$0E,$39        ;!
                      db $21,$39,$54,$39,$0F,$39,$0B,$39        ;!
                      db $21,$39,$12,$39,$15,$39,$5D,$39        ;!
                      db $5F,$39,$65,$39,$61,$39,$14,$39        ;!
                      db $0D,$39,$12,$39,$14,$39,$FF            ;!
																;!
C6Message3Stripe:     db $52,$C7,$00,$01,$59,$39,$52,$D2        ;!
                      db $00,$01,$59,$39,$52,$D3,$00,$01        ;!
                      db $59,$39,$52,$E4,$00,$2B,$5D,$39        ;!
                      db $01,$39,$55,$39,$05,$39,$0E,$39        ;!
                      db $77,$39,$16,$39,$20,$39,$05,$39        ;!
                      db $0D,$39,$1C,$39,$14,$39,$5D,$39        ;!
                      db $D0,$39,$4D,$39,$04,$39,$00,$39        ;!
                      db $56,$39,$20,$39,$09,$39,$01,$39        ;!
                      db $78,$39,$FF                            ;!
																;!
C6Message4Stripe:     db $53,$07,$00,$01,$59,$39,$53,$0E        ;!
                      db $00,$01,$59,$39,$53,$10,$00,$01        ;!
                      db $59,$39,$53,$15,$00,$01,$59,$39        ;!
                      db $53,$24,$00,$25,$5D,$39,$01,$39        ;!
                      db $0C,$39,$06,$39,$5D,$39,$67,$39        ;!
                      db $60,$39,$6A,$39,$3F,$39,$5D,$39        ;!
                      db $04,$39,$21,$39,$15,$39,$57,$39        ;!
                      db $5D,$39,$FF,$39,$48,$39,$6B,$39        ;!
                      db $3F,$39,$FF                            ;!
																;!
C7Message1Stripe:     db $52,$4A,$00,$01,$59,$39,$52,$4F        ;!
                      db $00,$01,$5B,$39,$52,$64,$00,$2D        ;!
                      db $0F,$39,$01,$39,$12,$39,$5D,$39        ;!
                      db $08,$39,$01,$39,$07,$39,$14,$39        ;!
                      db $6C,$39,$5F,$39,$65,$39,$61,$39        ;!
                      db $77,$39,$0D,$39,$50,$39,$09,$39        ;!
                      db $0D,$39,$3F,$39,$5D,$39,$14,$39        ;!
                      db $07,$39,$0A,$39,$15,$39,$FF            ;!
																;!
C7Message2Stripe:     db $52,$84,$00,$01,$5B,$39,$52,$89        ;!
                      db $00,$01,$59,$39,$52,$95,$00,$01        ;!
                      db $5B,$39,$52,$96,$00,$01,$59,$39        ;!
                      db $52,$A4,$00,$2D,$79,$39,$5A,$39        ;!
                      db $7B,$39,$16,$39,$1C,$39,$04,$39        ;!
                      db $5D,$39,$11,$39,$20,$39,$58,$39        ;!
                      db $57,$39,$10,$39,$01,$39,$56,$39        ;!
                      db $5D,$39,$5F,$39,$65,$39,$61,$39        ;!
                      db $09,$39,$84,$39,$02,$39,$14,$39        ;!
                      db $1A,$39,$FF                            ;!
																;!
C7Message3Stripe:     db $52,$C4,$00,$01,$59,$39,$52,$C5        ;!
                      db $00,$01,$59,$39,$52,$C7,$00,$01        ;!
                      db $5B,$39,$52,$D0,$00,$01,$59,$39        ;!
                      db $52,$E4,$00,$2D,$17,$39,$09,$39        ;!
                      db $12,$39,$79,$39,$5A,$39,$7B,$39        ;!
                      db $16,$39,$1C,$39,$77,$39,$0D,$39        ;!
                      db $0A,$39,$06,$39,$0D,$39,$09,$39        ;!
                      db $5D,$39,$07,$39,$14,$39,$51,$39        ;!
                      db $84,$39,$02,$39,$55,$39,$EF,$39        ;!
                      db $02,$39,$FF                            ;!
																;!
C7Message4Stripe:     db $53,$06,$00,$01,$59,$39,$53,$0F        ;!
                      db $00,$01,$59,$39,$53,$13,$00,$01        ;!
                      db $59,$39,$53,$14,$00,$01,$59,$39        ;!
                      db $53,$24,$00,$2B,$63,$39,$4C,$39        ;!
                      db $64,$39,$12,$39,$18,$39,$01,$39        ;!
                      db $58,$39,$77,$39,$11,$39,$55,$39        ;!
                      db $1D,$39,$11,$39,$0A,$39,$07,$39        ;!
                      db $11,$39,$04,$39,$10,$39,$51,$39        ;!
                      db $56,$39,$14,$39,$04,$39,$3D,$39        ;!
                      db $FF                                    ;!
                   else                                         ;<================ U, SS, E0, & E1 ==============
C1Message1Stripe:     db $52,$64,$00,$2F,$0C,$39,$40,$39        ;!
                      db $51,$39,$48,$39,$4E,$39,$1F,$39        ;!
                      db $1F,$39,$47,$39,$40,$39,$52,$39        ;!
                      db $1F,$39,$1F,$39,$43,$39,$44,$39        ;!
                      db $45,$39,$44,$39,$40,$39,$53,$39        ;!
                      db $44,$39,$43,$39,$1F,$39,$53,$39        ;!
                      db $47,$39,$44,$39,$FF                    ;!
																;!
C1Message2Stripe:     db $52,$84,$00,$2F,$43,$39,$44,$39        ;!
                      db $4C,$39,$44,$39,$4D,$39,$53,$39        ;!
                      db $44,$39,$43,$39,$1F,$39,$1F,$39        ;!
                      db $08,$39,$46,$39,$46,$39,$58,$39        ;!
                      db $1F,$39,$0A,$39,$4E,$39,$4E,$39        ;!
                      db $4F,$39,$40,$39,$1F,$39,$1F,$39        ;!
                      db $48,$39,$4D,$39,$FF                    ;!
																;!
C1Message3Stripe:     db $52,$A4,$00,$2F,$42,$39,$40,$39        ;!
                      db $52,$39,$53,$39,$4B,$39,$44,$39        ;!
                      db $1F,$39,$1F,$39,$5A,$39,$64,$39        ;!
                      db $1F,$39,$1F,$39,$40,$39,$4D,$39        ;!
                      db $43,$39,$1F,$39,$1F,$39,$51,$39        ;!
                      db $44,$39,$52,$39,$42,$39,$54,$39        ;!
                      db $44,$39,$43,$39,$FF                    ;!
																;!
C1Message4Stripe:     db $52,$C4,$00,$2F,$18,$39,$4E,$39        ;!
                      db $52,$39,$47,$39,$48,$39,$5D,$39        ;!
                      db $52,$39,$1F,$39,$1F,$39,$45,$39        ;!
                      db $51,$39,$48,$39,$44,$39,$4D,$39        ;!
                      db $43,$39,$1F,$39,$1F,$39,$56,$39        ;!
                      db $47,$39,$4E,$39,$1F,$39,$1F,$39        ;!
                      db $48,$39,$52,$39,$FF                    ;!
																;!
C1Message5Stripe:     db $52,$E4,$00,$2F,$52,$39,$53,$39        ;!
                      db $48,$39,$4B,$39,$4B,$39,$1F,$39        ;!
                      db $53,$39,$51,$39,$40,$39,$4F,$39        ;!
                      db $4F,$39,$44,$39,$43,$39,$1F,$39        ;!
                      db $48,$39,$4D,$39,$1F,$39,$40,$39        ;!
                      db $4D,$39,$1F,$39,$44,$39,$46,$39        ;!
                      db $46,$39,$1B,$39,$FF                    ;!
																;!
C1Message6Stripe:     db $53,$04,$00,$2F,$13,$39,$4E,$39        ;!
                      db $46,$39,$44,$39,$53,$39,$47,$39        ;!
                      db $44,$39,$51,$39,$1D,$39,$1F,$39        ;!
                      db $1F,$39,$1F,$39,$1F,$39,$53,$39        ;!
                      db $47,$39,$44,$39,$58,$39,$1F,$39        ;!
                      db $1F,$39,$1F,$39,$1F,$39,$4D,$39        ;!
                      db $4E,$39,$56,$39,$FF                    ;!
																;!
C1Message7Stripe:     db $53,$24,$00,$29,$53,$39,$51,$39        ;!
                      db $40,$39,$55,$39,$44,$39,$4B,$39        ;!
                      db $1F,$39,$53,$39,$4E,$39,$1F,$39        ;!
                      db $03,$39,$4E,$39,$4D,$39,$54,$39        ;!
                      db $53,$39,$1F,$39,$0B,$39,$40,$39        ;!
                      db $4D,$39,$43,$39,$1B,$39                ;!
																;!
BlankStripe:          db $FF                                    ;!
																;!
C2Message1Stripe:     db $52,$64,$00,$2F,$0C,$39,$4E,$39        ;!
                      db $51,$39,$53,$39,$4E,$39,$4D,$39        ;!
                      db $1F,$39,$1F,$39,$1F,$39,$0A,$39        ;!
                      db $4E,$39,$4E,$39,$4F,$39,$40,$39        ;!
                      db $1F,$39,$1F,$39,$1F,$39,$09,$39        ;!
                      db $51,$39,$1B,$39,$1F,$39,$1F,$39        ;!
                      db $4E,$39,$45,$39,$FF                    ;!
																;!
C2Message2Stripe:     db $52,$84,$00,$2F,$42,$39,$40,$39        ;!
                      db $52,$39,$53,$39,$4B,$39,$44,$39        ;!
                      db $1F,$39,$1F,$39,$5A,$39,$65,$39        ;!
                      db $1F,$39,$48,$39,$52,$39,$1F,$39        ;!
                      db $4D,$39,$4E,$39,$56,$39,$1F,$39        ;!
                      db $49,$39,$54,$39,$52,$39,$53,$39        ;!
                      db $1F,$39,$40,$39,$FF                    ;!
																;!
C2Message3Stripe:     db $52,$A4,$00,$2F,$4C,$39,$44,$39        ;!
                      db $4C,$39,$4E,$39,$51,$39,$58,$39        ;!
                      db $1B,$39,$1F,$39,$13,$39,$47,$39        ;!
                      db $44,$39,$1F,$39,$4D,$39,$44,$39        ;!
                      db $57,$39,$53,$39,$1F,$39,$40,$39        ;!
                      db $51,$39,$44,$39,$40,$39,$1F,$39        ;!
                      db $48,$39,$52,$39,$FF                    ;!
																;!
C2Message4Stripe:     db $52,$C4,$00,$2F,$53,$39,$47,$39        ;!
                      db $44,$39,$1F,$39,$1F,$39,$54,$39        ;!
                      db $4D,$39,$43,$39,$44,$39,$51,$39        ;!
                      db $46,$39,$51,$39,$4E,$39,$54,$39        ;!
                      db $4D,$39,$43,$39,$1F,$39,$15,$39        ;!
                      db $40,$39,$4D,$39,$48,$39,$4B,$39        ;!
                      db $4B,$39,$40,$39,$FF                    ;!
																;!
C2Message5Stripe:     db $52,$E4,$00,$2F,$03,$39,$4E,$39        ;!
                      db $4C,$39,$44,$39,$1B,$39,$1F,$39        ;!
                      db $1F,$39,$1F,$39,$16,$39,$47,$39        ;!
                      db $40,$39,$53,$39,$1F,$39,$53,$39        ;!
                      db $51,$39,$40,$39,$4F,$39,$52,$39        ;!
                      db $1F,$39,$40,$39,$56,$39,$40,$39        ;!
                      db $48,$39,$53,$39,$FF                    ;!
																;!
C2Message6Stripe:     db $53,$04,$00,$2F,$0C,$39,$40,$39        ;!
                      db $51,$39,$48,$39,$4E,$39,$1F,$39        ;!
                      db $48,$39,$4D,$39,$1F,$39,$53,$39        ;!
                      db $47,$39,$48,$39,$52,$39,$1F,$39        ;!
                      db $4D,$39,$44,$39,$56,$39,$1F,$39        ;!
                      db $56,$39,$4E,$39,$51,$39,$4B,$39        ;!
                      db $43,$39,$1E,$39,$FF                    ;!
																;!
C2Message7Stripe:     db $53,$24,$00,$2F,$16,$39,$47,$39        ;!
                      db $40,$39,$53,$39,$1F,$39,$1F,$39        ;!
                      db $1F,$39,$56,$39,$48,$39,$4B,$39        ;!
                      db $4B,$39,$1F,$39,$1F,$39,$1F,$39        ;!
                      db $41,$39,$44,$39,$42,$39,$4E,$39        ;!
                      db $4C,$39,$44,$39,$1F,$39,$1F,$39        ;!
                      db $4E,$39,$45,$39,$FF                    ;!
																;!
C2Message8Stripe:     db $53,$44,$00,$25,$0F,$39,$51,$39        ;!
                      db $48,$39,$4D,$39,$42,$39,$44,$39        ;!
                      db $52,$39,$52,$39,$1F,$39,$13,$39        ;!
                      db $4E,$39,$40,$39,$43,$39,$52,$39        ;!
                      db $53,$39,$4E,$39,$4E,$39,$4B,$39        ;!
                      db $1E,$39,$FF                            ;!
																;!
C3Message1Stripe:     db $52,$64,$00,$2F,$0C,$39,$40,$39        ;!
                      db $51,$39,$48,$39,$4E,$39,$1F,$39        ;!
                      db $47,$39,$40,$39,$52,$39,$1F,$39        ;!
                      db $53,$39,$51,$39,$48,$39,$54,$39        ;!
                      db $4C,$39,$4F,$39,$47,$39,$44,$39        ;!
                      db $43,$39,$1F,$39,$4E,$39,$55,$39        ;!
                      db $44,$39,$51,$39,$FF                    ;!
																;!
C3Message2Stripe:     db $52,$84,$00,$2F,$0B,$39,$44,$39        ;!
                      db $4C,$39,$4C,$39,$58,$39,$1F,$39        ;!
                      db $1F,$39,$0A,$39,$4E,$39,$4E,$39        ;!
                      db $4F,$39,$40,$39,$1F,$39,$1F,$39        ;!
                      db $4E,$39,$45,$39,$1F,$39,$1F,$39        ;!
                      db $42,$39,$40,$39,$52,$39,$53,$39        ;!
                      db $4B,$39,$44,$39,$FF                    ;!
																;!
C3Message3Stripe:     db $52,$A4,$00,$2F,$5A,$39,$66,$39        ;!
                      db $1B,$39,$1F,$39,$1F,$39,$0C,$39        ;!
                      db $40,$39,$51,$39,$48,$39,$4E,$39        ;!
                      db $5D,$39,$52,$39,$1F,$39,$1F,$39        ;!
                      db $50,$39,$54,$39,$44,$39,$52,$39        ;!
                      db $53,$39,$1F,$39,$1F,$39,$1F,$39        ;!
                      db $48,$39,$52,$39,$FF                    ;!
																;!
C3Message4Stripe:     db $52,$C4,$00,$2F,$52,$39,$53,$39        ;!
                      db $40,$39,$51,$39,$53,$39,$48,$39        ;!
                      db $4D,$39,$46,$39,$1F,$39,$1F,$39        ;!
                      db $53,$39,$4E,$39,$1F,$39,$1F,$39        ;!
                      db $46,$39,$44,$39,$53,$39,$1F,$39        ;!
                      db $1F,$39,$1F,$39,$4C,$39,$54,$39        ;!
                      db $42,$39,$47,$39,$FF                    ;!
																;!
C3Message5Stripe:     db $52,$E4,$00,$2F,$4C,$39,$4E,$39        ;!
                      db $51,$39,$44,$39,$1F,$39,$43,$39        ;!
                      db $48,$39,$45,$39,$45,$39,$48,$39        ;!
                      db $42,$39,$54,$39,$4B,$39,$53,$39        ;!
                      db $1B,$39,$1F,$39,$1F,$39,$1F,$39        ;!
                      db $1F,$39,$1F,$39,$07,$39,$40,$39        ;!
                      db $55,$39,$44,$39,$FF                    ;!
																;!
C3Message6Stripe:     db $53,$04,$00,$2F,$58,$39,$4E,$39        ;!
                      db $54,$39,$1F,$39,$1F,$39,$45,$39        ;!
                      db $4E,$39,$54,$39,$4D,$39,$43,$39        ;!
                      db $1F,$39,$1F,$39,$53,$39,$47,$39        ;!
                      db $44,$39,$1F,$39,$11,$39,$44,$39        ;!
                      db $43,$39,$1F,$39,$1F,$39,$40,$39        ;!
                      db $4D,$39,$43,$39,$FF                    ;!
																;!
C3Message7Stripe:     db $53,$24,$00,$25,$06,$39,$51,$39        ;!
                      db $44,$39,$44,$39,$4D,$39,$1F,$39        ;!
                      db $12,$39,$56,$39,$48,$39,$53,$39        ;!
                      db $42,$39,$47,$39,$44,$39,$52,$39        ;!
                      db $1F,$39,$58,$39,$44,$39,$53,$39        ;!
                      db $1E,$39,$FF                            ;!
																;!
C4Message1Stripe:     db $52,$64,$00,$2F,$0B,$39,$54,$39        ;!
                      db $43,$39,$56,$39,$48,$39,$46,$39        ;!
                      db $1F,$39,$55,$39,$4E,$39,$4D,$39        ;!
                      db $1F,$39,$1F,$39,$0A,$39,$4E,$39        ;!
                      db $4E,$39,$4F,$39,$40,$39,$5D,$39        ;!
                      db $52,$39,$1F,$39,$43,$39,$40,$39        ;!
                      db $58,$39,$52,$39,$FF                    ;!
																;!
C4Message2Stripe:     db $52,$84,$00,$2F,$4E,$39,$45,$39        ;!
                      db $1F,$39,$1F,$39,$1F,$39,$1F,$39        ;!
                      db $42,$39,$4E,$39,$4C,$39,$4F,$39        ;!
                      db $4E,$39,$52,$39,$48,$39,$4D,$39        ;!
                      db $46,$39,$1F,$39,$1F,$39,$1F,$39        ;!
                      db $1F,$39,$0A,$39,$4E,$39,$4E,$39        ;!
                      db $4F,$39,$40,$39,$FF                    ;!
																;!
C4Message3Stripe:     db $52,$A4,$00,$2F,$52,$39,$58,$39        ;!
                      db $4C,$39,$4F,$39,$47,$39,$4E,$39        ;!
                      db $4D,$39,$48,$39,$44,$39,$52,$39        ;!
                      db $1F,$39,$48,$39,$4D,$39,$1F,$39        ;!
                      db $42,$39,$40,$39,$52,$39,$53,$39        ;!
                      db $4B,$39,$44,$39,$1F,$39,$1F,$39        ;!
                      db $5A,$39,$67,$39,$FF                    ;!
																;!
C4Message4Stripe:     db $52,$C4,$00,$2F,$40,$39,$51,$39        ;!
                      db $44,$39,$1F,$39,$4E,$39,$55,$39        ;!
                      db $44,$39,$51,$39,$1B,$39,$1F,$39        ;!
                      db $1F,$39,$13,$39,$47,$39,$44,$39        ;!
                      db $1F,$39,$05,$39,$4E,$39,$51,$39        ;!
                      db $44,$39,$52,$39,$53,$39,$1F,$39        ;!
                      db $4E,$39,$45,$39,$FF                    ;!
																;!
C4Message5Stripe:     db $52,$E4,$00,$2F,$08,$39,$4B,$39        ;!
                      db $4B,$39,$54,$39,$52,$39,$48,$39        ;!
                      db $4E,$39,$4D,$39,$1F,$39,$1F,$39        ;!
                      db $1F,$39,$4B,$39,$48,$39,$44,$39        ;!
                      db $52,$39,$1F,$39,$1F,$39,$1F,$39        ;!
                      db $40,$39,$47,$39,$44,$39,$40,$39        ;!
                      db $43,$39,$1B,$39,$FF                    ;!
																;!
C4Message6Stripe:     db $53,$04,$00,$2F,$0C,$39,$40,$39        ;!
                      db $51,$39,$48,$39,$4E,$39,$1F,$39        ;!
                      db $4C,$39,$54,$39,$52,$39,$53,$39        ;!
                      db $1F,$39,$54,$39,$52,$39,$44,$39        ;!
                      db $1F,$39,$47,$39,$48,$39,$52,$39        ;!
                      db $1F,$39,$41,$39,$51,$39,$40,$39        ;!
                      db $48,$39,$4D,$39,$FF                    ;!
																;!
C4Message7Stripe:     db $53,$24,$00,$2F,$53,$39,$4E,$39        ;!
                      db $1F,$39,$52,$39,$4E,$39,$4B,$39        ;!
                      db $55,$39,$44,$39,$1F,$39,$1F,$39        ;!
                      db $53,$39,$47,$39,$44,$39,$1F,$39        ;!
                      db $4F,$39,$54,$39,$59,$39,$59,$39        ;!
                      db $4B,$39,$44,$39,$1F,$39,$1F,$39        ;!
                      db $4E,$39,$45,$39,$FF                    ;!
																;!
C4Message8Stripe:     db $53,$44,$00,$2D,$53,$39,$47,$39        ;!
                      db $48,$39,$52,$39,$1F,$39,$4F,$39        ;!
                      db $44,$39,$51,$39,$4F,$39,$4B,$39        ;!
                      db $44,$39,$57,$39,$48,$39,$4D,$39        ;!
                      db $46,$39,$1F,$39,$45,$39,$4E,$39        ;!
                      db $51,$39,$44,$39,$52,$39,$53,$39        ;!
                      db $1B,$39,$FF                            ;!
																;!
C5Message1Stripe:     db $52,$64,$00,$2F,$0C,$39,$40,$39        ;!
                      db $51,$39,$48,$39,$4E,$39,$1F,$39        ;!
                      db $1F,$39,$1F,$39,$45,$39,$4E,$39        ;!
                      db $54,$39,$4D,$39,$43,$39,$1F,$39        ;!
                      db $1F,$39,$1F,$39,$47,$39,$48,$39        ;!
                      db $52,$39,$1F,$39,$1F,$39,$56,$39        ;!
                      db $40,$39,$58,$39,$FF                    ;!
																;!
C5Message2Stripe:     db $52,$84,$00,$2F,$53,$39,$47,$39        ;!
                      db $51,$39,$4E,$39,$54,$39,$46,$39        ;!
                      db $47,$39,$1F,$39,$1F,$39,$53,$39        ;!
                      db $47,$39,$44,$39,$1F,$39,$1F,$39        ;!
                      db $05,$39,$4E,$39,$51,$39,$44,$39        ;!
                      db $52,$39,$53,$39,$1F,$39,$1F,$39        ;!
                      db $4E,$39,$45,$39,$FF                    ;!
																;!
C5Message3Stripe:     db $52,$A4,$00,$2F,$08,$39,$4B,$39        ;!
                      db $4B,$39,$54,$39,$52,$39,$48,$39        ;!
                      db $4E,$39,$4D,$39,$1F,$39,$40,$39        ;!
                      db $4D,$39,$43,$39,$1F,$39,$1F,$39        ;!
                      db $47,$39,$40,$39,$52,$39,$1F,$39        ;!
                      db $4F,$39,$54,$39,$53,$39,$1F,$39        ;!
                      db $40,$39,$4D,$39,$FF                    ;!
																;!
C5Message4Stripe:     db $52,$C4,$00,$2F,$44,$39,$4D,$39        ;!
                      db $43,$39,$1F,$39,$1F,$39,$53,$39        ;!
                      db $4E,$39,$1F,$39,$1F,$39,$1F,$39        ;!
                      db $11,$39,$4E,$39,$58,$39,$1F,$39        ;!
                      db $1F,$39,$0A,$39,$4E,$39,$4E,$39        ;!
                      db $4F,$39,$40,$39,$1F,$39,$1F,$39        ;!
                      db $4E,$39,$45,$39,$FF                    ;!
																;!
C5Message5Stripe:     db $52,$E4,$00,$2F,$42,$39,$40,$39        ;!
                      db $52,$39,$53,$39,$4B,$39,$44,$39        ;!
                      db $1F,$39,$1F,$39,$5A,$39,$68,$39        ;!
                      db $1B,$39,$1F,$39,$1F,$39,$1F,$39        ;!
                      db $0E,$39,$4D,$39,$56,$39,$40,$39        ;!
                      db $51,$39,$43,$39,$1F,$39,$1F,$39        ;!
                      db $53,$39,$4E,$39,$FF                    ;!
																;!
C5Message6Stripe:     db $53,$04,$00,$2F,$53,$39,$47,$39        ;!
                      db $44,$39,$1F,$39,$1F,$39,$1F,$39        ;!
                      db $1F,$39,$43,$39,$40,$39,$4D,$39        ;!
                      db $46,$39,$44,$39,$51,$39,$4E,$39        ;!
                      db $54,$39,$52,$39,$1F,$39,$1F,$39        ;!
                      db $1F,$39,$1F,$39,$5B,$39,$41,$39        ;!
                      db $54,$39,$53,$39,$FF                    ;!
																;!
C5Message7Stripe:     db $53,$24,$00,$2F,$53,$39,$40,$39        ;!
                      db $52,$39,$53,$39,$58,$39,$5C,$39        ;!
                      db $1F,$39,$02,$39,$47,$39,$4E,$39        ;!
                      db $42,$39,$4E,$39,$4B,$39,$40,$39        ;!
                      db $53,$39,$44,$39,$1F,$39,$08,$39        ;!
                      db $52,$39,$4B,$39,$40,$39,$4D,$39        ;!
                      db $43,$39,$1A,$39,$FF                    ;!
																;!
C6Message1Stripe:     db $52,$64,$00,$2F,$16,$39,$44,$39        ;!
                      db $4D,$39,$43,$39,$58,$39,$1F,$39        ;!
                      db $0E,$39,$1B,$39,$1F,$39,$0A,$39        ;!
                      db $4E,$39,$4E,$39,$4F,$39,$40,$39        ;!
                      db $1F,$39,$48,$39,$4D,$39,$1F,$39        ;!
                      db $42,$39,$40,$39,$52,$39,$53,$39        ;!
                      db $4B,$39,$44,$39,$FF                    ;!
																;!
C6Message2Stripe:     db $52,$84,$00,$2F,$5A,$39,$69,$39        ;!
                      db $1F,$39,$1F,$39,$47,$39,$40,$39        ;!
                      db $52,$39,$1F,$39,$1F,$39,$52,$39        ;!
                      db $54,$39,$4D,$39,$46,$39,$1F,$39        ;!
                      db $1F,$39,$47,$39,$44,$39,$51,$39        ;!
                      db $1F,$39,$1F,$39,$4B,$39,$40,$39        ;!
                      db $52,$39,$53,$39,$FF                    ;!
																;!
C6Message3Stripe:     db $52,$A4,$00,$2F,$52,$39,$4E,$39        ;!
                      db $4D,$39,$46,$39,$1B,$39,$1F,$39        ;!
                      db $1F,$39,$0C,$39,$40,$39,$51,$39        ;!
                      db $48,$39,$4E,$39,$1F,$39,$1F,$39        ;!
                      db $4C,$39,$54,$39,$52,$39,$53,$39        ;!
                      db $1F,$39,$1F,$39,$4C,$39,$44,$39        ;!
                      db $44,$39,$53,$39,$FF                    ;!
																;!
C6Message4Stripe:     db $52,$C4,$00,$2F,$53,$39,$47,$39        ;!
                      db $44,$39,$1F,$39,$1F,$39,$42,$39        ;!
                      db $47,$39,$40,$39,$4B,$39,$4B,$39        ;!
                      db $44,$39,$4D,$39,$46,$39,$44,$39        ;!
                      db $1F,$39,$1F,$39,$53,$39,$47,$39        ;!
                      db $40,$39,$53,$39,$1F,$39,$1F,$39        ;!
                      db $48,$39,$52,$39,$FF                    ;!
																;!
C6Message5Stripe:     db $52,$E4,$00,$2F,$4D,$39,$4E,$39        ;!
                      db $56,$39,$1F,$39,$41,$39,$44,$39        ;!
                      db $45,$39,$4E,$39,$51,$39,$44,$39        ;!
                      db $1F,$39,$47,$39,$48,$39,$4C,$39        ;!
                      db $1B,$39,$1F,$39,$1F,$39,$1F,$39        ;!
                      db $1F,$39,$13,$39,$47,$39,$44,$39        ;!
                      db $51,$39,$44,$39,$FF                    ;!
																;!
C6Message6Stripe:     db $53,$04,$00,$2F,$48,$39,$52,$39        ;!
                      db $1F,$39,$40,$39,$1F,$39,$1F,$39        ;!
                      db $52,$39,$54,$39,$4D,$39,$4A,$39        ;!
                      db $44,$39,$4D,$39,$1F,$39,$1F,$39        ;!
                      db $52,$39,$47,$39,$48,$39,$4F,$39        ;!
                      db $1F,$39,$1F,$39,$53,$39,$47,$39        ;!
                      db $40,$39,$53,$39,$FF                    ;!
																;!
C6Message7Stripe:     db $53,$24,$00,$2F,$40,$39,$4F,$39        ;!
                      db $4F,$39,$44,$39,$40,$39,$51,$39        ;!
                      db $52,$39,$1F,$39,$53,$39,$4E,$39        ;!
                      db $1F,$39,$41,$39,$44,$39,$1F,$39        ;!
                      db $1F,$39,$40,$39,$1F,$39,$46,$39        ;!
                      db $40,$39,$53,$39,$44,$39,$56,$39        ;!
                      db $40,$39,$58,$39,$FF                    ;!
																;!
C6Message8Stripe:     db $53,$44,$00,$2F,$53,$39,$4E,$39        ;!
                      db $1F,$39,$53,$39,$47,$39,$44,$39        ;!
                      db $1F,$39,$15,$39,$40,$39,$4B,$39        ;!
                      db $4B,$39,$44,$39,$58,$39,$1F,$39        ;!
                      db $4E,$39,$45,$39,$1F,$39,$01,$39        ;!
                      db $4E,$39,$56,$39,$52,$39,$44,$39        ;!
                      db $51,$39,$1B,$39,$FF                    ;!
																;!
C7Message1Stripe:     db $52,$64,$00,$2F,$0C,$39,$40,$39        ;!
                      db $51,$39,$48,$39,$4E,$39,$1F,$39        ;!
                      db $47,$39,$40,$39,$52,$39,$1F,$39        ;!
                      db $43,$39,$44,$39,$45,$39,$44,$39        ;!
                      db $40,$39,$53,$39,$44,$39,$43,$39        ;!
                      db $1F,$39,$0B,$39,$40,$39,$51,$39        ;!
                      db $51,$39,$58,$39,$FF                    ;!
																;!
C7Message2Stripe:     db $52,$84,$00,$2F,$0A,$39,$4E,$39        ;!
                      db $4E,$39,$4F,$39,$40,$39,$1F,$39        ;!
                      db $48,$39,$4D,$39,$1F,$39,$42,$39        ;!
                      db $40,$39,$52,$39,$53,$39,$4B,$39        ;!
                      db $44,$39,$1F,$39,$5A,$39,$6A,$39        ;!
                      db $1B,$39,$1F,$39,$1F,$39,$00,$39        ;!
                      db $4B,$39,$4B,$39,$FF                    ;!
																;!
C7Message3Stripe:     db $52,$A4,$00,$2F,$53,$39,$47,$39        ;!
                      db $40,$39,$53,$39,$1F,$39,$48,$39        ;!
                      db $52,$39,$1F,$39,$4B,$39,$44,$39        ;!
                      db $45,$39,$53,$39,$1F,$39,$48,$39        ;!
                      db $52,$39,$1F,$39,$01,$39,$4E,$39        ;!
                      db $56,$39,$52,$39,$44,$39,$51,$39        ;!
                      db $5D,$39,$52,$39,$FF                    ;!
																;!
C7Message4Stripe:     db $52,$C4,$00,$2F,$02,$39,$40,$39        ;!
                      db $52,$39,$53,$39,$4B,$39,$44,$39        ;!
                      db $1F,$39,$1F,$39,$56,$39,$47,$39        ;!
                      db $44,$39,$51,$39,$44,$39,$1F,$39        ;!
                      db $1F,$39,$1F,$39,$0F,$39,$51,$39        ;!
                      db $48,$39,$4D,$39,$42,$39,$44,$39        ;!
                      db $52,$39,$52,$39,$FF                    ;!
																;!
C7Message5Stripe:     db $52,$E4,$00,$2F,$13,$39,$4E,$39        ;!
                      db $40,$39,$43,$39,$52,$39,$53,$39        ;!
                      db $4E,$39,$4E,$39,$4B,$39,$1F,$39        ;!
                      db $48,$39,$52,$39,$1F,$39,$41,$39        ;!
                      db $44,$39,$48,$39,$4D,$39,$46,$39        ;!
                      db $1F,$39,$47,$39,$44,$39,$4B,$39        ;!
                      db $43,$39,$1B,$39,$FF                    ;!
																;!
C7Message6Stripe:     db $53,$04,$00,$2F,$02,$39,$40,$39        ;!
                      db $4D,$39,$1F,$39,$0C,$39,$40,$39        ;!
                      db $51,$39,$48,$39,$4E,$39,$1F,$39        ;!
                      db $51,$39,$44,$39,$52,$39,$42,$39        ;!
                      db $54,$39,$44,$39,$1F,$39,$47,$39        ;!
                      db $44,$39,$51,$39,$1F,$39,$40,$39        ;!
                      db $4D,$39,$43,$39,$FF                    ;!
																;!
C7Message7Stripe:     db $53,$24,$00,$2F,$51,$39,$44,$39        ;!
                      db $52,$39,$53,$39,$4E,$39,$51,$39        ;!
                      db $44,$39,$1F,$39,$1F,$39,$1F,$39        ;!
                      db $1F,$39,$1F,$39,$4F,$39,$44,$39        ;!
                      db $40,$39,$42,$39,$44,$39,$1F,$39        ;!
                      db $1F,$39,$1F,$39,$1F,$39,$1F,$39        ;!
                      db $53,$39,$4E,$39,$FF                    ;!
																;!
C7Message8Stripe:     db $53,$44,$00,$1B,$03,$39,$48,$39        ;!
                      db $4D,$39,$4E,$39,$52,$39,$40,$39        ;!
                      db $54,$39,$51,$39,$1F,$39,$0B,$39        ;!
                      db $40,$39,$4D,$39,$43,$39,$1E,$39        ;!
                      db $FF                                    ;!
                   endif                                        ;/ ENDIF

CODE_0CC94E:          LDA.W CutsceneTextTimer
                   if ver_is_arcade(!_VER)                      ;\======================= SS ====================
                      BNE SSCODE_0CC95B                         ;!
                      LDA.W EnterLevelAuto                      ;!
                      BNE +                                     ;!
                      LDA.B #$80                                ;!
                      STA.W EnterLevelAuto                      ;!
                    + BRA CODE_0CC97D                           ;!
													            ;!
SSCODE_0CC95B:        LDA.B byetudlrHold                        ;!
                      ORA.B axlr0000Hold                        ;!
                      BEQ CODE_0CC953                           ;!
                      LDA.W CutsceneTextTimer                   ;!
                      AND.B #$E0                                ;!
                      BNE +                                     ;!
                      LDA.B #$01                                ;!
                      STA.W CutsceneTextTimer                   ;!
                      BRA CODE_0CC953                           ;!
													            ;!
                    + INC A                                     ;!
                      STA.W CutsceneTextTimer                   ;!
                   else                                         ;<================ J, U, E0, & E1 ===============
                      BEQ CODE_0CC97D                           ;!
                   endif                                        ;/===============================================
CODE_0CC953:          DEC.W CutsceneTextTimer
                      LDA.W CutsceneTextTimer
                      AND.B #$1F
                      BNE CODE_0CC97D
                      LDA.W CutsceneID
                      DEC A
                      ASL A
                      ASL A
                   if ver_is_english(!_VER)                     ;\================ U, SS, E0, & E1 ==============
                      ASL A                                     ;!
                   endif                                        ;/===============================================
                      STA.B _0
                      LDA.W CutsceneTextTimer
                      AND.B #con($60,$E0,$E0,$E0,$E0)
                      LSR A
                      LSR A
                      LSR A
                      LSR A
                      LSR A
                      ORA.B _0
                      STA.B _0
                      ASL A
                      CLC
                      ADC.B _0
                      CLC
                      ADC.B #$21
                      STA.B StripeImage
CODE_0CC97D:          RTS

CODE_0CC97E:          PHB
                      PHK
                      PLB
                      JSR CODE_0CC94E
                      JSR CODE_0CCA8F
                      JSR CODE_0CC98C
                      PLB
                      RTL

CODE_0CC98C:          LDA.B TrueFrame
                      AND.B #$03
                      BNE +
                      LDA.W Layer2ScrollYSpeed+1
                      BEQ +
                      DEC.W Layer2ScrollYSpeed+1
                    + JSR CODE_0CD803
                      LDA.W CutsceneID
                      DEC A
                      JSL ExecutePtr

                      dw CODE_0CC9B3
                      dw CODE_0CC9CC
                      dw CODE_0CCA2F
                      dw CODE_0CCA04
                      dw CODE_0CC9E0
                      dw CODE_0CCA51
                      dw CODE_0CCA72

CODE_0CC9B3:          JSR CODE_0CCB5B
                      JSR CODE_0CCACE
                      LDA.W Layer1ScrollType
                      JSL ExecutePtr

                      dw CODE_0CCB1C
                      dw CODE_0CD295
                      dw CODE_0CCC51
                      dw CODE_0CD2B2
                      dw CODE_0CCD23
                      dw CODE_0CCFDE

CODE_0CC9CC:          JSR CODE_0CCACE
                      LDA.W Layer1ScrollType
                      JSL ExecutePtr

                      dw CODE_0CD003
                      dw CODE_0CD2B2
                      dw CODE_0CCD23
                      dw CODE_0CD557
                      dw CODE_0CCFDE

CODE_0CC9E0:          JSR CODE_0CCB5B
                      JSR CODE_0CCACE
                      JSR CODE_0CD4F8
                      LDA.W Layer1ScrollType
                      JSL ExecutePtr

                      dw CODE_0CCB1C
                      dw CODE_0CD2D0
                      dw CODE_0CCF72
                      dw CODE_0CCFC5
                      dw CODE_0CD295
                      dw CODE_0CCC51
                      dw CODE_0CD2B2
                      dw CODE_0CCD23
                      dw CODE_0CCFF7
                      dw CODE_0CCFDE

CODE_0CCA04:          JSR CODE_0CCB5B
                      LDA.W Layer1ScrollType
                      BNE +
                      LDY.B #$04
                      STY.W Layer2ScrollTimer
                    + CMP.B #$07
                      BNE +
                      JSR CODE_0CCF49
                    + LDA.W Layer1ScrollType
                      JSL ExecutePtr

                      dw CODE_0CCB1C
                      dw CODE_0CD295
                      dw CODE_0CCC51
                      dw CODE_0CD2BD
                      dw CODE_0CCDA1
                      dw CODE_0CD2E6
                      dw CODE_0CCEE2
                      dw CODE_0CCFDE

CODE_0CCA2F:          JSR CODE_0CD65B
                      JSR CODE_0CCACE
                      LDA.W Layer1ScrollType
                      CMP.B #$03
                      BEQ +
                      JSR CODE_0CD3A6
                    + JSR CODE_0CD3F4
                      LDA.W Layer1ScrollType
                      JSL ExecutePtr

                      dw CODE_0CD0BC
                      dw CODE_0CD2B2
                      dw CODE_0CCD23
                      dw CODE_0CCFDE

CODE_0CCA51:          JSR CODE_0CD6EC
                      LDA.W Layer1ScrollTimer
                      BEQ +
                      LDX.B #$30
                      LDA.B #$B0
                      STA.B _0
                      LDA.B #$68
                      STA.W CreditsSprXPosLow+1
                      JSR CODE_0CCAFD
                    + LDA.W Layer1ScrollType
                      JSL ExecutePtr

                      dw CODE_0CD0C9
                      dw CODE_0CCFDE

CODE_0CCA72:          LDA.W Layer1ScrollType
                      JSL ExecutePtr

                      dw CODE_0CD0D2
                      dw CODE_0CD108
                      dw CODE_0CD16F
                      dw CODE_0CD19C
                      dw CODE_0CCFDE

DATA_0CCA83:          db $20

DATA_0CCA84:          db $48

DATA_0CCA85:          db $A6

DATA_0CCA86:          db $18,$30,$48,$A8,$18,$28,$58,$8D
                      db $08

CODE_0CCA8F:          LDA.W Layer2ScrollXSpeed
                      BEQ Return0CCACD
                      LDY.B #$00
                      TYX
                    - LDA.W DATA_0CCA83,Y
                      STA.W OAMTileXPos+$1E0,X
                      LDA.W DATA_0CCA84,Y
                      STA.W OAMTileYPos+$1E0,X
                      LDA.W DATA_0CCA85,Y
                      STA.W OAMTileNo+$1E0,X
                      LDA.W DATA_0CCA86,Y
                      AND.B #$CF
                      ORA.B #$20
                      STA.W OAMTileAttr+$1E0,X
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.W DATA_0CCA86,Y
                      AND.B #$10
                      LSR A
                      LSR A
                      LSR A
                      STA.W OAMTileSize+$78,X
                      PLX
                      INX
                      INX
                      INX
                      INX
                      TXY
                      CPY.B #$0C
                      BNE -
Return0CCACD:         RTS

CODE_0CCACE:          LDA.W Layer1ScrollType
                      BNE CODE_0CCADD
                      STZ.W Layer1ScrollTimer
                      LDA.B #$98
                      STA.W CreditsSprXPosLow+1
                      BRA Return0CCAFC

CODE_0CCADD:          LDA.W Layer1ScrollTimer
                      BEQ Return0CCAFC
                      LDA.W CreditsSprXPosLow+1
                      CMP.B #$5C
                      BCC +
                      LDX.B #$01
                      LDA.B #$F0
                      STA.W CreditsSprXSpeed,X
                      JSR CODE_0CD368
                    + LDX.B #$30
                      LDA.B #$AB
                      STA.B _0
                      BRL CODE_0CCAFD
Return0CCAFC:         RTS

CODE_0CCAFD:          LDA.B _0
                      STA.W OAMTileXPos+$180,X
                      LDA.W CreditsSprXPosLow+1
                      STA.W OAMTileYPos+$180,X
                      LDA.B #$E6
                      STA.W OAMTileNo+$180,X
                      LDA.B #$21
                      STA.W OAMTileAttr+$180,X
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.B #$02
                      STA.W OAMTileSize+$60,X
                      RTS

CODE_0CCB1C:          LDA.W Layer1ScrollCmd
                      BNE CODE_0CCB30
                      LDA.B #$60
                      STA.W CreditsSprXPosLow
                      LDA.B #$01
                      STA.W Layer2ScrollCmd
                      STZ.W Layer1ScrollBits
                      BRA CODE_0CCB55

CODE_0CCB30:          LDX.B #$00
                      LDY.B #$30
                      LDA.W CutsceneID
                      DEC A
                      BNE +
                      LDY.B #$18
                    + TYA
                      STA.W CreditsSprXSpeed,X
                      JSR CODE_0CD368
                      DEC.W Layer2ScrollCmd
                      BPL CODE_0CCB55
                      LDA.B #$01
                      STA.W Layer2ScrollCmd
                      LDA.W Layer1ScrollBits
                      EOR.B #$01
                      STA.W Layer1ScrollBits
CODE_0CCB55:          JSR CODE_0CCBFA
                      BRL CODE_0CCB80
CODE_0CCB5B:          LDY.B #$E0
                      LDA.W Layer1ScrollCmd
                      BEQ +
                      LDY.B #$E2
                    + STY.W OAMTileNo
                      LDA.B #$39
                      STA.W OAMTileAttr
                      LDA.B #$50
                      STA.W OAMTileXPos
                      LDA.B #$67
                      STA.W OAMTileYPos
                      LDA.B #$02
                      STA.W OAMTileSize
                      RTS


DATA_0CCB7C:          db $86,$87,$96,$97

CODE_0CCB80:          LDX.B #$60
                      LDA.W Layer1ScrollCmd
                      BEQ +
                      LDX.W CreditsSprXPosLow
                    + STX.B _0
                      LDA.B #$67
                      STA.B _1
                      LDX.B #$14
CODE_0CCB92:          LDA.B _0
                      CMP.B #$B0
                      BCS +
                      STA.W OAMTileXPos+$180,X
                      LDA.B _1
                      STA.W OAMTileYPos+$180,X
                      LDA.B #$E4
                      STA.W OAMTileNo+$180,X
                      LDA.B #$3F
                      STA.W OAMTileAttr+$180,X
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.B #$02
                      STA.W OAMTileSize+$60,X
                      PLX
                    + LDA.B _0
                      CLC
                      ADC.B #$10
                      STA.B _0
                      INX
                      INX
                      INX
                      INX
                      CPX.B #$28
                      BNE CODE_0CCB92
                      LDA.W Layer1ScrollCmd
                      BEQ Return0CCBF9
                      LDA.W CreditsSprXPosLow
                      SEC
                      SBC.B #$08
                      CMP.B #$B0
                      BCC CODE_0CCBD8
                      INC.W Layer1ScrollType
                      BRA Return0CCBF9

CODE_0CCBD8:          STA.W OAMTileXPos+$180,X
                      LDA.B #$6F
                      STA.W OAMTileYPos+$180,X
                      LDY.B #$85
                      LDA.W Layer1ScrollBits
                      BEQ +
                      LDY.B #$95
                    + TYA
                      STA.W OAMTileNo+$180,X
                      LDA.B #$35
                      STA.W OAMTileAttr+$180,X
                      TXA
                      LSR A
                      LSR A
                      TAX
                      STZ.W OAMTileSize+$60,X
Return0CCBF9:         RTS

CODE_0CCBFA:          LDX.B #$04
CODE_0CCBFC:          LDA.B #$A8
                      STA.B _0
                      STA.B _2
                      LDA.B #$5F
                      STA.B _1
CODE_0CCC06:          LDY.B #$00
CODE_0CCC08:          LDA.B _0
                      STA.W OAMTileXPos+$180,X
                      LDA.B _1
                      STA.W OAMTileYPos+$180,X
                      LDA.W DATA_0CCB7C,Y
                      STA.W OAMTileNo+$180,X
                      LDA.B #$2D
                      STA.W OAMTileAttr+$180,X
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.B #$02
                      STA.W OAMTileSize+$60,X
                      PLX
                      INY
                      LDA.B _0
                      CLC
                      ADC.B #$08
                      STA.B _0
                      TYA
                      AND.B #$01
                      BNE +
                      LDA.B _2
                      STA.B _0
                      LDA.B _1
                      CLC
                      ADC.B #$08
                      STA.B _1
                    + INX
                      INX
                      INX
                      INX
                      CPY.B #$04
                      BNE CODE_0CCC08
                      RTS


DATA_0CCC49:          db $03,$01,$03,$01

DATA_0CCC4D:          db $93,$73,$FF,$7F

CODE_0CCC51:          LDA.B TrueFrame
                      AND.B #$02
                      TAX
                      REP #$20                                  ; A->16
                      LDA.W DATA_0CCC4D,X
                      STA.W BackgroundColor
                      SEP #$20                                  ; A->8
                      DEC.W Layer2ScrollBits
                      BPL CODE_0CCC82
                      JSR CODE_0CD373
                      LDX.W CutsceneID
                      LDA.L CutsceneBgColor-1,X
                      ASL A
                      TAX
                      REP #$20                                  ; A->16
                      LDA.L BackAreaColors,X
                      STA.W BackgroundColor
                      SEP #$20                                  ; A->8
                      INC.W Layer1ScrollType
                      BRL CODE_0CCC9A
CODE_0CCC82:          DEC.W Layer2ScrollCmd
                      BPL +
                      LDA.W Layer1ScrollBits
                      INC A
                      AND.B #$03
                      STA.W Layer1ScrollBits
                      TAX
                      LDA.W DATA_0CCC49,X
                      STA.W Layer2ScrollCmd
                    + JSR CODE_0CCCB6
CODE_0CCC9A:          LDX.B #$1C
                      BRL CODE_0CCBFC

DATA_0CCC9F:          db $A0,$A4,$00,$C0,$C4,$00,$A0,$A2
                      db $A4,$C0,$C2,$C4,$00,$00,$00,$00
                      db $00,$00

DATA_0CCCB1:          db $00,$06,$0C,$06,$0C

CODE_0CCCB6:          LDA.W Layer1ScrollBits
                      INC A
                      TAX
                      LDA.W DATA_0CCCB1,X
                      STA.B _8
                      LDY.B #$A0
                      CPX.B #$00
                      BNE +
                      LDY.B #$A8
                    + STY.B _0
                      STY.B _4
                      LDA.B #$57
                      STA.B _2
                      LDX.B #$00
CODE_0CCCD2:          LDY.B _8
                      LDA.W DATA_0CCC9F,Y
                      BEQ CODE_0CCD02
                      STA.W OAMTileNo+4,X
                      LDY.B #$35
                      LDA.W Layer1ScrollBits
                      BMI +
                      AND.B #$02
                      BEQ +
                      LDY.B #$39
                    + TYA
                      STA.W OAMTileAttr+4,X
                      LDA.B _0
                      STA.W OAMTileXPos+4,X
                      LDA.B _2
                      STA.W OAMTileYPos+4,X
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.B #$02
                      STA.W OAMTileSize+1,X
                      PLX
CODE_0CCD02:          INC.B _8
                      LDA.B _0
                      CLC
                      ADC.B #$10
                      STA.B _0
                      INX
                      INX
                      INX
                      INX
                      CPX.B #$0C
                      BNE +
                      LDA.B _4
                      STA.B _0
                      LDA.B _2
                      CLC
                      ADC.B #$10
                      STA.B _2
                    + CPX.B #$18
                      BNE CODE_0CCCD2
                      RTS

CODE_0CCD23:          LDX.B #$00
                      LDA.B Layer1YPos
                      CMP.B #$C0
                      BNE CODE_0CCD31
                      STZ.B Layer1XPos
                      STZ.B Layer1XPos+1
                      BRA CODE_0CCD75

CODE_0CCD31:          STA.W CreditsSprYPosLow,X
                      LDA.B Layer1YPos+1
                      STA.W CreditsSprYPosHigh,X
                      LDA.B #$F0
                      STA.W CreditsSprYSpeed,X
                      JSR CODE_0CD33A
                      JSR CODE_0CD283
                      SEP #$20                                  ; A->8
                      LDA.W CreditsSprYPosHigh,X
                      STA.B Layer1YPos+1
                      LDA.W CreditsSprYPosLow,X
                      STA.B Layer1YPos
                      CMP.B #$FD
                      BNE +
                      LDA.B #$01
                      STA.W Layer1ScrollTimer
                    + LDA.B Layer1YPos
                      CMP.B #$E8
                      BNE +
                      JSR CODE_0CD5C9
                    + LDA.B Layer1YPos
                      CMP.B #$C0
                      BNE CODE_0CCD75
                      LDA.B #!SFX_RUMBLINGOFF
                      STA.W SPCIO0                              ; / Play sound effect
                      JSR CODE_0CD1D0
                      LDA.B #$08
                      STA.W CreditsSprYSpeed+1
CODE_0CCD75:          LDX.B #$01
                      LDA.W CreditsSprYSpeed,X
                      BPL CODE_0CCD85
                      LDA.W CreditsSprYPosLow,X
                      CMP.B #$68
                      BCS CODE_0CCD91
                      BRA CODE_0CCD94

CODE_0CCD85:          LDA.W CreditsSprYPosLow,X
                      CMP.B #$78
                      BNE CODE_0CCD91
                      INC.W Layer1ScrollType
                      BRA +

CODE_0CCD91:          JSR CODE_0CD33A
CODE_0CCD94:          LDX.B #$00
                      LDA.B #$04
                      STA.W CreditsSprXSpeed,X
                      JSR CODE_0CD368
                    + BRL CODE_0CCE2A
CODE_0CCDA1:          LDA.B TrueFrame
                      AND.B #$03
                      BNE +
                      LDA.W Layer1ScrollXSpeed+1
                      CLC
                      ADC.B #$02
                      STA.W Layer1ScrollXSpeed+1
                      CMP.B #$80
                      BCC +
                      LDA.B #$7F
                      STA.W Layer1ScrollXSpeed+1
                    + LDX.B #$00
                      LDA.B Layer1YPos
                      STA.W CreditsSprYPosLow,X
                      LDA.B Layer1YPos+1
                      STA.W CreditsSprYPosHigh,X
                      LDA.W Layer1ScrollXSpeed+1
                      STA.W CreditsSprYSpeed,X
                      JSR CODE_0CD33A
                      LDA.W CreditsSprYPosLow,X
                      BEQ CODE_0CCDD7
                      CMP.B #$20
                      BCS +
CODE_0CCDD7:          JSR CODE_0CD283
                    + LDA.W CreditsSprYPosHigh,X
                      STA.B Layer1YPos+1
                      LDA.W CreditsSprYPosLow,X
                      STA.B Layer1YPos
                      BEQ +
                      CMP.B #$20
                      BCC +
                      LDA.B #$08
                      STA.W CreditsSprYSpeed+1
                    + LDA.W CreditsSprYPosLow,X
                      BEQ CODE_0CCE02
                      CMP.B #$A8
                      BCC CODE_0CCE02
                      LDA.B #$7F
                      STA.W Layer2ScrollCmd
                      INC.W Layer1ScrollType
                      BRA CODE_0CCE1E

CODE_0CCE02:          SEP #$20                                  ; A->8
                      LDX.B #$01
                      LDA.W CreditsSprYSpeed,X
                      BPL CODE_0CCE14
                      LDA.W CreditsSprYPosLow,X
                      CMP.B #$68
                      BCS CODE_0CCE1B
                      BRA CODE_0CCE1E

CODE_0CCE14:          LDA.W CreditsSprYPosLow,X
                      CMP.B #$78
                      BCS CODE_0CCE1E
CODE_0CCE1B:          JSR CODE_0CD33A
CODE_0CCE1E:          LDA.B #$77
                      SEC
                      SBC.W CreditsSprYPosLow
                      STA.W CreditsSprYPosLow+2
                      JSR CODE_0CCEAB
CODE_0CCE2A:          JSR CODE_0CCE48
                      LDX.B #$14
                      LDA.B #$A8
                      STA.B _0
                      STA.B _2
                      LDA.B #$5F
                      SEC
                      SBC.W CreditsSprYPosLow
                      STA.B _1
                      BRL CODE_0CCC06

DATA_0CCE40:          db $80,$81,$82,$83,$83,$82,$81,$80

CODE_0CCE48:          DEC.W Layer2ScrollCmd
                      BPL +
                      LDA.B #$05
                      STA.W Layer2ScrollCmd
                      LDA.W Layer1ScrollBits
                      EOR.B #$01
                      STA.W Layer1ScrollBits
                    + LDA.B #$98
                      STA.B _0
                      LDY.B #$21
                      LDA.W Layer1ScrollBits
                      BEQ +
                      LDY.B #$61
                    + STY.B _4
                      LDA.W Layer1ScrollBits
                      ASL A
                      ASL A
                      TAY
                      LDX.B #$00
                    - LDA.B _0
                      STA.W OAMTileXPos+4,X
                      LDA.W CreditsSprYPosLow+1
                      STA.W OAMTileYPos+4,X
                      LDA.W DATA_0CCE40,Y
                      STA.W OAMTileNo+4,X
                      LDA.B _4
                      STA.W OAMTileAttr+4,X
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.B #$02
                      STA.W OAMTileSize+1,X
                      PLX
                      LDA.B _0
                      CLC
                      ADC.B #$10
                      STA.B _0
                      INY
                      INX
                      INX
                      INX
                      INX
                      CPX.B #$10
                      BNE -
                      RTS


DATA_0CCEA3:          db $C6,$C8,$C6,$C8

DATA_0CCEA7:          db $25,$25,$65,$65

CODE_0CCEAB:          LDX.B #$30
                      LDA.B #$B0
                      STA.W OAMTileXPos,X
                      LDA.W CreditsSprYPosLow+2
                      STA.W OAMTileYPos,X
                      LDA.B TrueFrame
                      AND.B #$06
                      LSR A
                      TAY
                      LDA.W DATA_0CCEA3,Y
                      STA.W OAMTileNo,X
                      LDA.W DATA_0CCEA7,Y
                      STA.W OAMTileAttr,X
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.B #$02
                      STA.W OAMTileSize,X
                      RTS


DATA_0CCED4:          db $02,$FF,$02,$03,$04,$05,$06

DATA_0CCEDB:          db $03,$01,$07,$07,$07,$07,$07

CODE_0CCEE2:          LDA.W Layer1ScrollBits
                      CMP.B #$02
                      BCS CODE_0CCF0F
                      LDA.B TrueFrame
                      AND.B #$01
                      BEQ +
                      LDA.W Layer1ScrollBits
                      EOR.B #$01
                      STA.W Layer1ScrollBits
                    + LDX.B #$02
                      JSR CODE_0CD33A
                      JSR CODE_0CD368
                      LDA.W CreditsSprYPosLow+2
                      CMP.B #$5C
                      BCC CODE_0CCF0D
                      LDA.B #$02
                      STA.W Layer1ScrollBits
                      BRA CODE_0CCF0F

CODE_0CCF0D:          BRA CODE_0CCF38

CODE_0CCF0F:          DEC.W Layer2ScrollCmd
                      BPL CODE_0CCF38
                      LDA.W Layer1ScrollXSpeed+1
                      INC A
                      STA.W Layer1ScrollXSpeed+1
                      TAX
                      LDA.W DATA_0CCED4,X
                      STA.W Layer1ScrollBits
                      LDA.W DATA_0CCEDB,X
                      STA.W Layer2ScrollCmd
                      CPX.B #$01
                      BNE +
                      LDA.B #!SFX_SPINKILL
                      STA.W SPCIO0                              ; / Play sound effect
                    + CPX.B #$06
                      BNE CODE_0CCF38
                      JSR CODE_0CD5C6
CODE_0CCF38:          BRL CODE_0CCF49

DATA_0CCF3B:          db $B7,$B8,$89,$99,$A9,$B9,$E8

DATA_0CCF42:          db $25,$25,$23,$23,$23,$23,$23

CODE_0CCF49:          LDY.W Layer1ScrollBits
                      CPY.B #$FF
                      BEQ +
                      LDX.B #$04
                      LDA.W CreditsSprXPosLow+2
                      STA.W OAMTileXPos,X
                      LDA.W CreditsSprYPosLow+2
                      STA.W OAMTileYPos,X
                      LDA.W DATA_0CCF3B,Y
                      STA.W OAMTileNo,X
                      LDA.W DATA_0CCF42,Y
                      STA.W OAMTileAttr,X
                      TXA
                      LSR A
                      LSR A
                      TAX
                      STZ.W OAMTileSize,X
                    + RTS

CODE_0CCF72:          DEC.W Layer2ScrollCmd
                      BPL CODE_0CCF8B
                      LDA.B #$03
                      STA.W Layer2ScrollCmd
                      INC.W Layer1ScrollBits
                      LDA.W Layer1ScrollBits
                      CMP.B #$04
                      BNE CODE_0CCF8B
                      INC.W Layer1ScrollType
                      BRA CODE_0CCF90

CODE_0CCF8B:          LDX.B #$04
                      JSR CODE_0CCFA3
CODE_0CCF90:          LDX.B #$08
                      LDA.B #$A8
                      STA.B _0
                      STA.B _2
                      LDA.B #$5F
                      STA.B _1
                      BRL CODE_0CCC06

DATA_0CCF9F:          db $60,$62,$64,$66

CODE_0CCFA3:          LDY.W Layer1ScrollBits
                      LDA.W DATA_0CCF9F,Y
                      STA.W OAMTileNo,X
                      LDY.B #$21
                      STA.W OAMTileAttr,X
                      LDA.B #$AC
                      STA.W OAMTileXPos,X
                      LDA.B #$63
                      STA.W OAMTileYPos,X
                      TXA
                      LSR A
                      LSR A
                      TAX
                      LDA.B #$02
                      STA.W OAMTileSize,X
                      RTS

CODE_0CCFC5:          LDA.W Layer2ScrollTimer
                      CMP.B #$03
                      BEQ CODE_0CCFD3
                      LDA.B #$08
                      STA.W Layer2ScrollCmd
                      BRA +

CODE_0CCFD3:          DEC.W Layer2ScrollCmd
                      BPL +
                      INC.W Layer1ScrollType
                    + BRL CODE_0CCF90
CODE_0CCFDE:          LDA.W CutsceneTextTimer
                      ORA.W Layer2ScrollYSpeed+1
                      BNE +
                   if ver_is_console(!_VER)                     ;\=============== J, U, E0, & E1 ================
                      LDA.B byetudlrFrame                       ;!
                      ORA.B axlr0000Frame                       ;!
                      AND.B #$C0                                ;!
                   else                                         ;<====================== SS =====================
                      DEC.W EnterLevelAuto                      ;!
                      BEQ CODE_0CCFEE                           ;!
                      LDA.B byetudlrFrame                       ;!
                      ORA.B axlr0000Frame                       ;!
                   endif                                        ;/===============================================
                      BEQ +
CODE_0CCFEE:          STZ.W Layer1ScrollType
                      LDA.B #$0B
                      STA.W GameMode
                    + RTS

CODE_0CCFF7:          LDA.W CutsceneTextTimer
                      BEQ Return0CD002
                      INC.W Layer1ScrollType
                      BRL CODE_0CCFDE
Return0CD002:         RTS

CODE_0CD003:          LDA.W Layer1ScrollCmd
                      CMP.B #$01
                      BEQ CODE_0CD034
                      CMP.B #$02
                      BEQ CODE_0CD019
                      CMP.B #$03
                      BEQ CODE_0CD03E
                      LDA.B #$10
                      STA.W Layer2ScrollCmd
                      BRA CODE_0CD046

CODE_0CD019:          LDA.W Layer2ScrollCmd
                      AND.B #$F8
                      BEQ +
                      JSR CODE_0CD069
                    + JSR CODE_0CD283
                      DEC.W Layer2ScrollCmd
                      BPL CODE_0CD046
                      STZ.W Layer1ScrollCmd
                      STZ.B Layer1XPos
                      STZ.B Layer1XPos+1
                      BRA CODE_0CD046

CODE_0CD034:          LDA.B #$3F
                      STA.W Layer2ScrollCmd
                      LDA.B #$03
                      STA.W Layer1ScrollCmd
CODE_0CD03E:          DEC.W Layer2ScrollCmd
                      BPL CODE_0CD046
                      INC.W Layer1ScrollType
CODE_0CD046:          LDX.B #$A8
                      LDA.B Layer1XPos
                      BEQ CODE_0CD054
                      BPL CODE_0CD052
                      LDX.B #$A9
                      BRA CODE_0CD054

CODE_0CD052:          LDX.B #$A7
CODE_0CD054:          STX.B _0
                      STX.B _2
                      LDA.B #$5F
                      STA.B _1
                      LDX.B #$08
                      BRL CODE_0CCC06

DATA_0CD061:          db $7C,$7D,$7D,$7C

DATA_0CD065:          db $30,$30,$F0,$F0

CODE_0CD069:          LDY.B #$00
                      LDX.B #$04
                      LDA.B PlayerXPosNext
                      CLC
                      ADC.B #$10
                      STA.B _0
                      STA.B _2
                      LDA.B PlayerYPosNext
                      CLC
                      ADC.B #$10
                      STA.B _1
CODE_0CD07D:          LDA.B _0
                      STA.W OAMTileXPos,X
                      LDA.B _1
                      STA.W OAMTileYPos,X
                      LDA.W DATA_0CD061,Y
                      STA.W OAMTileNo,X
                      LDA.W DATA_0CD065,Y
                      STA.W OAMTileAttr,X
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      STZ.W OAMTileSize,X
                      PLX
                      LDA.B _0
                      CLC
                      ADC.B #$08
                      STA.B _0
                      INY
                      CPY.B #$02
                      BNE +
                      LDA.B _2
                      STA.B _0
                      LDA.B _1
                      CLC
                      ADC.B #$08
                      STA.B _1
                    + INX
                      INX
                      INX
                      INX
                      CPY.B #$04
                      BNE CODE_0CD07D
                      RTS

CODE_0CD0BC:          LDA.W Layer1ScrollCmd
                      CMP.B #$01
                      BNE +
                      INC.W Layer1ScrollType
                    + BRL CODE_0CCF90
CODE_0CD0C9:          LDA.W Layer1ScrollCmd
                      BEQ +
                      JSR CODE_0CD5C6
                    + RTS

CODE_0CD0D2:          LDA.W Layer1ScrollCmd
                      CMP.B #$01
                      BNE Return0CD107
                      LDX.B #$00
                      LDA.B Layer1YPos
                      STA.W CreditsSprYPosLow,X
                      LDA.B Layer1YPos+1
                      STA.W CreditsSprYPosHigh,X
                      LDA.B #$08
                      STA.W CreditsSprYSpeed,X
                      JSR CODE_0CD33A
                      LDA.W CreditsSprYPosLow,X
                      BEQ CODE_0CD100
                      CMP.B #$09
                      BCC CODE_0CD100
                      INC.W Layer1ScrollType
                      LDA.B #$28
                      STA.W Layer1ScrollXSpeed+1
                      BRA Return0CD107

CODE_0CD100:          STA.B Layer1YPos
                      LDA.W CreditsSprYPosHigh,X
                      STA.B Layer1YPos+1
Return0CD107:         RTS

CODE_0CD108:          LDA.W Layer1ScrollCmd
                      CMP.B #$02
                      BNE Return0CD16E
                      DEC.W Layer1ScrollXSpeed+1
                      LDA.W Layer1ScrollXSpeed+1
                      BMI +
                      CMP.B #$24
                      BCC +
                      JSR CODE_0CD069
                    + LDA.W Layer1ScrollXSpeed+1
                      CMP.B #$C8
                      BNE +
                      LDA.B #$40
                      STA.W Layer2ScrollCmd
                      LDA.B #!SFX_CUTSCENEBOMB
                      STA.W SPCIO3                              ; / Play sound effect
                      INC.W Layer1ScrollType
                      RTS

                    + LDX.B #$00
                      LDA.B Layer1YPos
                      STA.W CreditsSprYPosLow,X
                      LDA.B Layer1YPos+1
                      STA.W CreditsSprYPosHigh,X
                      LDA.B Layer1XPos
                      STA.W CreditsSprXPosLow,X
                      LDA.B Layer1XPos+1
                      STA.W CreditsSprXPosHigh,X
                      LDA.W Layer1ScrollXSpeed+1
                      STA.W CreditsSprYSpeed,X
                      LDA.B #$E8
                      STA.W CreditsSprXSpeed,X
                      JSR CODE_0CD33A
                      JSR CODE_0CD368
                      LDA.W CreditsSprYPosLow,X
                      STA.B Layer1YPos
                      LDA.W CreditsSprYPosHigh,X
                      STA.B Layer1YPos+1
                      LDA.W CreditsSprXPosLow,X
                      STA.B Layer1XPos
                      LDA.W CreditsSprXPosHigh,X
                      STA.B Layer1XPos+1
Return0CD16E:         RTS

CODE_0CD16F:          LDA.B TrueFrame
                      AND.B #$02
                      BEQ CODE_0CD183
                      REP #$20                                  ; A->16
                      LDA.B Layer2YPos
                      SEC
                      SBC.W #$0001
                      STA.B Layer2YPos
                      SEP #$20                                  ; A->8
                      BRA +

CODE_0CD183:          REP #$20                                  ; A->16
                      LDA.B Layer2YPos
                      CLC
                      ADC.W #$0001
                      STA.B Layer2YPos
                      SEP #$20                                  ; A->8
                    + DEC.W Layer2ScrollCmd
                      BPL +
                      STZ.B Layer2YPos
                      STZ.B Layer2YPos+1
                      INC.W Layer1ScrollType
                    + RTS

CODE_0CD19C:          LDA.W Layer1ScrollCmd
                      CMP.B #$03
                      BNE +
                      JSR CODE_0CD5C6
                    + RTS


DATA_0CD1A7:          db $20,$B3,$00,$0F,$C0,$01,$C1,$01
                      db $C2,$01,$C3,$01,$C4,$01,$C5,$01
                      db $C1,$41,$C0,$41,$20,$D3,$00,$0F
                      db $D0,$01,$D1,$01,$D2,$01,$D3,$01
                      db $D4,$01,$D5,$01,$D1,$41,$D0,$41
                      db $FF

CODE_0CD1D0:          LDY.B #$28
                      TYA
                      CLC
                      ADC.L DynStripeImgSize
                      TAX
                    - LDA.W DATA_0CD1A7,Y
                      STA.L DynamicStripeImage,X
                      DEX
                      DEY
                      BPL -
                      LDA.B #$28
                      CLC
                      ADC.L DynStripeImgSize
                      STA.L DynStripeImgSize
                      RTS


DATA_0CD1F0:          db $20,$F4,$40,$02,$F8,$00,$21,$14
                      db $40,$02,$F8,$00,$FF

DATA_0CD1FD:          db $20,$F4

DATA_0CD1FF:          db $21,$14,$21,$34,$21,$54,$21,$74
                      db $21,$94,$21,$B4,$21,$D4,$20,$F6
                      db $21,$16,$21,$36,$21,$56,$21,$76
                      db $21,$96,$21,$B6,$21,$D6,$20,$F8
                      db $21,$18,$21,$38,$21,$58,$21,$78
                      db $21,$98,$21,$B8,$21,$D8

CODE_0CD22D:          LDA.L DynStripeImgSize
                      STA.B _1
                      LDY.B #$0C
                      TYA
                      CLC
                      ADC.L DynStripeImgSize
                      TAX
                    - LDA.W DATA_0CD1F0,Y
                      STA.L DynamicStripeImage,X
                      DEX
                      DEY
                      BPL -
                      LDA.B #$0C
                      CLC
                      ADC.L DynStripeImgSize
                      STA.L DynStripeImgSize
                      LDA.B PlayerXPosNext
                      SEC
                      SBC.B #$A0
                      STA.B _0
                      LDA.W Layer1ScrollXSpeed
                      SEC
                      SBC.B #$38
                      LSR A
                      LSR A
                      CLC
                      ADC.B _0
                      TAY
                      LDX.B _1
                      REP #$20                                  ; A->16
                      LDA.W DATA_0CD1FD,Y
                      STA.L DynamicStripeImage,X
                      LDA.W DATA_0CD1FF,Y
                      STA.L DynamicStripeImage+6,X
                      SEP #$20                                  ; A->8
                      CPY.B #$1C
                      BNE +
                      LDA.B #$01
                      STA.W Layer1ScrollTimer
                    + RTS

CODE_0CD283:          REP #$20                                  ; A->16
                      LDA.B TrueFrame
                      AND.W #$0001
                      BEQ CODE_0CD290
                      INC.B Layer1XPos
                      BRA +

CODE_0CD290:          DEC.B Layer1XPos
                    + SEP #$20                                  ; A->8
                      RTS

CODE_0CD295:          LDA.B #!SFX_CUTSCENEBOMB
                      STA.W SPCIO3                              ; / Play sound effect
                      LDA.B #$FF
                      STA.W Layer1ScrollBits
                      LDA.B #$30
                      STA.W Layer2ScrollBits
                      LDA.B #$01
                      STA.W Layer2ScrollCmd
                      INC.W Layer1ScrollType
                      JSR CODE_0CCBFA
                      BRL CODE_0CCC51
CODE_0CD2B2:          LDA.B #!SFX_RUMBLINGON
                      STA.W SPCIO0                              ; / Play sound effect
                      JSR CODE_0CD31A
                      BRL CODE_0CCD23
CODE_0CD2BD:          LDA.B #!SFX_FIRESPIT
                      STA.W SPCIO3                              ; / Play sound effect
                      LDA.B #$77
                      STA.W CreditsSprYPosLow+2
                      STZ.W Layer1ScrollXSpeed+1
                      JSR CODE_0CD31A
                      BRL CODE_0CCDA1
CODE_0CD2D0:          LDA.B #$03
                      STA.W Layer2ScrollCmd
                      STZ.W Layer1ScrollBits
                      LDA.B #!SFX_CLAP
                      STA.W SPCIO3                              ; / Play sound effect
                      INC.W Layer1ScrollType
                      JSR CODE_0CCBFA
                      BRL CODE_0CCF72
CODE_0CD2E6:          DEC.W Layer2ScrollCmd
                      BMI +
                      RTS

                    + JSR CODE_0CD373
                      LDA.B #$0F
                      STA.W CreditsSprYPosLow+2
                      STZ.W CreditsSprYPosHigh+2
                      LDA.B #$90
                      STA.W CreditsSprXPosLow+2
                      STZ.W CreditsSprXPosHigh+2
                      LDA.B #$08
                      STA.W CreditsSprYSpeed+2
                      LDA.B #$FF
                      STA.W CreditsSprXSpeed+2
                      LDA.B #$02
                      STA.W Layer2ScrollCmd
                      STZ.W Layer1ScrollBits
                      STZ.W Layer1ScrollXSpeed+1
                      INC.W Layer1ScrollType
                      BRL CODE_0CCEE2
CODE_0CD31A:          LDA.B #$77
                      STA.W CreditsSprYPosLow+1
                      LDA.B #$5F
                      STA.W CreditsSprXPosLow
                      LDA.B #$F8
                      STA.W CreditsSprYSpeed+1
                      STZ.W Layer1ScrollBits
                      LDA.B #$05
                      STA.W Layer2ScrollCmd
                      INC.W Layer1ScrollType
                      JSR CODE_0CD373
                      BRL CODE_0CCBFA
CODE_0CD33A:          LDA.W CreditsSprYSpeed,X
                      ASL A
                      ASL A
                      ASL A
                      ASL A
                      CLC
                      ADC.W CreditsSprYPosSpx,X
                      STA.W CreditsSprYPosSpx,X
                      PHP
                      LDA.W CreditsSprYSpeed,X
                      LSR A
                      LSR A
                      LSR A
                      LSR A
                      CMP.B #$08
                      LDY.B #$00
                      BCC +
                      ORA.B #$F0
                      DEY
                    + PLP
                      ADC.W CreditsSprYPosLow,X
                      STA.W CreditsSprYPosLow,X
                      TYA
                      ADC.W CreditsSprYPosHigh,X
                      STA.W CreditsSprYPosHigh,X
                      RTS

CODE_0CD368:          PHX
                      TXA
                      CLC
                      ADC.B #$0F
                      TAX
                      JSR CODE_0CD33A
                      PLX
                      RTS

CODE_0CD373:          STZ.W CreditsSprYPosSpx
                      STZ.W CreditsSprXPosSpx
                      STZ.W CreditsSprYPosSpx+1
                      STZ.W CreditsSprXPosSpx+1
                      STZ.W CreditsSprXPosSpx+2
                      STZ.W CreditsSprXPosSpx+2
                      RTS


DATA_0CD386:          db $C8,$C0,$C4,$C8,$C0,$B4,$C8,$B8
                      db $C6,$B7,$C4,$B0,$C8,$C0,$C8,$C4
DATA_0CD396:          db $18,$F8,$0A,$20,$E8,$1A,$EA,$08
                      db $F0,$18,$E0,$2A,$F8,$20,$FA,$18

CODE_0CD3A6:          LDA.W Layer2ScrollTimer
                      DEC A
                      CMP.B #$E7
                      BCS Return0CD3F3
                      LDA.B TrueFrame
                      AND.B #$01
                      BNE Return0CD3F3
                      LDX.B #$0E
CODE_0CD3B6:          LDA.W CastleCutExSprSlot,X
                      BNE CODE_0CD3EE
                      LDA.B #$01
                      STA.W CastleCutExSprSlot,X
                      LDA.B #$04
                      STA.W CastleCutExSprAccel,X
                      LDA.W DATA_0CD386,X
                      STA.W CreditsSprYSpeed,X
                      LDA.W DATA_0CD396,X
                      STA.W CreditsSprXSpeed,X
                      LDA.B PlayerXPosNext
                      CLC
                      ADC.B #$18
                      STA.W CreditsSprXPosLow,X
                      LDA.B PlayerYPosNext
                      CLC
                      ADC.B #$20
                      STA.W CreditsSprYPosLow,X
                      LDA.B TrueFrame
                      AND.B #$02
                      BNE Return0CD3F3
                      LDA.B #!SFX_SHATTER
                      STA.W SPCIO3                              ; / Play sound effect
                      BRA Return0CD3F3

CODE_0CD3EE:          DEX
                      CPX.B #$03
                      BNE CODE_0CD3B6
Return0CD3F3:         RTS

CODE_0CD3F4:          LDX.B #$0E
CODE_0CD3F6:          LDA.W CastleCutExSprSlot,X
                      BEQ CODE_0CD41B
                      LDA.W CreditsSprYSpeed,X
                      BMI CODE_0CD404
                      CMP.B #$40
                      BCS +
CODE_0CD404:          CLC
                      ADC.W CastleCutExSprAccel,X
                      STA.W CreditsSprYSpeed,X
                    + JSR CODE_0CD33A
                      JSR CODE_0CD368
                      LDA.W CreditsSprYPosLow,X
                      CMP.B #$80
                      BCC CODE_0CD41B
                      STZ.W CastleCutExSprSlot,X
CODE_0CD41B:          DEX
                      CPX.B #$03
                      BNE CODE_0CD3F6
                      BRL +

DATA_0CD423:          db $3C,$3D

                    + LDY.B #$0E
                      LDX.B #$14
CODE_0CD429:          LDA.W CastleCutExSprSlot,Y
                      BEQ +
                      LDA.W CreditsSprXPosLow,Y
                      CMP.B #$50
                      BCC +
                      STA.W OAMTileXPos,X
                      LDA.W CreditsSprYPosLow,Y
                      STA.W OAMTileYPos,X
                      PHY
                      LDA.B TrueFrame
                      AND.B #$02
                      LSR A
                      TAY
                      LDA.W DATA_0CD423,Y
                      STA.W OAMTileNo,X
                      PLY
                      LDA.B #$22
                      STA.W OAMTileAttr,X
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      STZ.W OAMTileSize,X
                      PLX
                    + INX
                      INX
                      INX
                      INX
                      DEY
                      CPY.B #$03
                      BNE CODE_0CD429
                      RTS


DATA_0CD464:          db $F8,$80,$80,$80,$FA,$80,$80,$80
                      db $FA,$FC,$80,$80,$FA,$FC,$04,$80
                      db $F8,$80,$80,$80,$F9,$80,$80,$80
                      db $FA,$80,$80,$80,$FA,$80,$80,$80
DATA_0CD484:          db $08,$80,$80,$80,$09,$80,$80,$80
                      db $09,$FF,$80,$80,$09,$FF,$F6,$80
                      db $04,$80,$80,$80,$00,$80,$80,$80
                      db $FF,$80,$80,$80,$00,$80,$80,$80

CODE_0CD4A4:          PHB
                      PHK
                      PLB
                      LDX.B #$00
                      LDA.B Powerup
                      BNE +
                      LDX.B #$08
                    + STX.B _2
                      LDX.B #$40
                      LDA.B #$02
                      STA.B _0
                      LDA.W Layer2ScrollTimer
                      ASL A
                      ASL A
                      TAY
CODE_0CD4BD:          LDA.W DATA_0CD484,Y
                      CMP.B #$80
                      BEQ +
                      CLC
                      ADC.B PlayerXPosNext
                      STA.W OAMTileXPos,X
                      LDA.W DATA_0CD464,Y
                      CLC
                      ADC.B PlayerYPosNext
                      CLC
                      ADC.B _2
                      STA.W OAMTileYPos,X
                      LDA.B #$B6
                      STA.W OAMTileNo,X
                      LDA.B #$31
                      STA.W OAMTileAttr,X
                      PHX
                      TXA
                      LSR A
                      LSR A
                      TAX
                      STZ.W OAMTileSize,X
                      PLX
                      INX
                      INX
                      INX
                      INX
                    + INY
                      DEC.B _0
                      BPL CODE_0CD4BD
                      PLB
                      RTL


DATA_0CD4F4:          db $89,$99,$A9,$B9

CODE_0CD4F8:          LDA.W Layer1ScrollXSpeed
                      BNE CODE_0CD502
                      STZ.W CreditsSprYPosLow+2
                      BRA Return0CD556

CODE_0CD502:          LDX.B #$02
                      LDA.B #$FD
                      STA.W CreditsSprYSpeed,X
                      JSR CODE_0CD33A
                      LDA.W CreditsSprYPosLow,X
                      EOR.B #$FF
                      INC A
                      CMP.B #$0D
                      BCC CODE_0CD51B
                      STZ.W Layer1ScrollXSpeed
                      BRA Return0CD556

CODE_0CD51B:          LDX.B #$00
                      LDA.B Powerup
                      BNE +
                      LDX.B #$08
                    + STX.B _0
                      LDX.B #$34
                      LDA.B #$04
                      CLC
                      ADC.B PlayerXPosNext
                      STA.W OAMTileXPos,X
                      LDA.W CreditsSprYPosLow+2
                      CLC
                      ADC.B PlayerYPosNext
                      CLC
                      ADC.B _0
                      STA.W OAMTileYPos,X
                      LDA.W CreditsSprYPosLow+2
                      EOR.B #$FF
                      INC A
                      LSR A
                      LSR A
                      TAY
                      LDA.W DATA_0CD4F4,Y
                      STA.W OAMTileNo,X
                      LDA.B #$33
                      STA.W OAMTileAttr,X
                      TXA
                      LSR A
                      LSR A
                      TAX
                      STZ.W OAMTileSize,X
Return0CD556:         RTS

CODE_0CD557:          LDA.W Layer1ScrollXSpeed
                      BNE CODE_0CD564
                      STZ.W CreditsSprYPosLow+3
                      STZ.W CreditsSprXPosLow+3
                      BRA Return0CD5C5

CODE_0CD564:          LDX.B #$03
                      LDA.B #$06
                      STA.W CreditsSprXSpeed,X
                      LDA.B #$01
                      STA.W CreditsSprYSpeed,X
                      JSR CODE_0CD33A
                      JSR CODE_0CD368
                      LDA.W CreditsSprXPosLow,X
                      CMP.B #$0D
                      BCC CODE_0CD585
                      STZ.W Layer1ScrollXSpeed
                      INC.W Layer1ScrollType
                      BRA Return0CD5C5

CODE_0CD585:          LDX.B #$0F
                      LDY.B #$0C
                      LDA.B Powerup
                      BNE +
                      LDX.B #$13
                    + STX.B _0
                      STY.B _1
                      LDX.B #$34
                      LDA.W CreditsSprXPosLow+3
                      CLC
                      ADC.B PlayerXPosNext
                      CLC
                      ADC.B _1
                      STA.W OAMTileXPos,X
                      LDA.W CreditsSprYPosLow+3
                      CLC
                      ADC.B PlayerYPosNext
                      CLC
                      ADC.B _0
                      STA.W OAMTileYPos,X
                      LDA.W CreditsSprXPosLow+3
                      LSR A
                      LSR A
                      TAY
                      LDA.W DATA_0CD4F4,Y
                      STA.W OAMTileNo,X
                      LDA.B #$33
                      STA.W OAMTileAttr,X
                      TXA
                      LSR A
                      LSR A
                      TAX
                      STZ.W OAMTileSize,X
Return0CD5C5:         RTS

CODE_0CD5C6:          INC.W Layer1ScrollType
CODE_0CD5C9:          LDA.B #con($80,$FF,$FF,$FF,$FF)
                      STA.W CutsceneTextTimer
                      LDA.B #con($D0,$D0,$01,$A8,$A8)
                      STA.W Layer2ScrollYSpeed+1
                      LDA.B #!BGM_CUTSCENEFULL
                      STA.W SPCIO2
                      RTS


DATA_0CD5D9:          db $35,$36,$36,$36,$37,$37,$37,$37
                      db $37,$37,$37,$36,$36,$35,$35,$35
DATA_0CD5E9:          db $8B,$8B,$8B,$AA,$8A,$CE,$CE,$8A
                      db $8A,$AA,$AA,$8A,$AA,$8A,$CE,$8A
                      db $AA,$8A,$8A

DATA_0CD5FC:          db $61,$61,$61,$61,$61,$61,$61,$21
                      db $21,$21,$21,$A1,$21,$21,$61,$61
                      db $61,$E1,$E1

DATA_0CD60F:          db $FA,$FA,$F9,$FB,$F9,$FA,$F0,$F0
                      db $F4,$F4,$FD,$FF,$FD,$FF,$0A,$0D
                      db $0A,$0D,$0E,$10,$0E,$10,$0D,$0E
                      db $0E,$10,$0A,$0D,$FD,$FF,$F4,$F4
                      db $F0,$F0,$F3,$F0,$F3,$F0

DATA_0CD635:          db $06,$03,$07,$00,$07,$01,$0D,$08
                      db $02,$FB,$FF,$F6,$FF,$F6,$03,$FD
                      db $03,$FD,$0A,$06,$0A,$06,$12,$0F
                      db $0A,$06,$03,$FD,$FF,$F6,$02,$FB
                      db $0D,$08,$15,$10,$15,$10

CODE_0CD65B:          LDA.W Layer2ScrollXSpeed
                      LSR A
                      BEQ Return0CD6C3
                      LDA.W Layer2ScrollTimer
                      BNE CODE_0CD66B
                      LDX.W PlayerWalkingPose
                      BRA CODE_0CD680

CODE_0CD66B:          DEC.W Layer2ScrollTimer
                      CMP.B #$F0
                      BCC +
                      LDA.B #$0F
                    + AND.B #$0F
                      CLC
                      ADC.B #$03
                      TAX
                      LDA.W DATA_0CD5D9-3,X
                      STA.W PlayerPose
CODE_0CD680:          LDY.B #$00
                      LDA.W DATA_0CD5E9,X
                      LSR A
                      BCC +
                      LDY.B #$30
                    + ASL A
                      STA.W OAMTileNo+$FC,Y
                      LDA.B PlayerDirection
                      LSR A
                      ROR A
                      LSR A
                      EOR.W DATA_0CD5FC,X
                      STA.W OAMTileAttr+$FC,Y
                      TXA
                      ASL A
                      TAX
                      LDA.B Powerup
                      BEQ +
                      INX
                    + LDA.B PlayerYPosNext
                      CLC
                      ADC.W DATA_0CD635,X
                      STA.W OAMTileYPos+$FC,Y
                      LDA.W DATA_0CD60F,X
                      LDX.B PlayerDirection
                      BNE +
                      EOR.B #$FF
                      INC A
                    + CLC
                      ADC.B PlayerXPosNext
                      STA.W OAMTileXPos+$FC,Y
                      TYA
                      LSR A
                      LSR A
                      TAY
                      LDA.B #$02
                      STA.W OAMTileSize+$3F,Y
Return0CD6C3:         RTS


DATA_0CD6C4:          db $E7,$E6,$E6,$EA,$E6,$E7

DATA_0CD6CA:          db $F6,$F1,$F3,$F3,$F3,$F4

DATA_0CD6D0:          db $1B,$0E,$00

DATA_0CD6D3:          db $15,$08,$00

DATA_0CD6D6:          db $8E,$8E,$8C

DATA_0CD6D9:          db $FC,$04,$FC,$04,$FC,$04,$FC

DATA_0CD6E0:          db $34,$68,$34,$68,$34

DATA_0CD6E5:          db $68,$34,$B0,$00,$C0,$00,$C0

CODE_0CD6EC:          LDA.W Layer2ScrollXSpeed
                      LSR A
                      BEQ Return0CD751
                      LDY.B #$04
                      LDA.B #$02
                    - STA.W OAMTileSize+$50,Y
                      DEY
                      BPL -
                      LDA.W Layer1ScrollXSpeed
                      BNE CODE_0CD752
                      LDA.W PlayerWalkingPose
                      ASL A
                      LDY.B Powerup
                      BEQ +
                      INC A
                    + TAY
                      LDA.W DATA_0CD6C4,Y
                      STA.B _0
                      LDA.W DATA_0CD6CA,Y
                      STA.B _1
                      LDA.B PlayerDirection
                      LSR A
                      ROR A
                      LSR A
                      EOR.B #$61
                      STA.B _2
                      LDX.B #$02
CODE_0CD720:          TXA
                      ASL A
                      ASL A
                      TAY
                      LDA.B _0
                      CLC
                      ADC.W DATA_0CD6D0,X
                      BIT.B _2
                      BVC +
                      EOR.B #$FF
                      INC A
                    + CLC
                      ADC.B PlayerXPosNext
                      STA.W OAMTileXPos+$140,Y
                      LDA.B _1
                      CLC
                      ADC.W DATA_0CD6D3,X
                      CLC
                      ADC.B PlayerYPosNext
                      STA.W OAMTileYPos+$140,Y
                      LDA.W DATA_0CD6D6,X
                      STA.W OAMTileNo+$140,Y
                      LDA.B _2
                      STA.W OAMTileAttr+$140,Y
                      DEX
                      BPL CODE_0CD720
Return0CD751:         RTS

CODE_0CD752:          AND.B #$07
                      BNE +
                      JSR CODE_0CD22D
                    + LDY.B #$25
                      LDA.W Layer1ScrollXSpeed
                      CMP.B #$4C
                      BCC +
                      LDY.B #$38                                ; \ Mario's image = Ducking, back to camera
                    + STY.W PlayerPose                          ; /
                      LDY.B #$00
                      DEC A
                      STA.B _0
                      LDA.B #$AC
CODE_0CD76E:          STA.W OAMTileNo+$140,Y
                      LDA.B #$21
                      STA.W OAMTileAttr+$140,Y
                      LDA.B PlayerXPosNext
                      STA.W OAMTileXPos+$140,Y
                      LDA.B _0
                      STA.W OAMTileYPos+$140,Y
                      CLC
                      ADC.B #$10
                      CMP.B #$68
                      BCC +
                      LDA.B #$68
                    + STA.B _0
                      INY
                      INY
                      INY
                      INY
                      LDA.B #$AE
                      CPY.B #$14
                      BCC CODE_0CD76E
                      LDX.W Layer2ScrollTimer
                      LDA.W Layer1ScrollXSpeed
                      CMP.W DATA_0CD6E0,X
                      BNE CODE_0CD7DE
                      LDA.W Layer1ScrollYSpeed+1
                      BEQ +
                      DEC.W Layer1ScrollYSpeed+1
                      RTS

                    + TXA
                      BEQ CODE_0CD7C9
                      LSR A
                      BCS CODE_0CD7C9
                      LDA.B TrueFrame
                      AND.B #$04
                      BEQ +
                      LDA.B #$39                                ; \ Mario's image = Standing, back to camera
                      STA.W PlayerPose                          ; /
                    + LDA.B PlayerXPosNext
                      CMP.W DATA_0CD6E5,X
                      BEQ CODE_0CD7C9
                      INC A
                      STA.B PlayerXPosNext
                      AND.B #$0F
                      BEQ CODE_0CD7E5
                      RTS

CODE_0CD7C9:          INC.W Layer2ScrollTimer
                      CPX.B #$06
                      BCC +
                      STZ.W Layer1ScrollXSpeed
                      RTS

                    + TXA
                      LSR A
                      BCS +
                      LDA.B #!SFX_FLYHIT
                      STA.W SPCIO0                              ; / Play sound effect
                    + RTS

CODE_0CD7DE:          CLC
                      ADC.W DATA_0CD6D9,X
                      STA.W Layer1ScrollXSpeed
CODE_0CD7E5:          LDA.B #$10
                      STA.W Layer1ScrollYSpeed+1
                      RTS


DATA_0CD7EB:          db $F7,$F6

DATA_0CD7ED:          db $63,$5F,$62,$5F,$62,$5E

DATA_0CD7F3:          db $67,$66,$65,$65,$64,$64,$64,$64
                      db $64,$64,$64,$64,$65,$65,$66,$67

CODE_0CD803:          LDX.W Layer2ScrollXSpeed
                      BNE CODE_0CD83D
                      LDA.W PickUpItemTimer
                      BEQ CODE_0CD812
                      LSR A
                      BEQ CODE_0CD849
                      BRA CODE_0CD84F

CODE_0CD812:          LDY.B Powerup
                      BEQ +
                      LDY.B #$01
                    + LDA.B PlayerXPosNext
                      CLC
                      ADC.W DATA_0CD7EB,Y
                      STA.W Layer2ScrollYSpeed
                      TYA
                      LSR A
                      LDA.W PlayerWalkingPose
                      ROL A
                      TAY
                      LDA.W DATA_0CD7ED,Y
                      LDY.B PlayerXPosNext
                      CPY.B #$40
                      BCS CODE_0CD858
                      LDY.B PlayerXSpeed
                      BNE CODE_0CD858
                      LDA.B #$10
                      STA.W PickUpItemTimer
                      STZ.W IsCarryingItem
CODE_0CD83D:          LDA.B #$05
                      CMP.B PlayerXPosNext
                      BCC +
                      STA.B PlayerXPosNext
                      LDA.B PlayerXSpeed
                      BMI +
CODE_0CD849:          INC.W Layer2ScrollXSpeed
                    + INC.W Layer2ScrollXSpeed+1
CODE_0CD84F:          LDA.W Layer2ScrollXSpeed+1
                      AND.B #$0F
                      TAY
                      LDA.W DATA_0CD7F3,Y
CODE_0CD858:          STA.W OAMTileYPos+$180
                      LDA.W Layer2ScrollYSpeed
                      STA.W OAMTileXPos+$180
                      STZ.W OAMTileNo+$180
                      LDA.B #$21
                      STA.W OAMTileAttr+$180
                      LDA.B #$02
                      STA.W OAMTileSize+$60
                      RTS

                      %insert_empty($13,$91,$6A,$91,$91)

DATA_0CD900:          db $E2,$00,$00,$01,$81,$02,$00,$03
                      db $B0,$00,$03,$04,$05,$06,$07,$8B
                      db $00,$00,$08,$81,$09,$00,$0A,$8B
                      db $00,$03,$0B,$0C,$09,$0D,$8B,$00
                      db $00,$0B,$81,$09,$00,$0D,$8B,$00
                      db $08,$0B,$0E,$09,$0D,$0F,$10,$11
                      db $00,$11,$86,$00,$00,$0B,$81,$09
                      db $05,$0D,$12,$13,$14,$00,$14,$84
                      db $00,$0D,$04,$05,$15,$16,$0C,$0D
                      db $02,$17,$18,$00,$18,$01,$02,$03
                      db $81,$00,$00,$08,$81,$09,$09,$19
                      db $1A,$1B,$1C,$1D,$18,$02,$18,$02
                      db $03,$82,$00,$04,$0B,$0C,$09,$0D
                      db $1E,$81,$09,$05,$1F,$18,$00,$20
                      db $06,$07,$82,$00,$10,$0B,$09,$0E
                      db $0D,$0B,$09,$0E,$0D,$18,$00,$0C
                      db $09,$0A,$00,$04,$05,$0B,$81,$09
                      db $01,$0D,$0B,$81,$09,$02,$0D,$18
                      db $00,$81,$09,$0D,$0D,$00,$08,$09
                      db $0B,$0C,$09,$0D,$0B,$0C,$09,$0D
                      db $18,$00,$81,$09,$04,$0D,$02,$0B
                      db $09,$0B,$81,$09,$01,$0D,$0B,$81
                      db $09,$00,$0D,$81,$02,$06,$09,$0E
                      db $0D,$00,$0B,$0E,$0B,$81,$09,$08
                      db $0D,$0B,$09,$0E,$0D,$02,$03,$09
                      db $21,$88,$22,$06,$23,$09,$0D,$18
                      db $00,$16,$24,$88,$25,$0A,$26,$09
                      db $0D,$18,$02,$22,$23,$0D,$02,$0B
                      db $21,$89,$22,$05,$25,$26,$0D,$22
                      db $0B,$24,$89,$25,$FF,$00,$87,$00
                      db $00,$01,$81,$02,$00,$03,$84,$00
                      db $00,$01,$81,$02,$00,$03,$D8,$00
                      db $01,$0F,$10,$8D,$00,$02,$12,$13
                      db $03,$83,$00,$02,$0F,$10,$11,$82
                      db $00,$04,$0F,$10,$11,$27,$28,$84
                      db $00,$02,$12,$13,$14,$82,$00,$04
                      db $12,$13,$14,$29,$28,$82,$00,$00
                      db $01,$81,$02,$01,$17,$18,$82,$00
                      db $07,$29,$28,$18,$27,$2A,$0F,$10
                      db $11,$81,$00,$03,$27,$13,$18,$01
                      db $81,$02,$07,$17,$28,$18,$27,$2B
                      db $12,$13,$14,$81,$00,$02,$29,$28
                      db $18,$82,$00,$18,$29,$28,$2C,$02
                      db $0B,$02,$17,$18,$04,$05,$1C,$1D
                      db $18,$00,$01,$02,$2A,$20,$1C,$1D
                      db $0B,$29,$28,$18,$08,$81,$09,$01
                      db $1F,$18,$81,$02,$01,$03,$2B,$81
                      db $09,$15,$1F,$0B,$27,$13,$18,$0B
                      db $0C,$09,$0D,$18,$00,$2D,$22,$0B
                      db $09,$0C,$0D,$0B,$29,$2C,$02,$0B
                      db $81,$09,$09,$0D,$02,$03,$2E,$25
                      db $0B,$09,$1A,$1B,$15,$83,$22,$02
                      db $23,$0E,$0D,$83,$22,$01,$0B,$21
                      db $82,$22,$83,$25,$02,$26,$09,$0D
                      db $83,$25,$01,$0B,$24,$82,$25,$FF
                      db $FF

DATA_0CDAB9:          db $D6,$00,$02,$2F,$30,$31,$95,$00
                      db $02,$2F,$30,$31,$BE,$00,$01,$32
                      db $33,$8C,$00,$02,$34,$35,$36,$8C
                      db $00,$02,$37,$38,$32,$86,$00,$00
                      db $33,$84,$00,$81,$39,$01,$3A,$3B
                      db $81,$00,$01,$2F,$31,$81,$00,$01
                      db $36,$3C,$83,$00,$03,$3D,$34,$35
                      db $3E,$84,$00,$01,$33,$32,$81,$00
                      db $00,$3F,$81,$00,$04,$32,$40,$41
                      db $3D,$42,$83,$00,$01,$36,$39,$81
                      db $00,$00,$43,$81,$00,$00,$39,$81
                      db $00,$01,$3C,$44,$83,$00,$04,$45
                      db $41,$00,$3F,$46,$84,$00,$01,$47
                      db $3D,$86,$00,$01,$48,$49,$83,$00
                      db $02,$4A,$4B,$4C,$84,$00,$0A,$2F
                      db $31,$43,$4D,$4E,$2F,$31,$00,$4F
                      db $50,$3E,$82,$00,$00,$32,$82,$00
                      db $02,$51,$52,$3E,$82,$00,$02,$53
                      db $54,$55,$81,$00,$02,$33,$3A,$3B
                      db $81,$00,$02,$4F,$54,$3E,$82,$00
                      db $0C,$51,$56,$49,$30,$31,$36,$57
                      db $3E,$00,$33,$58,$49,$55,$82,$00
                      db $0C,$4F,$59,$5A,$3F,$3C,$34,$35
                      db $40,$00,$36,$51,$56,$49,$81,$00
                      db $0D,$3F,$58,$5B,$5C,$48,$3F,$5D
                      db $32,$39,$00,$5D,$46,$57,$4D,$81
                      db $00,$0D,$48,$34,$35,$5E,$43,$48
                      db $32,$39,$33,$00,$39,$35,$3A,$35
                      db $81,$00,$00,$43,$81,$45,$0A,$39
                      db $3D,$43,$41,$40,$36,$3C,$39,$41
                      db $39,$5D,$E5,$00,$02,$2F,$30,$31
                      db $D6,$00,$02,$2F,$30,$31,$93,$00
                      db $00,$32,$8D,$00,$07,$2F,$5E,$31
                      db $3C,$00,$2F,$30,$31,$81,$00,$01
                      db $2F,$31,$82,$00,$04,$33,$34,$35
                      db $2F,$31,$89,$00,$04,$3C,$36,$5F
                      db $3A,$3B,$8B,$00,$03,$45,$39,$38
                      db $41,$82,$00,$01,$2F,$31,$86,$00
                      db $02,$33,$5D,$45,$82,$00,$00,$33
                      db $86,$00,$02,$32,$00,$36,$83,$00
                      db $01,$33,$36,$86,$00,$02,$3A,$3B
                      db $39,$83,$00,$01,$36,$32,$81,$00
                      db $07,$33,$00,$3F,$00,$34,$35,$41
                      db $3F,$81,$00,$03,$46,$00,$34,$35
                      db $81,$00,$26,$36,$00,$43,$00,$5F
                      db $39,$3D,$43,$00,$4A,$4B,$4C,$45
                      db $39,$00,$33,$32,$00,$45,$00,$41
                      db $3A,$3B,$45,$00,$53,$54,$3E,$40
                      db $41,$3C,$36,$39,$00,$4E,$3F,$33
                      db $40,$41,$81,$00,$28,$4A,$4B,$4C
                      db $3F,$47,$3F,$00,$41,$00,$3D,$48
                      db $36,$3C,$33,$00,$3F,$4F,$50,$54
                      db $43,$49,$48,$33,$00,$33,$41,$43
                      db $39,$00,$36,$00,$43,$53,$54,$49
                      db $51,$5A,$43,$36,$00,$36,$FF,$FF
DATA_0CDC71:          db $A1,$00,$01,$60,$61,$F5,$00,$01
                      db $60,$61,$9A,$00,$01,$60,$61,$DA
                      db $00,$01,$60,$61,$BE,$00,$00,$62
                      db $88,$00,$00,$63,$83,$00,$02,$64
                      db $65,$66,$82,$00,$00,$67,$82,$00
                      db $01,$63,$68,$82,$00,$0C,$64,$69
                      db $6A,$6B,$66,$6C,$6D,$6E,$6F,$70
                      db $71,$68,$22,$81,$00,$01,$64,$69
                      db $81,$72,$0C,$73,$6B,$74,$75,$76
                      db $77,$78,$79,$7A,$22,$00,$64,$69
                      db $81,$72,$00,$6A,$81,$72,$01,$6B
                      db $74,$82,$76,$06,$77,$78,$79,$64
                      db $69,$6A,$73,$81,$72,$09,$73,$6A
                      db $73,$6B,$74,$75,$76,$75,$76,$77
                      db $E8,$00,$01,$60,$61,$E7,$00,$01
                      db $60,$61,$BE,$00,$01,$63,$7B,$84
                      db $00,$01,$60,$61,$85,$00,$03,$63
                      db $68,$7C,$7B,$89,$00,$02,$7D,$7E
                      db $68,$81,$22,$01,$7C,$7B,$87,$00
                      db $01,$63,$68,$84,$22,$02,$7C,$7F
                      db $80,$85,$00,$00,$68,$87,$22,$01
                      db $7C,$7B,$84,$00,$89,$22,$01,$7C
                      db $7B,$83,$00,$8A,$22,$01,$7C,$7B
                      db $82,$00,$8B,$22,$01,$7C,$7B,$81
                      db $00,$00,$7A,$8B,$22,$05,$7C,$7B
                      db $00,$78,$79,$7A,$8A,$22,$01,$7C
                      db $7B,$FF,$FF

DATA_0CDD44:          db $DA,$00,$01,$60,$61,$93,$00,$01
                      db $60,$61,$B4,$00,$02,$7D,$7E,$7B
                      db $8B,$00,$04,$63,$68,$22,$81,$7B
                      db $89,$00,$01,$63,$68,$82,$22,$01
                      db $7C,$7B,$87,$00,$01,$63,$68,$84
                      db $22,$02,$7C,$7F,$80,$83,$00,$02
                      db $7D,$7E,$68,$87,$22,$01,$7C,$7F
                      db $81,$00,$01,$63,$68,$8A,$22,$03
                      db $82,$00,$63,$68,$8A,$22,$03,$83
                      db $68,$63,$68,$8A,$22,$03,$83,$68
                      db $22,$68,$87,$22,$04,$83,$84,$85
                      db $81,$68,$81,$22,$00,$86,$86,$22
                      db $04,$83,$68,$22,$7C,$86,$82,$22
                      db $03,$7C,$86,$82,$85,$82,$22,$01
                      db $83,$68,$82,$22,$01,$7C,$87,$82
                      db $22,$06,$7C,$68,$7C,$87,$82,$88
                      db $68,$8B,$22,$01,$83,$68,$8C,$22
                      db $01,$83,$68,$84,$22,$01,$82,$85
                      db $85,$22,$01,$81,$68,$84,$22,$01
                      db $81,$68,$A4,$22,$EB,$00,$01,$60
                      db $61,$97,$00,$01,$60,$61,$C9,$00
                      db $01,$63,$7B,$8B,$00,$04,$80,$63
                      db $68,$7C,$7B,$8A,$00,$01,$88,$68
                      db $81,$22,$01,$83,$7B,$86,$00,$02
                      db $60,$61,$00,$82,$22,$03,$83,$68
                      db $7C,$7B,$88,$00,$81,$22,$01,$81
                      db $68,$81,$22,$04,$7C,$7F,$80,$63
                      db $7B,$84,$00,$87,$22,$07,$83,$68
                      db $7C,$7B,$7D,$80,$7D,$7E,$86,$22
                      db $01,$81,$68,$81,$22,$04,$7C,$86
                      db $7C,$86,$83,$8B,$22,$06,$7C,$86
                      db $7C,$68,$22,$83,$86,$89,$22,$06
                      db $7C,$87,$22,$81,$68,$7C,$86,$8E
                      db $22,$01,$7C,$84,$BA,$22,$FF,$FF
DATA_0CDE54:          db $A2,$00,$01,$60,$61,$CD,$00,$01
                      db $60,$61,$F8,$00,$01,$60,$61,$8A
                      db $00,$01,$60,$61,$A9,$00,$00,$67
                      db $8C,$00,$05,$6C,$6D,$6E,$6F,$70
                      db $67,$87,$00,$09,$6C,$6D,$89,$76
                      db $77,$75,$78,$6E,$6F,$70,$83,$00
                      db $04,$6C,$6D,$89,$77,$8A,$83,$76
                      db $0D,$77,$76,$78,$6F,$70,$8B,$6D
                      db $89,$75,$77,$8C,$65,$74,$77,$82
                      db $76,$04,$75,$77,$8D,$8E,$8F,$81
                      db $77,$0B,$76,$8C,$69,$6A,$6B,$74
                      db $76,$75,$76,$8D,$8E,$8F,$81,$72
                      db $0C,$76,$77,$8C,$69,$6A,$72,$6A
                      db $6B,$74,$8D,$8E,$8F,$6A,$82,$72
                      db $02,$90,$8C,$69,$83,$72,$03,$73
                      db $6B,$91,$73,$83,$72,$06,$6A,$8C
                      db $69,$73,$6A,$72,$6A,$82,$72,$01
                      db $6B,$91,$81,$6A,$02,$72,$92,$93
                      db $B2,$00,$01,$60,$61,$C8,$00,$01
                      db $60,$61,$95,$00,$01,$60,$61,$EB
                      db $00,$01,$60,$61,$A5,$00,$01,$60
                      db $61,$92,$00,$00,$94,$8D,$00,$03
                      db $95,$65,$96,$97,$83,$00,$00,$67
                      db $86,$00,$0A,$72,$6A,$73,$98,$96
                      db $97,$6C,$6D,$6E,$6F,$70,$83,$00
                      db $01,$6C,$6A,$82,$72,$03,$92,$93
                      db $89,$77,$81,$76,$05,$78,$6F,$70
                      db $6C,$6D,$89,$81,$72,$02,$92,$93
                      db $89,$84,$76,$09,$75,$77,$78,$99
                      db $90,$77,$92,$93,$89,$77,$84,$76
                      db $00,$75,$83,$76,$04,$78,$99,$89
                      db $76,$77,$82,$76,$01,$77,$75,$86
                      db $76,$00,$77,$FF,$FF

DATA_0CDF59:          db $BB,$00,$01,$60,$61,$83,$00,$01
                      db $60,$61,$D4,$00,$01,$60,$61,$B5
                      db $00,$01,$60,$61,$9B,$00,$00,$7D
                      db $8D,$00,$01,$63,$68,$84,$00,$02
                      db $9A,$9B,$9C,$83,$00,$0C,$7D,$7E
                      db $68,$22,$9D,$9A,$9B,$9C,$9A,$9E
                      db $9F,$09,$9C,$81,$00,$01,$71,$68
                      db $82,$22,$0B,$A0,$9E,$A1,$09,$A2
                      db $9F,$A3,$A4,$A5,$9A,$A6,$A7,$83
                      db $22,$01,$A8,$9F,$81,$09,$01,$A9
                      db $A3,$81,$72,$03,$AA,$AB,$9F,$A9
                      db $82,$22,$05,$A0,$9F,$A3,$AC,$AD
                      db $AE,$82,$72,$08,$AF,$AD,$B0,$A6
                      db $A7,$A0,$9B,$A2,$A3,$82,$72,$00
                      db $B1,$81,$72,$56,$AF,$A3,$B2,$9E
                      db $9F,$B3,$B4,$A1,$09,$B5,$72,$B2
                      db $9B,$B6,$B2,$9B,$B3,$B2,$9E,$9F
                      db $09,$A3,$B2,$A1,$09,$B6,$B5,$9F
                      db $A3,$A5,$B7,$AC,$09,$A2,$9B,$09
                      db $A3,$AA,$9E,$B8,$A9,$A4,$AA,$AE
                      db $72,$B9,$BA,$72,$AC,$09,$B3,$A9
                      db $72,$B2,$9E,$9F,$09,$72,$BB,$B7
                      db $B2,$B8,$09,$B6,$B1,$A4,$AF,$A3
                      db $B2,$BC,$9B,$BD,$B3,$AA,$BE,$B2
                      db $9E,$B7,$A4,$A5,$72,$AA,$A3,$B2
                      db $BF,$C0,$B8,$81,$09,$D6,$00,$01
                      db $60,$61,$A8,$00,$01,$60,$61,$A8
                      db $00,$01,$60,$61,$95,$00,$01,$60
                      db $61,$98,$00,$00,$80,$8E,$00,$01
                      db $7C,$7B,$84,$00,$02,$9A,$9F,$9C
                      db $85,$00,$09,$22,$7C,$7F,$7E,$7F
                      db $80,$9A,$9F,$A3,$A5,$81,$00,$01
                      db $7D,$80,$81,$00,$84,$22,$4A,$7C
                      db $A8,$9F,$C1,$AA,$00,$63,$68,$7C
                      db $7F,$7E,$22,$A0,$9B,$C2,$22,$A0
                      db $9F,$A3,$BA,$C3,$71,$68,$A0,$9F
                      db $A7,$22,$C4,$9F,$A3,$AA,$C4,$9F
                      db $A3,$B2,$9E,$9F,$A7,$22,$A8,$A1
                      db $09,$A7,$9E,$A1,$C1,$AA,$BD,$AE
                      db $B2,$9F,$BD,$A3,$C5,$A0,$9F,$A3
                      db $A4,$B2,$A2,$A1,$C1,$AF,$A3,$B2
                      db $9F,$09,$A3,$B2,$9B,$BE,$AC,$C1
                      db $B2,$9F,$81,$A3,$02,$AA,$B0,$9B
                      db $81,$BD,$20,$B3,$C1,$A8,$A1,$A3
                      db $AA,$B2,$BC,$A6,$72,$B5,$B2,$9E
                      db $9F,$09,$A3,$A5,$B2,$9E,$B8,$B6
                      db $72,$C0,$A2,$9F,$B6,$72,$A8,$9F
                      db $09,$A3,$72,$AA,$81,$9E,$04,$9F
                      db $A9,$72,$AA,$09,$81,$A3,$11,$B2
                      db $9E,$A1,$09,$BE,$72,$AA,$9E,$9F
                      db $09,$A3,$72,$AA,$AE,$72,$BD,$A2
                      db $9F,$81,$09,$05,$A3,$72,$AF,$BD
                      db $09,$A9,$81,$72,$02,$B2,$B7,$72
                      db $FF,$FF

DATA_0CE103:          db $7F,$C6,$9E,$C7,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $C9,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$CA,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$CA,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $CB,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $CB,$CC,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C7,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$C9,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$CA,$9E,$C8,$9E,$C6,$9E,$C8
                      db $C9,$80,$C6,$7F,$9E,$C8,$9E,$CA
                      db $9E,$C8,$9E,$C6,$C9,$C8,$9E,$C6
                      db $9E,$C8,$9E,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C7,$9E,$C6,$9E,$C8,$9E,$C6
                      db $C9,$C8,$9E,$C6,$9E,$CD,$CE,$C6
                      db $9E,$C8,$9E,$C6,$9E,$C8,$CB,$CC
                      db $9E,$C8,$9E,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C8,$9E,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C8,$9E,$CA,$9E,$C8,$9E,$C6
                      db $9E,$C8,$9E,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C8,$C9,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C8,$C9,$C6,$9E,$C8,$9E,$CA
                      db $9E,$C8,$9E,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C8,$9E,$C6,$9E,$C8,$9E,$CA
                      db $9E,$C8,$9E,$C6,$CF,$C8,$9E,$C6
                      db $CF,$C8,$C9,$C6,$CF,$C8,$C9,$C6
                      db $CF,$C8,$9E,$C6,$80,$D0,$7F,$C8
                      db $9E,$CA,$D0,$C8,$9E,$C6,$D0,$C8
                      db $9E,$C6,$D0,$C7,$9E,$D1,$D2,$D3
                      db $D4,$D1,$D2,$D3,$D4,$D1,$D2,$D3
                      db $D4,$D1,$D2,$D3,$D4,$D5,$D6,$D7
                      db $D8,$D5,$D6,$D7,$D8,$D5,$D6,$D7
                      db $D8,$D5,$D6,$D7,$D8,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$80
                      db $00,$7F,$DA,$00,$D9,$00,$DA,$00
                      db $D9,$00,$DA,$00,$D9,$00,$DA,$00
                      db $D9,$00,$DA,$00,$D9,$00,$DA,$00
                      db $D9,$00,$DA,$00,$D9,$00,$DA,$00
                      db $D9,$00,$DA,$00,$D9,$00,$DA,$00
                      db $D9,$00,$DA,$00,$D9,$00,$DA,$C6
                      db $9E,$C8,$9E,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C8,$9E,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C7,$9E,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C8,$9E,$C6,$9E,$C8,$C9,$C6
                      db $9E,$C8,$9E,$C6,$9E,$C7,$9E,$C6
                      db $9E,$C8,$9E,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C8,$9E,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C8,$C9,$C6,$9E,$C8,$9E,$CC
                      db $9E,$C8,$9E,$CA,$9E,$C8,$CB,$CC
                      db $9E,$C8,$9E,$C6,$C9,$C8,$9E,$C6
                      db $9E,$C8,$80,$9E,$7F,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C7
                      db $9E,$C6,$C9,$C8,$9E,$C6,$9E,$C8
                      db $C9,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$C9,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$CA,$9E,$C8
                      db $9E,$C6,$9E,$C7,$9E,$C6,$9E,$C8
                      db $9E,$CA,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$C9,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C7
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$9E,$C6,$9E,$C8
                      db $9E,$C6,$9E,$C8,$C9,$80,$C6,$7F
                      db $9E,$C8,$9E,$C6,$9E,$C8,$9E,$CA
                      db $9E,$C8,$9E,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C8,$C9,$C6,$9E,$C8,$9E,$C6
                      db $9E,$C8,$9E,$C6,$CF,$C8,$9E,$C6
                      db $CF,$C8,$9E,$C6,$CF,$C8,$C9,$C6
                      db $CF,$C8,$9E,$C6,$D0,$CD,$CE,$C6
                      db $D0,$C8,$C9,$C6,$D0,$C8,$9E,$C6
                      db $D0,$C8,$C9,$D1,$D2,$D3,$D4,$D1
                      db $D2,$D3,$D4,$D1,$D2,$D3,$D4,$D1
                      db $D2,$D3,$D4,$D5,$D6,$D7,$D8,$D5
                      db $D6,$D7,$D8,$D5,$D6,$D7,$D8,$D5
                      db $D6,$D7,$D8,$00,$D9,$00,$DA,$00
                      db $D9,$00,$DA,$00,$D9,$00,$DA,$00
                      db $D9,$00,$DA,$00,$D9,$00,$DA,$00
                      db $D9,$00,$DA,$00,$D9,$00,$DA,$00
                      db $D9,$00,$DA,$00,$D9,$00,$DA,$00
                      db $80,$D9,$59,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$00,$D9,$00
                      db $DA,$00,$D9,$00,$DA,$FF,$FF

DATA_0CE472:          db $81,$00,$01,$60,$61,$C4,$00,$01
                      db $60,$61,$A1,$00,$02,$63,$7F,$80
                      db $8B,$00,$03,$63,$68,$22,$7C,$89
                      db $00,$02,$7D,$7E,$68,$82,$22,$88
                      db $00,$01,$63,$68,$84,$22,$87,$00
                      db $01,$63,$68,$85,$22,$86,$00,$01
                      db $63,$68,$86,$22,$83,$00,$03,$63
                      db $7B,$71,$68,$87,$22,$82,$00,$03
                      db $63,$68,$7C,$86,$88,$22,$03,$DB
                      db $7D,$7E,$68,$81,$22,$02,$7C,$84
                      db $85,$81,$22,$00,$DC,$83,$22,$01
                      db $DD,$DE,$87,$22,$02,$DF,$65,$E0
                      db $81,$22,$03,$E1,$76,$DD,$DE,$85
                      db $22,$06,$DF,$69,$6A,$6B,$E0,$E1
                      db $E2,$81,$76,$01,$DD,$DE,$83,$22
                      db $01,$DF,$69,$81,$72,$02,$6A,$6B
                      db $74,$82,$76,$02,$77,$DD,$DE,$81
                      db $22,$02,$DF,$69,$6A,$83,$72,$02
                      db $6B,$74,$77,$82,$76,$05,$DD,$DE
                      db $DF,$69,$73,$6A,$84,$72,$00,$6B
                      db $84,$76,$01,$8C,$69,$87,$72,$06
                      db $6A,$90,$76,$77,$76,$8C,$69,$83
                      db $72,$00,$73,$84,$72,$04,$78,$99
                      db $90,$8C,$69,$81,$72,$00,$6A,$84
                      db $72,$00,$73,$81,$72,$03,$77,$76
                      db $8C,$69,$88,$72,$07,$6A,$73,$E3
                      db $76,$8C,$69,$72,$73,$87,$72,$05
                      db $E3,$E4,$8F,$8C,$69,$73,$86,$72
                      db $03,$73,$E3,$E4,$8F,$81,$72,$00
                      db $69,$81,$73,$02,$6A,$72,$6A,$82
                      db $72,$03,$E3,$E4,$8F,$73,$82,$72
                      db $A7,$00,$01,$60,$61,$BD,$00,$00
                      db $E5,$86,$00,$01,$7F,$E6,$84,$00
                      db $02,$E7,$E8,$DB,$85,$00,$02,$22
                      db $E9,$80,$82,$00,$04,$E7,$E2,$76
                      db $DD,$DB,$84,$00,$81,$22,$04,$7C
                      db $7F,$80,$E7,$E2,$82,$76,$01,$DD
                      db $DB,$83,$00,$83,$22,$03,$E1,$E2
                      db $76,$77,$82,$76,$01,$DD,$DB,$82
                      db $00,$82,$22,$01,$E1,$E2,$81,$77
                      db $83,$76,$02,$75,$DD,$DB,$81,$00
                      db $81,$22,$01,$E1,$E2,$88,$76,$05
                      db $DD,$DB,$00,$22,$E1,$E2,$8A,$76
                      db $03,$DD,$DB,$E1,$E2,$86,$76,$00
                      db $75,$82,$76,$03,$75,$76,$DD,$E2
                      db $8D,$76,$00,$77,$83,$76,$00,$77
                      db $84,$76,$00,$77,$84,$76,$01,$75
                      db $77,$85,$76,$00,$EA,$8C,$76,$04
                      db $EB,$EC,$6E,$99,$90,$82,$76,$08
                      db $75,$76,$74,$76,$75,$76,$EB,$EC
                      db $89,$82,$76,$02,$78,$99,$90,$82
                      db $76,$05,$6B,$74,$EB,$EC,$89,$77
                      db $81,$76,$01,$77,$75,$81,$76,$07
                      db $78,$99,$90,$76,$72,$6B,$74,$ED
                      db $81,$76,$01,$75,$77,$84,$76,$08
                      db $77,$78,$99,$72,$E3,$E4,$EE,$EF
                      db $F0,$88,$76,$02,$77,$E4,$8F,$81
                      db $72,$05,$6A,$98,$EF,$F0,$76,$77
                      db $83,$76,$02,$75,$76,$73,$85,$72
                      db $02,$98,$EF,$F0,$81,$76,$00,$75
                      db $82,$76,$03,$72,$6A,$72,$73,$82
                      db $72,$05,$6A,$72,$98,$EF,$F0,$77
                      db $82,$76,$83,$72,$00,$73,$83,$72
                      db $06,$6A,$72,$98,$EF,$F0,$76,$8C
                      db $FF,$FF

DATA_0CE674:          db $FF,$F1,$FF,$F1,$FF,$F1,$FF,$F1
                      db $FF,$F1,$FF,$F1,$DF,$F1,$FF,$FF
DATA_0CE684:          db $8D,$00,$00,$F2,$83,$00,$02,$F3
                      db $00,$F3,$95,$00,$00,$F4,$83,$00
                      db $00,$F4,$87,$00,$00,$F2,$89,$00
                      db $00,$F2,$81,$00,$00,$F5,$93,$00
                      db $00,$F3,$88,$00,$00,$F2,$89,$00
                      db $00,$F5,$88,$00,$00,$F2,$82,$00
                      db $00,$F2,$92,$00,$00,$F2,$82,$00
                      db $00,$F4,$89,$00,$00,$F3,$88,$00
                      db $00,$F5,$8F,$00,$00,$F2,$83,$00
                      db $02,$F3,$00,$F3,$95,$00,$00,$F4
                      db $83,$00,$00,$F4,$87,$00,$00,$F2
                      db $89,$00,$00,$F2,$81,$00,$00,$F5
                      db $93,$00,$00,$F3,$88,$00,$00,$F2
                      db $89,$00,$00,$F5,$88,$00,$00,$F2
                      db $82,$00,$00,$F2,$92,$00,$00,$F2
                      db $82,$00,$00,$F4,$89,$00,$00,$F3
                      db $88,$00,$00,$F5,$8C,$00,$00,$F3
                      db $83,$00,$0D,$F6,$F7,$F8,$F6,$F7
                      db $F8,$F6,$F7,$F8,$F6,$F7,$F8,$F6
                      db $F7,$81,$F8,$AF,$72,$8D,$00,$00
                      db $F2,$83,$00,$02,$F3,$00,$F3,$95
                      db $00,$00,$F4,$83,$00,$00,$F4,$87
                      db $00,$00,$F2,$89,$00,$00,$F2,$81
                      db $00,$00,$F5,$93,$00,$00,$F3,$88
                      db $00,$00,$F2,$89,$00,$00,$F5,$88
                      db $00,$00,$F2,$82,$00,$00,$F2,$92
                      db $00,$00,$F2,$82,$00,$00,$F4,$89
                      db $00,$00,$F3,$88,$00,$00,$F5,$8F
                      db $00,$00,$F2,$83,$00,$02,$F3,$00
                      db $F3,$95,$00,$00,$F4,$83,$00,$00
                      db $F4,$87,$00,$00,$F2,$89,$00,$00
                      db $F2,$81,$00,$00,$F5,$93,$00,$00
                      db $F3,$88,$00,$00,$F2,$89,$00,$00
                      db $F5,$88,$00,$00,$F2,$82,$00,$00
                      db $F2,$92,$00,$00,$F2,$82,$00,$00
                      db $F4,$89,$00,$00,$F3,$88,$00,$00
                      db $F5,$8C,$00,$00,$F3,$83,$00,$0D
                      db $F6,$F7,$F8,$F6,$F7,$F8,$F6,$F7
                      db $F8,$F6,$F7,$F8,$F6,$F7,$81,$F8
                      db $AF,$72,$FF,$FF

DATA_0CE7C0:          db $9A,$00,$01,$60,$61,$C9,$00,$01
                      db $60,$61,$EE,$00,$01,$9A,$9C,$87
                      db $00,$01,$60,$61,$83,$00,$02,$A8
                      db $A1,$9C,$8B,$00,$03,$9A,$9E,$A1
                      db $A3,$82,$00,$01,$60,$61,$85,$00
                      db $05,$9A,$9E,$9F,$09,$AA,$9C,$89
                      db $00,$05,$A8,$9E,$A1,$A3,$AF,$A9
                      db $88,$00,$06,$9A,$9E,$9F,$A3,$AF
                      db $09,$A3,$88,$00,$06,$A8,$9E,$A1
                      db $AF,$B8,$A3,$AA,$88,$00,$07,$A8
                      db $9F,$B0,$A3,$09,$B6,$AF,$9C,$81
                      db $00,$00,$9A,$84,$00,$0A,$9F,$B0
                      db $9F,$72,$09,$B0,$A1,$A9,$00,$9A
                      db $9F,$83,$00,$00,$9A,$81,$09,$08
                      db $A3,$AF,$B0,$9E,$A1,$A3,$9A,$9E
                      db $A1,$82,$00,$09,$9A,$9E,$09,$A3
                      db $AF,$A9,$9E,$9F,$C1,$AA,$81,$9E
                      db $00,$A1,$81,$00,$00,$9A,$81,$9E
                      db $0A,$B0,$A1,$AC,$A9,$9F,$09,$C1
                      db $AF,$A2,$9F,$09,$81,$00,$01,$A8
                      db $9F,$81,$9E,$07,$A1,$72,$A3,$AF
                      db $A3,$AF,$09,$A3,$81,$09,$06,$00
                      db $9A,$9F,$A3,$9E,$9F,$A3,$83,$72
                      db $04,$09,$A3,$72,$AC,$B0,$CC,$00
                      db $01,$60,$61,$A3,$00,$01,$60,$61
                      db $FF,$00,$95,$00,$01,$60,$61,$83
                      db $00,$01,$9A,$9C,$8C,$00,$02,$9A
                      db $9F,$A3,$8C,$00,$02,$A8,$A1,$AA
                      db $8C,$00,$03,$9F,$09,$AC,$9C,$8B
                      db $00,$04,$09,$B0,$B6,$AC,$9C,$8A
                      db $00,$01,$B0,$9F,$81,$72,$03,$AA
                      db $9A,$9B,$9C,$87,$00,$01,$9F,$A3
                      db $81,$72,$03,$AA,$9E,$B8,$A3,$87
                      db $00,$07,$09,$BB,$09,$B6,$AF,$C0
                      db $9E,$AA,$87,$00,$08,$B0,$B7,$72
                      db $AF,$A3,$72,$C0,$AF,$9C,$86,$00
                      db $08,$B7,$72,$AF,$A3,$AF,$C1,$AA
                      db $B8,$A3,$87,$00,$FF,$FF

DATA_0CE8EE:          db $FF,$00,$FF,$00,$FF,$00,$FF,$00
                      db $FF,$00,$FF,$00,$E0,$00,$FF,$FF
DATA_0CE8FE:          db $0C,$00,$01,$02,$03,$04,$05,$06
                      db $07,$08,$02,$09,$0A,$0B,$81,$01
                      db $0D,$0C,$0D,$01,$0E,$02,$0F,$10
                      db $06,$07,$11,$03,$12,$13,$14,$81
                      db $01,$13,$0C,$0D,$15,$0C,$03,$12
                      db $09,$06,$07,$08,$03,$16,$17,$18
                      db $01,$15,$00,$19,$1A,$00,$81,$03
                      db $08,$1B,$06,$1C,$11,$02,$01,$05
                      db $1D,$01,$81,$0D,$1E,$1E,$1F,$0D
                      db $02,$20,$05,$06,$21,$08,$0E,$01
                      db $0B,$22,$15,$23,$19,$21,$1F,$22
                      db $03,$0F,$10,$06,$21,$24,$0C,$09
                      db $10,$0D,$23,$25,$81,$1E,$0D,$1F
                      db $0D,$0E,$26,$01,$06,$1E,$1F,$00
                      db $27,$01,$0D,$28,$1E,$81,$21,$0C
                      db $1F,$0D,$0C,$12,$09,$29,$2A,$2B
                      db $0D,$0E,$01,$22,$28,$82,$21,$06
                      db $1F,$22,$00,$02,$05,$18,$22,$81
                      db $0D,$0C,$0C,$01,$0D,$2C,$1E,$2D
                      db $1E,$2E,$23,$19,$03,$06,$18,$82
                      db $0D,$0C,$00,$01,$22,$2F,$21,$1F
                      db $21,$30,$31,$21,$02,$06,$1D,$82
                      db $0D,$05,$22,$15,$23,$32,$1E,$1F
                      db $81,$21,$06,$07,$1E,$03,$06,$19
                      db $1A,$22,$82,$0D,$00,$28,$81,$21
                      db $00,$1F,$81,$21,$05,$1C,$21,$03
                      db $0B,$21,$1F,$82,$0D,$01,$22,$28
                      db $81,$21,$00,$33,$82,$21,$09,$2D
                      db $0E,$06,$1E,$2E,$0D,$23,$19,$34
                      db $28,$81,$21,$0E,$1E,$21,$35,$21
                      db $1F,$0C,$06,$21,$08,$0D,$28,$21
                      db $01,$36,$37,$81,$21,$7F,$38,$39
                      db $1E,$1F,$0C,$0B,$21,$30,$1A,$2C
                      db $1E,$01,$3A,$2F,$1E,$2D,$11,$36
                      db $2A,$2B,$3B,$06,$1E,$35,$3C,$3D
                      db $21,$01,$18,$3E,$21,$1F,$3F,$22
                      db $34,$40,$02,$06,$2A,$39,$1C,$21
                      db $41,$01,$42,$36,$2A,$2B,$1F,$40
                      db $01,$0C,$03,$43,$3A,$28,$21,$35
                      db $08,$01,$0B,$19,$1A,$22,$1F,$44
                      db $01,$44,$02,$45,$18,$28,$1E,$07
                      db $08,$01,$06,$1E,$1F,$0D,$33,$02
                      db $01,$03,$46,$01,$18,$28,$21,$07
                      db $30,$01,$06,$2A,$2B,$40,$21,$03
                      db $01,$03,$47,$01,$42,$36,$2A,$07
                      db $1E,$01,$0B,$40,$3A,$0C,$1E,$03
                      db $01,$46,$04,$01,$43,$3A,$34,$48
                      db $37,$01,$49,$3B,$18,$44,$80,$2A
                      db $12,$02,$4A,$4B,$4C,$4D,$4B,$18
                      db $01,$2B,$3E,$4D,$4B,$46,$42,$03
                      db $34,$46,$49,$4E,$81,$46,$0F,$4E
                      db $4F,$01,$22,$40,$50,$4E,$45,$49
                      db $02,$4D,$4B,$45,$49,$26,$45,$81
                      db $49,$0D,$4D,$3A,$0C,$45,$43,$4A
                      db $26,$03,$46,$13,$01,$26,$47,$4A
                      db $81,$26,$31,$02,$4F,$0C,$4D,$4B
                      db $49,$26,$02,$04,$09,$0A,$04,$0F
                      db $01,$12,$26,$05,$01,$0B,$06,$21
                      db $07,$21,$01,$0F,$27,$13,$0F,$27
                      db $09,$20,$05,$06,$01,$06,$0B,$21
                      db $07,$38,$15,$27,$16,$13,$51,$02
                      db $05,$04,$0B,$06,$09,$81,$06,$7F
                      db $21,$1C,$11,$0D,$03,$01,$13,$06
                      db $03,$06,$0F,$10,$0B,$12,$06,$0B
                      db $52,$21,$24,$23,$03,$01,$17,$29
                      db $16,$0B,$27,$01,$29,$03,$0B,$06
                      db $36,$2A,$2B,$28,$03,$01,$51,$1D
                      db $01,$29,$0E,$09,$18,$53,$49,$06
                      db $23,$19,$1A,$28,$03,$09,$06,$0D
                      db $01,$1D,$00,$27,$18,$0C,$26,$14
                      db $28,$21,$1F,$28,$02,$12,$06,$22
                      db $15,$0D,$22,$16,$1D,$0C,$26,$18
                      db $2C,$21,$1F,$28,$53,$02,$06,$0D
                      db $23,$19,$1A,$01,$22,$00,$27,$18
                      db $54,$1E,$55,$2C,$0C,$03,$14,$0D
                      db $28,$1E,$1F,$15,$23,$19,$03,$18
                      db $54,$21,$24,$2F,$0C,$02,$18,$0D
                      db $2C,$21,$1F,$23,$25,$21,$16,$1D
                      db $80,$54,$0A,$41,$2B,$2F,$00,$03
                      db $18,$0D,$2F,$1E,$33,$31,$81,$21
                      db $0A,$01,$55,$2F,$08,$23,$32,$1A
                      db $16,$1D,$22,$3E,$81,$21,$00,$07
                      db $81,$21,$07,$01,$24,$54,$08,$28
                      db $21,$1F,$01,$81,$0D,$00,$28,$81
                      db $1E,$0A,$1C,$21,$2D,$01,$3A,$54
                      db $08,$28,$21,$1F,$15,$81,$0D,$01
                      db $36,$52,$81,$21,$08,$2D,$2B,$01
                      db $18,$3E,$24,$28,$21,$2B,$81,$22
                      db $39,$0D,$34,$36,$52,$21,$2E,$40
                      db $4A,$18,$23,$19,$31,$21,$22,$34
                      db $0D,$2C,$01,$55,$28,$1E,$08,$0C
                      db $49,$18,$28,$21,$07,$21,$40,$01
                      db $0D,$54,$01,$24,$28,$21,$11,$0C
                      db $45,$4F,$28,$21,$1C,$21,$0C,$4D
                      db $3A,$3E,$4D,$3A,$2C,$21,$3F,$44
                      db $01,$0B,$28,$82,$21,$16,$3B,$46
                      db $18,$40,$02,$42,$2F,$1E,$33,$02
                      db $01,$06,$36,$52,$21,$56,$46,$45
                      db $18,$0C,$03,$06,$54,$81,$21,$2F
                      db $02,$01,$49,$3A,$28,$21,$07,$45
                      db $01,$4F,$0C,$02,$06,$57,$35,$1E
                      db $03,$4D,$4B,$4F,$25,$41,$07,$4D
                      db $01,$49,$3B,$03,$0B,$58,$07,$37
                      db $03,$02,$13,$0B,$1E,$11,$07,$46
                      db $4D,$4B,$50,$03,$06,$36,$39,$3E
                      db $81,$03,$0B,$4E,$06,$21,$08,$48
                      db $47,$50,$4E,$47,$02,$06,$22,$81
                      db $36,$1F,$03,$02,$49,$0B,$1E,$3F
                      db $2B,$04,$26,$49,$4C,$46,$06,$40
                      db $3A,$34,$02,$03,$26,$06,$2A,$2B
                      db $40,$04,$26,$47,$46,$45,$06,$44
                      db $42,$01,$81,$46,$04,$26,$0B,$3A
                      db $34,$0C,$FF,$FF

DATA_0CEC82:          db $86,$59,$01,$5A,$5B,$87,$59,$01
                      db $5A,$5B,$94,$59,$04,$5C,$5D,$59
                      db $5A,$5B,$87,$59,$05,$5C,$5D,$5C
                      db $5E,$5F,$5D,$88,$59,$03,$5C,$5E
                      db $5F,$5E,$81,$60,$03,$5F,$5D,$5C
                      db $5D,$82,$59,$06,$5C,$5D,$5C,$5E
                      db $60,$61,$62,$81,$60,$04,$63,$5F
                      db $5E,$5F,$5D,$81,$5C,$07,$5E,$5F
                      db $5E,$60,$61,$64,$65,$62,$84,$60
                      db $00,$5F,$81,$5E,$01,$66,$63,$81
                      db $60,$04,$5F,$5D,$5C,$5E,$66,$86
                      db $60,$01,$67,$68,$82,$60,$01,$5F
                      db $5E,$87,$60,$05,$67,$69,$6A,$68
                      db $67,$68,$81,$60,$01,$67,$68,$83
                      db $60,$14,$67,$68,$69,$6B,$6C,$6A
                      db $69,$6A,$68,$67,$69,$6A,$68,$67
                      db $68,$67,$69,$6A,$6B,$6C,$6D,$82
                      db $6B,$01,$6A,$69,$81,$6B,$03,$6A
                      db $69,$6A,$69,$81,$6B,$00,$6E,$82
                      db $6B,$04,$6F,$6E,$6B,$6C,$6D,$81
                      db $6B,$20,$70,$71,$70,$71,$6B,$72
                      db $6E,$6B,$6F,$73,$72,$6E,$6B,$6C
                      db $6B,$70,$74,$75,$74,$75,$71,$67
                      db $69,$6C,$6A,$68,$60,$72,$6E,$6B
                      db $70,$74,$76,$82,$77,$01,$75,$69
                      db $81,$6B,$07,$6F,$73,$60,$67,$69
                      db $6B,$78,$79,$82,$77,$01,$7A,$79
                      db $82,$6B,$03,$6A,$68,$67,$69,$81
                      db $6B,$06,$70,$74,$77,$76,$7A,$7B
                      db $78,$81,$6B,$03,$70,$71,$70,$71
                      db $81,$6B,$10,$70,$74,$77,$76,$7C
                      db $75,$71,$70,$6B,$70,$74,$75,$74
                      db $75,$71,$70,$74,$84,$77,$03,$75
                      db $74,$70,$74,$83,$77,$01,$75,$74
                      db $86,$77,$01,$7A,$74,$82,$77,$01
                      db $76,$7C,$83,$77,$01,$7D,$7E,$82
                      db $77,$01,$75,$7E,$83,$77,$01,$76
                      db $7C,$81,$77,$03,$7D,$7F,$80,$7E
                      db $82,$77,$03,$80,$7E,$7D,$7E,$83
                      db $77,$05,$7D,$7F,$81,$82,$80,$7E
                      db $81,$77,$09,$83,$80,$7F,$80,$7E
                      db $7D,$7E,$77,$84,$85,$82,$83,$02
                      db $80,$7E,$77,$81,$83,$12,$82,$83
                      db $80,$7F,$80,$7E,$77,$84,$85,$86
                      db $85,$86,$87,$77,$83,$81,$83,$86
                      db $85,$81,$83,$08,$80,$7E,$77,$84
                      db $87,$84,$87,$77,$76,$81,$83,$07
                      db $86,$87,$84,$85,$83,$82,$80,$7E
                      db $85,$77,$8F,$59,$01,$5A,$5B,$85
                      db $59,$01,$5A,$5B,$8A,$59,$01,$5A
                      db $5B,$89,$59,$03,$5C,$5D,$5C,$5D
                      db $8A,$59,$07,$5C,$5E,$5F,$5E,$5F
                      db $5D,$5C,$5D,$85,$59,$02,$5A,$5B
                      db $5E,$82,$60,$04,$66,$5F,$5E,$5F
                      db $5D,$81,$59,$01,$5C,$5D,$82,$59
                      db $81,$60,$01,$61,$62,$81,$60,$07
                      db $63,$60,$5F,$5D,$5C,$5E,$5F,$5D
                      db $81,$59,$06,$60,$61,$64,$65,$62
                      db $61,$62,$81,$60,$08,$5F,$5E,$60
                      db $63,$5F,$5D,$5C,$61,$64,$81,$59
                      db $05,$65,$64,$65,$62,$60,$66,$83
                      db $60,$05,$5F,$5E,$5F,$5D,$5C,$5D
                      db $81,$59,$01,$5C,$5E,$81,$60,$01
                      db $67,$68,$83,$60,$19,$68,$5F,$5E
                      db $5F,$5D,$5C,$5E,$67,$68,$67,$69
                      db $6A,$68,$67,$68,$67,$6A,$68,$66
                      db $60,$67,$68,$67,$69,$6A,$69,$81
                      db $6B,$0F,$6A,$69,$6A,$69,$6D,$6A
                      db $68,$67,$69,$6A,$69,$6B,$70,$71
                      db $6B,$6D,$82,$6B,$00,$6F,$81,$6B
                      db $10,$6A,$69,$6C,$6D,$6B,$70,$74
                      db $75,$71,$70,$71,$6B,$6F,$73,$71
                      db $70,$71,$83,$6B,$0C,$78,$79,$76
                      db $75,$74,$75,$71,$6A,$68,$75,$74
                      db $75,$71,$83,$6B,$0E,$78,$79,$7A
                      db $79,$7A,$7B,$6B,$6A,$79,$77,$7C
                      db $75,$71,$6B,$6C,$81,$6B,$08,$78
                      db $7B,$78,$7B,$6B,$70,$71,$74,$76
                      db $81,$77,$04,$75,$71,$70,$71,$6D
                      db $82,$6B,$05,$70,$71,$78,$7B,$7A
                      db $79,$82,$77,$0D,$75,$74,$75,$71
                      db $70,$71,$70,$74,$75,$71,$6B,$7B
                      db $78,$79,$84,$77,$06,$75,$74,$75
                      db $74,$77,$7C,$75,$81,$71,$04,$6B
                      db $78,$79,$77,$7C,$88,$77,$81,$75
                      db $05,$71,$70,$74,$77,$76,$7C,$87
                      db $77,$03,$7D,$77,$75,$74,$87,$77
                      db $07,$76,$7C,$77,$7D,$7F,$77,$7D
                      db $7E,$83,$77,$01,$7D,$7E,$84,$77
                      db $0B,$84,$85,$77,$84,$87,$77,$7D
                      db $7E,$7D,$7F,$80,$7E,$83,$77,$02
                      db $7D,$7F,$7C,$81,$77,$07,$7D,$7F
                      db $80,$7F,$83,$82,$80,$7E,$81,$77
                      db $02,$7D,$7F,$83,$81,$77,$03,$7D
                      db $7F,$81,$82,$83,$83,$05,$80,$7E
                      db $77,$84,$85,$83,$FF,$FF

DATA_0CEF80:          db $86,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $88,$83,$07,$89,$83,$8A,$8B,$8C
                      db $83,$8B,$8C,$86,$83,$0A,$8D,$8E
                      db $8F,$90,$91,$92,$8A,$91,$92,$8A
                      db $8C,$83,$83,$0D,$8D,$8C,$93,$90
                      db $94,$95,$96,$97,$95,$96,$97,$8C
                      db $89,$83,$82,$98,$1C,$99,$9A,$9B
                      db $9C,$9D,$9E,$9C,$9D,$9E,$9F,$A0
                      db $A1,$98,$83,$89,$8A,$90,$A2,$A3
                      db $A4,$A5,$A4,$A6,$A5,$A4,$A6,$96
                      db $A7,$8C,$82,$98,$5C,$99,$9A,$9B
                      db $9C,$A8,$9E,$9C,$A8,$9E,$9F,$A0
                      db $A1,$98,$A9,$83,$8D,$8E,$A2,$A3
                      db $A4,$AA,$A6,$AB,$AA,$A6,$AB,$96
                      db $AC,$8F,$AD,$AE,$8D,$AD,$AE,$AF
                      db $A3,$B0,$AB,$B1,$B0,$B2,$AF,$AD
                      db $AE,$A9,$B3,$B4,$92,$B3,$B4,$92
                      db $90,$B5,$B6,$B1,$B5,$B6,$B1,$B3
                      db $B4,$B7,$B8,$B9,$B7,$B8,$B9,$92
                      db $8C,$88,$97,$8E,$88,$97,$8E,$B8
                      db $B9,$8C,$BA,$8C,$B7,$93,$A9,$8D
                      db $8C,$88,$8A,$A9,$88,$8A,$A9,$83
                      db $93,$8C,$8F,$98,$8F,$B5,$8F,$88
                      db $C6,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$00,$88,$81,$83,$00,$88
                      db $8B,$83,$03,$88,$83,$93,$88,$88
                      db $83,$07,$89,$83,$8A,$8B,$8C,$83
                      db $8B,$8C,$86,$83,$0C,$8D,$8E,$8F
                      db $90,$91,$90,$BB,$91,$92,$8A,$8C
                      db $83,$89,$81,$83,$0D,$8D,$8A,$8E
                      db $A2,$A3,$95,$BC,$A3,$95,$BD,$BC
                      db $B1,$8F,$8C,$82,$98,$1C,$99,$9A
                      db $9B,$9C,$9D,$9E,$9C,$9D,$9E,$9F
                      db $A0,$A1,$98,$83,$89,$8A,$90,$A2
                      db $A3,$A4,$A5,$A4,$A6,$A5,$A4,$A6
                      db $96,$A7,$8C,$82,$98,$5C,$99,$9A
                      db $9B,$9C,$A8,$9E,$9C,$A8,$9E,$9F
                      db $A0,$A1,$98,$A9,$83,$8D,$8E,$A2
                      db $A3,$A4,$AA,$A6,$AB,$AA,$A4,$A6
                      db $96,$AC,$8F,$AD,$AE,$8D,$AD,$AE
                      db $AF,$A3,$B0,$BE,$BC,$B0,$BE,$BC
                      db $AD,$AE,$A9,$B3,$B4,$B7,$B3,$B4
                      db $92,$90,$B5,$B6,$B1,$B5,$B6,$B1
                      db $B3,$B4,$B7,$B8,$B9,$B7,$B8,$B9
                      db $92,$8C,$88,$97,$8E,$88,$97,$8E
                      db $B8,$B9,$8E,$83,$8D,$8E,$8F,$8C
                      db $8D,$8C,$88,$8A,$A9,$88,$8A,$BA
                      db $8A,$A9,$8D,$8F,$98,$8F,$B5,$8F
                      db $88,$BF,$83,$FF,$FF

DATA_0CF175:          db $00,$BF,$81,$C0,$04,$C1,$C2,$C3
                      db $C0,$BF,$81,$C0,$00,$BF,$81,$C0
                      db $00,$BF,$81,$C0,$00,$C4,$81,$C5
                      db $04,$C1,$C2,$C6,$C5,$C4,$81,$C5
                      db $00,$C4,$81,$C5,$00,$C4,$81,$C5
                      db $82,$C0,$02,$C1,$C2,$C3,$89,$C0
                      db $82,$C5,$02,$C1,$C2,$C6,$89,$C5
                      db $82,$C7,$01,$C8,$C2,$88,$C7,$01
                      db $C9,$C7,$82,$CA,$01,$C1,$C2,$83
                      db $CA,$81,$CB,$84,$CA,$82,$CC,$02
                      db $C1,$C2,$CD,$89,$CC,$82,$C5,$02
                      db $C1,$C2,$C6,$89,$C5,$82,$C0,$04
                      db $C1,$C2,$C3,$C0,$BF,$81,$C0,$00
                      db $BF,$81,$C0,$00,$CE,$81,$59,$82
                      db $C5,$04,$C1,$CF,$C6,$C5,$C4,$81
                      db $C5,$00,$C4,$81,$C5,$00,$D0,$81
                      db $59,$00,$D1,$81,$C0,$02,$C8,$C2
                      db $C3,$85,$C0,$00,$CE,$82,$59,$00
                      db $D2,$81,$C5,$02,$C1,$C2,$C6,$85
                      db $C5,$00,$D0,$81,$59,$00,$D3,$82
                      db $C0,$02,$C1,$C2,$C3,$84,$C0,$00
                      db $CE,$81,$59,$01,$D1,$D4,$82,$C5
                      db $02,$C1,$C2,$C6,$84,$C5,$04,$D0
                      db $D3,$D5,$D2,$C5,$82,$C0,$04,$C1
                      db $C2,$C3,$C0,$BF,$83,$C0,$03,$D4
                      db $C0,$BF,$C0,$82,$C5,$04,$C1,$C2
                      db $C6,$C5,$C4,$81,$C5,$00,$D6,$82
                      db $C5,$07,$C4,$C5,$59,$D1,$C0,$C1
                      db $C2,$C3,$81,$C0,$02,$CE,$59,$D7
                      db $85,$59,$04,$D2,$C5,$C1,$C2,$C6
                      db $81,$C5,$00,$D0,$82,$59,$00,$D5
                      db $81,$59,$00,$D3,$82,$C7,$01,$C1
                      db $C2,$84,$C7,$00,$C9,$84,$C7,$81
                      db $CA,$03,$CB,$C1,$C2,$CB,$87,$CA
                      db $01,$CB,$CA,$82,$CC,$02,$C1,$CF
                      db $CD,$89,$CC,$82,$C5,$02,$C1,$C2
                      db $C6,$89,$C5,$00,$BF,$81,$C0,$04
                      db $C8,$C2,$C3,$C0,$CE,$82,$59,$02
                      db $D1,$C0,$BF,$81,$C0,$00,$C4,$81
                      db $C5,$0A,$C1,$C2,$C6,$C5,$D0,$59
                      db $D3,$59,$D2,$D6,$C4,$81,$C5,$82
                      db $C0,$02,$C1,$C2,$C3,$82,$C0,$03
                      db $D4,$CE,$59,$D7,$82,$59,$82,$C5
                      db $02,$C1,$CF,$C6,$83,$C5,$01,$D0
                      db $59,$81,$D5,$01,$D3,$59,$82,$C0
                      db $02,$C1,$C2,$C3,$87,$C0,$02,$D4
                      db $C0,$BF,$81,$C0,$04,$C1,$C2,$C3
                      db $C0,$BF,$81,$C0,$00,$BF,$81,$C0
                      db $00,$BF,$81,$C0,$00,$C4,$81,$C5
                      db $04,$C1,$C2,$C6,$C5,$C4,$81,$C5
                      db $00,$C4,$81,$C5,$00,$C4,$81,$C5
                      db $82,$C0,$02,$C8,$C2,$C3,$89,$C0
                      db $82,$C5,$02,$C1,$C2,$C6,$89,$C5
                      db $82,$C7,$03,$C1,$C2,$C7,$C9,$83
                      db $C7,$00,$C9,$83,$C7,$82,$CA,$01
                      db $C1,$C2,$81,$CA,$00,$CB,$87,$CA
                      db $82,$CC,$02,$C1,$C2,$CD,$89,$CC
                      db $82,$C5,$02,$C1,$C2,$C6,$89,$C5
                      db $00,$D1,$81,$C0,$04,$C1,$C2,$C3
                      db $C0,$CE,$83,$59,$01,$D1,$BF,$81
                      db $C0,$00,$D2,$81,$C5,$04,$C1,$C2
                      db $C6,$C5,$D0,$83,$59,$01,$D2,$C4
                      db $81,$C5,$82,$59,$03,$C1,$C2,$C3
                      db $CE,$89,$59,$05,$D5,$59,$C1,$C2
                      db $C6,$D0,$81,$59,$03,$D3,$59,$D5
                      db $D3,$81,$59,$00,$D5,$82,$C0,$02
                      db $C8,$C2,$C3,$82,$C0,$03,$D4,$BF
                      db $C0,$D4,$82,$C0,$82,$C5,$02,$C1
                      db $C2,$C6,$81,$C5,$02,$D6,$C5,$C4
                      db $84,$C5,$82,$C0,$01,$C1,$C2,$82
                      db $59,$00,$D7,$83,$59,$00,$D1,$81
                      db $C0,$82,$C5,$02,$C1,$C2,$D5,$86
                      db $59,$09,$D2,$D6,$C5,$59,$D1,$C0
                      db $C1,$CF,$C3,$CE,$81,$59,$00,$D1
                      db $82,$C0,$01,$CE,$D7,$81,$59,$08
                      db $D2,$C5,$C1,$C2,$C6,$D0,$D5,$59
                      db $D2,$82,$C5,$00,$D0,$81,$59,$82
                      db $C7,$03,$C1,$C2,$C7,$C9,$84,$C7
                      db $00,$C9,$82,$C7,$82,$CA,$01,$C1
                      db $CF,$84,$CA,$00,$CB,$84,$CA,$82
                      db $CC,$02,$C1,$C2,$CD,$89,$CC,$82
                      db $C5,$02,$C1,$C2,$C6,$89,$C5,$00
                      db $BF,$81,$C0,$05,$C8,$C2,$C3,$C0
                      db $BF,$CE,$81,$59,$02,$D1,$C0,$BF
                      db $81,$C0,$00,$C4,$81,$C5,$05,$C1
                      db $CF,$C6,$C5,$C4,$D0,$81,$59,$02
                      db $D2,$D6,$C4,$81,$C5,$00,$D1,$81
                      db $C0,$03,$C1,$C2,$C3,$CE,$84,$59
                      db $01,$D7,$D1,$81,$C0,$00,$D2,$81
                      db $C5,$04,$C1,$C2,$C6,$D0,$D3,$81
                      db $59,$00,$D5,$81,$59,$00,$D2,$81
                      db $C5,$82,$C0,$04,$C1,$C2,$C3,$C0
                      db $D4,$87,$C0,$FF,$FF

DATA_0CF45A:          db $0B,$76,$D8,$D9,$DA,$DB,$D9,$DA
                      db $DB,$76,$D8,$D9,$DA,$81,$DB,$0D
                      db $D9,$DA,$76,$DC,$DD,$DE,$DF,$DD
                      db $DE,$DF,$76,$DC,$DD,$DE,$81,$DF
                      db $09,$DD,$DE,$DB,$D9,$DA,$D9,$DA
                      db $DB,$D9,$DA,$83,$DB,$0B,$D9,$E0
                      db $DA,$DB,$DF,$DD,$DE,$DD,$DE,$DF
                      db $DD,$DE,$83,$DF,$0B,$DD,$E1,$DE
                      db $DF,$E2,$E3,$E4,$E3,$E4,$E2,$E3
                      db $E4,$83,$E2,$06,$E3,$E5,$E4,$E2
                      db $E6,$E7,$E8,$81,$E9,$03,$E7,$E8
                      db $E9,$E6,$82,$E9,$06,$E7,$E8,$E9
                      db $E7,$76,$EA,$DE,$81,$DF,$04,$DD
                      db $DE,$DF,$76,$DC,$81,$DF,$0D,$DD
                      db $DE,$DF,$DD,$76,$EB,$E4,$E2,$DF
                      db $E3,$E4,$DF,$76,$DC,$81,$E2,$3F
                      db $E3,$E4,$E2,$E3,$76,$EC,$ED,$EE
                      db $E5,$ED,$EE,$E4,$76,$EB,$ED,$EE
                      db $E0,$ED,$EE,$DA,$76,$EB,$EF,$F0
                      db $E0,$EF,$F0,$DA,$76,$EC,$EF,$F0
                      db $E1,$EF,$F0,$E4,$76,$EC,$F1,$F2
                      db $E5,$F1,$F2,$E4,$76,$EB,$F1,$F2
                      db $E5,$F1,$F2,$DA,$76,$DC,$D9,$DA
                      db $DB,$D9,$DA,$DB,$76,$D8,$D9,$DA
                      db $81,$DB,$0D,$D9,$DE,$76,$DC,$DD
                      db $DE,$DF,$DD,$DE,$DF,$76,$DC,$DD
                      db $DE,$81,$DF,$09,$DD,$DE,$DB,$D9
                      db $DA,$D9,$DA,$DB,$D9,$DA,$81,$DB
                      db $1F,$D9,$DA,$D9,$DA,$DB,$D9,$DF
                      db $DD,$DE,$DD,$DE,$DF,$DD,$E4,$E2
                      db $F3,$E3,$DE,$DD,$DE,$DF,$DD,$E2
                      db $E3,$E4,$E3,$E4,$E2,$E3,$E4,$E3
                      db $E4,$81,$E2,$06,$E3,$E4,$E2,$E3
                      db $E6,$E7,$E8,$81,$E9,$03,$E7,$E8
                      db $E9,$E6,$83,$E9,$0C,$E7,$E8,$E7
                      db $76,$EA,$DE,$DF,$E2,$DD,$DE,$DF
                      db $76,$DC,$82,$DF,$0C,$DD,$DE,$DD
                      db $76,$EB,$E4,$E3,$E0,$E5,$E4,$DF
                      db $76,$DC,$82,$E2,$3D,$E3,$E4,$E3
                      db $76,$EC,$ED,$EE,$E1,$ED,$EE,$E4
                      db $76,$EB,$ED,$EE,$E0,$ED,$EE,$DA
                      db $76,$EB,$EF,$F0,$E5,$EF,$F0,$DA
                      db $76,$EC,$EF,$F0,$E1,$EF,$F0,$DE
                      db $76,$EC,$F1,$F2,$E0,$F1,$F2,$E4
                      db $76,$EB,$F1,$F2,$E5,$F1,$F2,$DE
                      db $76,$DC,$D9,$DA,$DF,$D9,$DA,$DB
                      db $76,$D8,$D9,$81,$E0,$0D,$DA,$D9
                      db $E4,$76,$DC,$DD,$DE,$DF,$DD,$DE
                      db $DF,$76,$DC,$DD,$81,$E1,$05,$DE
                      db $DD,$DA,$DB,$D9,$DA,$82,$DB,$01
                      db $D9,$DA,$83,$DB,$00,$D9,$81,$E0
                      db $04,$DA,$DF,$DD,$E4,$E2,$81,$F3
                      db $01,$E3,$DE,$83,$DF,$00,$DD,$81
                      db $E1,$03,$DE,$DF,$DD,$DA,$82,$DB
                      db $01,$D9,$DE,$83,$DF,$00,$DD,$81
                      db $E5,$0C,$DE,$76,$D8,$D9,$DA,$DB
                      db $D9,$DA,$DB,$76,$D8,$D9,$DA,$81
                      db $DB,$0D,$D9,$DA,$76,$DC,$DD,$DE
                      db $DF,$DD,$DE,$DF,$76,$DC,$DD,$DE
                      db $81,$DF,$06,$DD,$DE,$D9,$E0,$DA
                      db $D9,$DA,$81,$DB,$06,$D9,$DA,$D9
                      db $DA,$DB,$D9,$DA,$81,$DB,$04,$DD
                      db $E1,$DE,$DD,$DE,$81,$DF,$06,$DD
                      db $DE,$DD,$DE,$DF,$DD,$DE,$81,$DF
                      db $04,$E3,$E5,$E4,$E3,$E4,$81,$E2
                      db $06,$E3,$E4,$E3,$E4,$E2,$E3,$E4
                      db $81,$E2,$04,$E6,$E7,$E8,$E7,$E8
                      db $82,$E9,$0C,$E6,$E7,$E8,$E9,$E7
                      db $E8,$E9,$E7,$76,$EA,$DE,$DD,$DE
                      db $81,$DF,$4E,$E2,$76,$EB,$DE,$DF
                      db $DD,$DE,$DF,$DD,$76,$EB,$E4,$E3
                      db $E4,$E2,$E3,$DA,$76,$EC,$E4,$E2
                      db $E3,$E4,$E2,$E3,$76,$EC,$ED,$EE
                      db $E0,$ED,$EE,$DE,$76,$EA,$ED,$EE
                      db $E0,$ED,$EE,$DA,$76,$EB,$EF,$F0
                      db $E5,$EF,$F0,$DE,$76,$EA,$EF,$F0
                      db $E1,$EF,$F0,$E4,$76,$EC,$F1,$F2
                      db $E0,$F1,$F2,$E4,$76,$EB,$F1,$F2
                      db $E5,$F1,$F2,$DA,$76,$DC,$D9,$DA
                      db $DD,$DA,$81,$DB,$03,$76,$EC,$E0
                      db $DA,$81,$DB,$07,$D9,$DE,$76,$DC
                      db $DD,$DE,$DD,$DE,$81,$DF,$03,$76
                      db $EB,$E1,$DE,$81,$DF,$06,$DD,$DE
                      db $DA,$D9,$DA,$DB,$D9,$81,$E0,$06
                      db $DA,$DB,$D9,$DA,$DB,$D9,$DA,$81
                      db $DB,$0D,$DE,$DD,$DE,$DF,$DD,$E5
                      db $E1,$DE,$DF,$DD,$DE,$DF,$DD,$DE
                      db $81,$DF,$04,$E4,$E3,$E4,$E2,$E3
                      db $81,$E5,$06,$E4,$E2,$E3,$E4,$E2
                      db $E3,$E4,$81,$E2,$02,$E6,$E7,$E8
                      db $81,$E9,$03,$E7,$E8,$E9,$E6,$82
                      db $E9,$06,$E7,$E8,$E9,$E7,$76,$EA
                      db $DE,$81,$DF,$04,$DD,$DE,$DF,$76
                      db $DC,$81,$DF,$06,$DD,$DE,$DF,$DD
                      db $76,$EB,$E4,$81,$E2,$04,$E3,$E4
                      db $DF,$76,$DC,$81,$E2,$51,$E3,$E4
                      db $E2,$E3,$76,$EC,$ED,$EE,$E0,$ED
                      db $EE,$E4,$76,$EB,$ED,$EE,$E0,$ED
                      db $EE,$DA,$76,$EA,$EF,$F0,$E1,$EF
                      db $F0,$DA,$76,$EC,$EF,$F0,$E1,$EF
                      db $F0,$E4,$76,$EA,$F1,$F2,$E5,$F1
                      db $F2,$E4,$76,$EB,$F1,$F2,$E5,$F1
                      db $F2,$DA,$76,$EB,$E0,$DA,$DB,$D9
                      db $DA,$DB,$76,$EC,$DA,$DB,$D9,$DA
                      db $DB,$DF,$76,$EC,$E1,$DE,$DF,$DD
                      db $DE,$DF,$76,$EA,$DE,$DF,$DD,$DE
                      db $81,$DF,$02,$DB,$D9,$DA,$82,$DB
                      db $0C,$D9,$DA,$DB,$D9,$DA,$DB,$D9
                      db $DA,$D9,$DA,$DF,$DD,$DE,$82,$DF
                      db $0C,$DD,$E4,$E2,$E3,$DE,$DF,$DD
                      db $DE,$DD,$DE,$DF,$DD,$DE,$82,$DF
                      db $09,$DD,$DA,$DB,$D9,$DE,$DF,$DD
                      db $DE,$DD,$DE,$FF,$FF

OWL1TileData:         db $00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$82
                      db $04,$6E,$04,$72,$04,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $99,$A3,$9B,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $97,$98,$6E,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$10,$00,$00,$00,$00,$00
                      db $00,$00,$83,$00,$00,$00,$00,$00
                      db $0A,$03,$63,$04,$4E,$04,$6E,$15
                      db $1A,$00,$10,$00,$00,$00,$00,$00
                      db $12,$00,$10,$00,$00,$00,$3F,$00
                      db $20,$88,$4C,$00,$00,$00,$00,$00
                      db $10,$00,$3F,$00,$00,$00,$2A,$00
                      db $6E,$04,$5D,$00,$00,$00,$76,$04
                      db $7B,$00,$2A,$00,$BC,$00,$1E,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$74,$32,$35,$00,$19,$16
                      db $82,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$2A,$00,$20,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$8A,$A6
                      db $00,$00,$00,$00,$00,$58,$04,$04
                      db $04,$6E,$04,$6E,$04,$04,$04,$5D
                      db $15,$1A,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$10
                      db $00,$20,$00,$00,$00,$00,$00,$08
                      db $16,$70,$4F,$04,$6E,$00,$00,$3F
                      db $00,$3F,$00,$00,$00,$00,$0D,$0E
                      db $00,$3F,$00,$00,$1E,$00,$00,$41
                      db $00,$3F,$00,$00,$00,$00,$82,$00
                      db $00,$3F,$00,$00,$19,$06,$06,$09
                      db $00,$3F,$00,$00,$00,$00,$00,$00
                      db $00,$72,$00,$00,$00,$00,$00,$00
                      db $00,$3F,$00,$00,$00,$00,$00,$00
                      db $00,$31,$BC,$00,$00,$00,$00,$00
                      db $00,$10,$00,$00,$00,$00,$00,$00
                      db $28,$29,$00,$00,$00,$00,$00,$00
                      db $00,$4D,$00,$00,$00,$00,$00,$00
                      db $2F,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$28
                      db $29,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$3A
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$3A
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$5A,$00,$2F
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$99,$A3,$9B
                      db $00,$00,$10,$00,$63,$04,$82,$00
                      db $00,$00,$00,$00,$00,$00,$00,$76
                      db $00,$00,$70,$00,$10,$00,$00,$00
                      db $00,$00,$00,$00,$00,$97,$00,$10
                      db $00,$9B,$10,$00,$5A,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$3F
                      db $97,$98,$10,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$40
                      db $00,$00,$10,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$99,$9A,$9B
                      db $00,$00,$4D,$00,$00,$00,$00,$00
                      db $00,$00,$00,$4D,$00,$00,$00,$00
                      db $00,$00,$A6,$00,$00,$00,$00,$00
                      db $00,$00,$00,$2A,$00,$00,$00,$00
                      db $00,$00,$5D,$87,$00,$00,$00,$00
                      db $00,$2E,$2B,$7F,$BD,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$82
                      db $96,$50,$00,$00,$00,$00,$00,$00
                      db $56,$00,$00,$00,$00,$00,$00,$10
                      db $00,$2A,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$1E
                      db $00,$4C,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$19
                      db $16,$5D,$8C,$00,$58,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$10,$00,$00,$10,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$6E,$04,$04,$57,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$3F,$00,$30
                      db $00,$5A,$04,$58,$04,$04,$42,$00
                      db $00,$4D,$00,$00,$00,$34,$00,$30
                      db $00,$00,$00,$00,$00,$A6,$00,$00
                      db $00,$0F,$00,$00,$00,$37,$33,$2D
                      db $00,$00,$00,$00,$8B,$5D,$04,$04
                      db $05,$09,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$10,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$10,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$10,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$63,$04,$04,$6E,$00,$00
                      db $00,$00,$00,$00,$00,$BE,$00,$82
                      db $00,$00,$10,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$10,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$0A,$02
                      db $1A,$00,$10,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$12,$00
                      db $20,$00,$10,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$10,$00
                      db $70,$04,$70,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$1E,$00
                      db $0F,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$19,$06
                      db $09,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$40,$00,$00,$00,$00,$4D,$00
                      db $00,$00,$00,$00,$00,$97,$00,$98
                      db $00,$3F,$00,$99,$9A,$9B,$4C,$87
                      db $9C,$9D,$00,$00,$00,$00,$9E,$9F
                      db $00,$2A,$00,$00,$00,$00,$5D,$15
                      db $1A,$9E,$9F,$00,$00,$99,$A0,$00
                      db $00,$3F,$00,$00,$A1,$A2,$99,$A0
                      db $20,$00,$00,$00,$00,$00,$00,$00
                      db $00,$3F,$00,$00,$00,$00,$00,$00
                      db $72,$00,$00,$00,$00,$00,$00,$00
                      db $00,$0F,$00,$00,$9E,$00,$9F,$00
                      db $0F,$00,$00,$00,$00,$00,$00,$00
                      db $0D,$0E,$00,$B5,$99,$A0,$6E,$05
                      db $09,$00,$00,$00,$00,$00,$00,$00
                      db $12,$00,$07,$56,$17,$00,$10,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $66,$05,$11,$00,$1F,$16,$66,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$B6,$BB,$00
                      db $82,$00,$00,$B8,$00,$00,$00,$00
                      db $00,$00,$B8,$00,$00,$B8,$00,$B9
                      db $3F,$00,$00,$00,$00,$63,$04,$04
                      db $15,$1A,$B9,$00,$00,$00,$00,$BA
                      db $3F,$B8,$00,$00,$00,$10,$00,$00
                      db $00,$20,$00,$00,$00,$5A,$04,$04
                      db $70,$00,$00,$00,$BB,$3F,$B9,$00
                      db $00,$6F,$00,$00,$00,$00,$00,$B8
                      db $3F,$B6,$00,$00,$B8,$3F,$BA,$00
                      db $B8,$10,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$A7,$A8,$00,$00,$00
                      db $00,$00,$00,$00,$00,$6E,$04,$82
                      db $00,$00,$A9,$AA,$AB,$AC,$00,$00
                      db $00,$00,$00,$00,$00,$10,$00,$00
                      db $00,$00,$AD,$AE,$AF,$B0,$B1,$00
                      db $00,$00,$00,$00,$00,$82,$00,$00
                      db $B6,$5A,$B2,$B3,$60,$B4,$61,$00
                      db $BA,$00,$00,$00,$00,$00,$00,$B8
                      db $00,$10,$00,$8D,$4C,$00,$10,$00
                      db $00,$00,$BA,$00,$00,$00,$00,$00
                      db $BA,$70,$04,$04,$5D,$00,$58,$00
                      db $00,$B8,$BB,$00,$00,$00,$00,$00
                      db $B7,$10,$00,$00,$10,$00,$10,$00
                      db $00,$00,$00,$00,$00,$00,$B8,$78
                      db $00,$6E,$04,$04,$63,$04,$71,$04
                      db $6E,$04,$15,$1A,$00,$B6,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$20,$00,$BB,$B9,$00
                      db $00,$82,$04,$04,$6E,$04,$04,$04
                      db $04,$82,$00,$47,$00,$00,$00,$00
                      db $00,$00,$00,$8E,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$8F,$90,$91,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$92,$93,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$B9,$00
                      db $3F,$BB,$00,$00,$00,$74,$32,$35
                      db $00,$6E,$00,$00,$00,$00,$00,$00
                      db $1E,$00,$00,$00,$00,$31,$00,$3D
                      db $00,$4C,$00,$00,$00,$00,$00,$00
                      db $19,$16,$71,$04,$2C,$2D,$00,$78
                      db $00,$5D,$89,$00,$00,$00,$00,$00
                      db $00,$00,$10,$00,$00,$00,$00,$00
                      db $00,$0F,$00,$00,$00,$00,$00,$00
                      db $00,$00,$48,$00,$00,$00,$00,$82
                      db $05,$09,$00,$00,$00,$00,$99,$9A
                      db $9B,$00,$00,$00,$00,$4D,$00,$00
                      db $00,$97,$A3,$9B,$00,$9F,$00,$00
                      db $00,$00,$00,$00,$00,$10,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$0A,$03,$7A,$04,$69,$15,$1A
                      db $00,$9E,$9F,$00,$00,$9A,$9B,$00
                      db $00,$12,$00,$10,$00,$00,$00,$20
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$10,$00,$10,$00,$00,$00,$10
                      db $00,$00,$99,$9A,$9B,$98,$00,$00
                      db $00,$70,$00,$10,$00,$00,$00,$74
                      db $04,$78,$00,$00,$00,$00,$00,$00
                      db $00,$10,$00,$1E,$00,$00,$00,$0F
                      db $00,$00,$00,$00,$00,$00,$9E,$9F
                      db $00,$6E,$00,$19,$16,$70,$05,$09
                      db $00,$00,$97,$A3,$9B,$97,$A3,$9B
                      db $00,$0F,$00,$00,$00,$10,$00,$00
                      db $00,$00,$00,$00,$00,$42,$04,$04
                      db $05,$09,$00,$00,$00,$10,$00,$00
                      db $00,$99,$9A,$9B,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$4D,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$5A,$04
                      db $7D,$04,$04,$6E,$04,$04,$7D,$04
                      db $04,$6E,$04,$15,$1A,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$30,$00,$5F,$04
                      db $66,$04,$04,$7D,$04,$04,$6E,$04
                      db $04,$7D,$04,$05,$09,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$49,$5F,$4A,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$0A,$22,$00,$21,$1A,$00
                      db $00,$00,$00,$00,$00,$00,$00,$5F
                      db $04,$04,$68,$00,$5A,$00,$68,$04
                      db $04,$5F,$00,$00,$00,$00,$00,$19
                      db $18,$00,$00,$00,$3F,$00,$00,$00
                      db $08,$09,$00,$00,$00,$00,$00,$00
                      db $4B,$68,$00,$00,$3F,$00,$00,$68
                      db $51,$00,$00,$00,$00,$00,$00,$00
                      db $0D,$0E,$00,$07,$68,$17,$00,$1D
                      db $1C,$00,$00,$00,$00,$00,$00,$0A
                      db $22,$07,$01,$11,$00,$1F,$14,$17
                      db $21,$1A,$00,$00,$00,$00,$00,$5F
                      db $05,$11,$00,$00,$00,$00,$00,$1F
                      db $16,$5F,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00,$00,$00,$00
                      db $00,$00,$00,$00,$00

                      %insert_empty($21,$21,$21,$21,$21)