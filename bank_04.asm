...
DATA_048000:          dw $B480,$B498,$B4B0

DATA_048006:          dw $B300,$B318,$B330,$B348
...
CODE_0480F9:          LSR.B _0
                      ROR.W GfxDecompOWAni,X
                    + DEX
                      BPL CODE_0480E8
CODE_048101:          LDA.B TrueFrame                           ; \
                      AND.B #$07                                ; |If lower 3 bits of frame counter isn't 0,
                    ...
                    - LDA.W GfxDecompOWAni+$40,X
                      ...
                      LDX.B #$40
                      JSR CODE_048172
CODE_048123:          REP #$30                                  ; AXY->16
                      LDA.W #$0060
                      STA.B _D
                      STZ.B _B
CODE_04812C:          LDX.W #$0038
                      LDA.B _B
                      CMP.W #$0020
                      BCS +
                      LDX.W #$0070
...
CODE_048172:          REP #$20                                  ; A->16
                    ...
                    - PHX
                      ...
                      STA.B _0
                      PLX
CODE_048183:          LDA.W GfxDecompOWAni,X
                      STA.B _2
                      LDA.B _0
                      STA.W GfxDecompOWAni,X
                      LDA.B _2
...
OWScrollArrowStripe:  db $50,$CF,$00,$03,$7E,$78,$7E,$38
...
OWScrollEraseStripe:  db $50,$CF,$40,$02,$FC,$00,$50,$EF
...
OWScrollSpeed:        db $00,$00,$02,$00,$FE,$FF,$02,$00
                      db $00,$00,$02,$00,$FE,$FF,$02,$00
OWMaxScrollRange:     db $00,$00,$11,$01,$EF,$FF,$11,$01
...
CODE_048246:          LDA.W byetudlrP1Frame,X                   ; |
...
CODE_048261:          DEX
                    ...
                    + LDA.W OverworldPromptProcess
                      ...
                      BEQ CODE_048295
CODE_04828F:          JSR CODE_04F3E5
                      JMP CODE_048413

CODE_048295:          LDA.W PauseFlag
                      LSR A
                      BNE +
                      JMP CODE_048356
...
CODE_048356:          LDA.W OverworldProcess
                      CMP.B #$03
                      BEQ CODE_048366
                      CMP.B #$04
                      BNE CODE_04839A
...
CODE_04839A:          LDA.W PauseFlag
                      ...
                      AND.B #$18
                      BNE +
CODE_0483BD:          LDY.B #$18
                    + STY.B StripeImage
                      BRA CODE_04840D

CODE_0483C3:          LDX.W OverworldEarthquake
                      BEQ CODE_04840A
                      CPX.B #$FE
                      BNE +
                    ...
                    + CPX.B #$80
                      ...
                      CMP.B #$02
                      BNE CODE_04840A
CODE_0483FE:          DEC.W OverworldEarthquake
                      BNE CODE_04840D
                      LDA.B #!SFX_RUMBLINGOFF
                      STA.W SPCIO0                              ; / Play sound effect
                      BRA CODE_04840D

CODE_04840A:          JSR CODE_048576
CODE_04840D:          JSR CODE_04F708
CODE_048410:          JSR CODE_04862E
CODE_048413:          PLB
                      RTL

OWFreeLook:           TAY                                       ; A=$2,X=$0 for right; A=$4,X=$0 for left; A=$A,X=$2 for down; A=$C,X=$2 for up
                      REP #$20                                  ; A->16
                      LDA.B Layer1XPos,X
                      CLC
...
CODE_048509:          LDY.W PlayerTurnLvl                       ; \ Get current player's submap
...
CODE_04851A:          LDA.W DATA_048431,Y
                      EOR.B _0
                      CMP.W #$0200
                      BCS CODE_048531
...
CODE_048531:          DEY
                      DEY
                      BPL CODE_04851A
CODE_048535:          STY.W StarWarpIndex                       ; Store Y in "Warp destination"
                      SEP #$20                                  ; A->8
                      RTS
...
CODE_048576:          LDA.W OverworldProcess
                      JSL ExecutePtrLong
...
DrawOWBoarder_:       JSR CODE_04862E
CODE_0485A7:          REP #$20                                  ; A->16
...
CODE_0486BC:          LDA.B GraphicsCompPtr+1
                      SEC
                      SBC.B #$05                                ; subtract 5 from Y-pos if on land
                      STA.B GraphicsCompPtr+1                   ; $8B = 1P Y-pos on screen
                      STA.B _7                                  ; $07 = 1P Y-pos on screen
CODE_0486C5:          REP #$30                                  ; AXY->16
                      LDA.W PlayerTurnOW                        ; A = player x 4
                      XBA                                       ; A = player x #$400
...
CODE_048730:          LDA.B GraphicsCompPtr+1
                      SEC
                      SBC.B #$05                                ; subtract 5 from Y-pos if on land
                      STA.B GraphicsCompPtr+1                   ; $8B = 2P Y-pos on screen
                      STA.B _7                                  ; $07 = 2P Y-pos on screen
CODE_048739:          REP #$30                                  ; AXY->16
...
CODE_048786:          SEP #$30                                  ; AXY->8
                      RTS

CODE_048789:          LDA.B GraphicsCompPtr                     ; A = Y-pos on screen | X-pos on screen
                      PHA
                      PHX                                       ; X = player x #$20
                      LDA.B _4                                  ; A = player x #$200
                      XBA                                       ; A = player x 2
...
OverworldMusic:       db !BGM_DONUTPLAINS
                      db !BGM_YOSHISISLAND
                      db !BGM_VANILLADOME
...
CODE_048DDF:          LDA.W CutsceneID
                   ...
                   if ver_is_english(!_VER)                     ;\================ U, SS, E0, & E1 ==============
                      ...
                      LDY.W #$0014
CODE_048E25:          LDA.W TranslevelNo
                      AND.W #$00FF
                      CMP.W DATA_048D74,Y
                      BEQ CODE_048E38
                      DEY
                      DEY
                      BPL CODE_048E25
CODE_048E34:          SEP #$30                                  ; AXY->8
                      BRA +

CODE_048E38:          SEP #$30                                  ; AXY->8
                      LDX.W PlayerTurnLvl
                      LDA.W OWPlayerSubmap,X
                      TAX
...
ADDR_048ED9:          LDA.W OWLevelExitMode
                      BMI CODE_048EE1
                      STZ.W OWSpriteNumber,X
CODE_048EE1:          REP #$30                                  ; AXY->16
                      JSR OWMoveScroll
                      SEP #$30                                  ; AXY->8
                      JSR DrawOWBoarder_
                      JSR CODE_048086
...
CODE_048F56:          SEP #$20                                  ; A->8
                      LDA.B #$05
                      STA.W OverworldProcess
                      BRA CODE_048F7A

CODE_048F5F:          REP #$20                                  ; A->16
                      LDX.B _4
                      LDA.L OWLayer1Translevel,X
                      AND.W #$00FF
                      TAX
...
CODE_048F7A:          REP #$30                                  ; AXY->16
                      JMP OWMoveScroll


DATA_048F7F:          db $58,$59,$5D,$63,$77,$79,$7E,$80

CODE_048F87:          JSR CODE_049903
                   if ver_is_console(!_VER)                     ;\================= J, U, E0, & E1 ==============
                      LDX.B #$07                                ;!
CODE_048F8C:          LDA.W OverworldLayer1Tile                 ;!
                      CMP.W DATA_048F7F,X                       ;!
                      BNE CODE_049000                           ;!
                      LDX.B #$2C                                ;!
...
CODE_048FFB:          INC.W ShowSavePrompt
                      BRA CODE_049003
...
OWHardCodedTiles:
OWPathDP2_DP1:        db $10,$10,$1E,$19,$16,$66
OWPathDP1_DP2:        db $16,$19,$1E,$10,$10,$66
OWPathCI3_CF:         db $04,$04,$04,$58
OWPathCF_CI3:         db $04,$04,$04,$66
OWPathFoI4_FoI2:      db $04,$04,$04,$04,$04,$6A
OWPathFoI2_FoI4:      db $04,$04,$04,$04,$04,$66
OWPathCI2_Pipe:       db $1E,$19,$06,$09,$0F,$20,$1A,$21
                      db $1A,$14,$19,$18,$1F,$17,$82
OWPathPipe_CI2:       db $17,$1F,$18,$19,$14,$1A,$21,$1A
                      db $20,$0F,$09,$06,$19,$1E,$66
OWPathStar_FD:        db $04,$04,$58
OWPathFD_Star:        db $04,$04,$5F

OWHardCodedDirs:
OWDirDP2_DP1:         db $02,$02,$02,$02,$06,$06
OWDirDP1_DP2:         db $04,$04,$00,$00,$00,$00
OWDirCI3_CF:          db $04,$04,$04,$04
OWDirCF_CI3:          db $06,$06,$06,$06
OWDirFoI4_FoI2:       db $06,$06,$06,$06,$06,$06
OWDirFoI2_FoI4:       db $04,$04,$04,$04,$04,$04
OWDirCI2_Pipe:        db $02,$02,$06,$06,$00,$00,$00,$04
                      db $00,$04,$04,$00,$04,$00,$04
OWDirPipe_CI2:        db $06,$02,$06,$02,$06,$06,$02,$06
                      db $02,$02,$02,$04,$04,$00,$00
OWDirStar_FD:         db $06,$06,$06
OWDirFD_Star:         db $04,$04,$04

DATA_04910E:          db OWPathDP2_DP1-OWHardCodedTiles
...
DATA_049118:          db $08,$00,$04,$00,$02,$00,$01,$00

CODE_049120:          STZ.W PlayerSwitching
                      ...
                      BEQ CODE_049132
                      BRL CODE_0491E9
CODE_049132:          LDA.B byetudlrFrame
                      AND.B #$20
...
OWPU_NotOnStar:       LDA.W OverworldLayer1Tile                 ; \
...
CODE_0491E9:          REP #$20                                  ; A->16
                      ...
                      AND.W #$00FF                              ; /
                      LDY.W #$000A
CODE_04922A:          CMP.W DATA_04906C,Y
                      BNE CODE_04923B
                      LDA.W #$0005
                      STA.W OverworldProcess
                      JSR CODE_049037
...
CODE_04926E:          DEC.W Layer1ScrollXPosUpd                 ; \ Decrease "Face walking dir" timer
...
CODE_0492FE:          BRL CODE_049384
CODE_049301:          STZ.W HardcodedPathIsUsed
                      ...
                      STA.B _0
                      LDX.W #$0009
CODE_049315:          LDA.W HardCodedOWPaths,X
                      AND.W #$00FF
                      CMP.W #$00FF
                      BNE CODE_049349
                      PHX
...
CODE_049349:          CMP.B _0
                      BNE CODE_049374
CODE_04934D:          STX.B _0
                      LDA.W DATA_04910E,X
                      AND.W #$00FF
                      TAX
                      DEC A
...
CODE_049374:          DEX
                      BMI CODE_04937A
                      BRL CODE_049315
CODE_04937A:          SEP #$20                                  ; A->8
                      STZ.W OWLevelExitMode
                      REP #$20                                  ; A->16
                      JMP CODE_049411

CODE_049384:          STA.W OverworldLayer1Tile
                      STA.B _0
                      STZ.B _2
                      LDX.W #$0017
CODE_04938E:          LDA.W DATA_04A03C,X
                      AND.W #$00FF
                      CMP.B _0
                      BNE CODE_0493B5
                      LDA.W DATA_04A0E4,X
...
CODE_049411:          JMP OWMoveScroll


DATA_049414:          db $0D,$08

OWScrollLowerBound:   db $EF,$FF,$D7,$FF

OWScrollUpperBound:   db $11,$01,$31,$01
...
CODE_04945D:          LDA.W PlayerSwitching
                      BEQ +
                      LDA.B #$08
                      STA.W OverworldProcess
                      RTS
                    ...
                    + REP #$30                                  ; AXY->16
                      ...
                      TAY
                      LDX.W #$0002
CODE_049475:          LDA.W OverworldDestXPos,Y
                      SEC
                    ...
                    + REP #$20                                  ; A->16
                      ...
                      STA.B _A
                      LDX.W #$0002
CODE_0494F0:          LDA.B _8
                      BMI CODE_0494F8
                      LDA.B _A
                      BRA +

CODE_0494F8:          LDA.B _E
                    + BIT.B _0,X
                      BPL +
                      EOR.W #$FFFF
                      INC A
...
ADDR_049575:          LDA.B _8
                      ...
                      BEQ ADDR_049575
                      TAY
CODE_049582:          LDX.W PlayerTurnOW
                      LDA.W OWPlayerXPosPtr,X
                      STA.B _0
                      LDA.W OWPlayerYPosPtr,X
                      STA.B _2
                    ...
                    + LDA.B _0,X
                      ...
                      CMP.W #$0058
                      BNE CODE_0495DE
CODE_0495CE:          LDX.B _4
                      BMI ADDR_049575
                      CMP.W #$0800
                      BCS ADDR_049575
                      LDA.L Map16TilesLow,X                     ; \ Load OW tile number
                      AND.W #$00FF                              ; /
CODE_0495DE:          STA.W OverworldLayer1Tile                 ; Set "Current OW tile"
                      BEQ ADDR_049575
                      CMP.W #$0087
                      BCS ADDR_049575
                      PHA
                    ...
                    + STY.W OverworldClimbing
                      ...
                      SEP #$30                                  ; AXY->8
                      LDX.B #$09
CODE_049616:          CMP.W DATA_049426,X
                      BNE CODE_049645
                      PHY
                      JSR CODE_049A24
                      PLY
...
CODE_049648:          REP #$30                                  ; AXY->16
                    ...
                    + CMP.W #$0080
                      ...
                      CMP.W #$006E
                      BCS CODE_049676
CODE_049663:          LDA.W PlayerTurnOW
                      LSR A
                      AND.W #$0002
                      TAX
                      LDA.W OWPlayerAnimation,X
...
CODE_049676:          LDA.W PlayerTurnOW
                      LSR A
                      AND.W #$0002
                      TAX
                      LDA.W OWPlayerAnimation,X
                    ...
                    + LDA.W PlayerTurnOW
                      ...
                      STA.B _0
                      LDX.W #$0017
CODE_0496F2:          LDA.W DATA_04A03C,X
                      AND.W #$00FF
                      CMP.B _0
                      BNE CODE_049704
                      TXA
...
CODE_04971D:          DEC A
                    ...
                    + STA.B _0
                      ...
                      SEP #$20                                  ; A->8
                      LDX.W #$001C
CODE_049730:          CMP.W DATA_049430,X
                      BEQ CODE_04973B
                      DEX
                      DEX
                      BPL CODE_049730
...
CODE_0497E3:          LDA.W OverworldDestYPos,X
                      AND.B #$0F
                      BNE +
                      LDY.W #$0000
                      LDA.B _1
...
CODE_049801:          REP #$20                                  ; A->16
                      LDA.W PlayerTurnOW
                      CLC
                      ADC.W #$0002
                      TAX
                      LDY.W #$0002
CODE_04980E:          LDA.W Layer3ScrollType,Y
                      AND.W #$00FF
                      CLC
...
OWScrollNoChecks:     REP #$30                                  ; AXY->16
                      ...
                      LDX.W #$0002
                      TXY
CheckOWScrollBounds:  LDA.B _0,X
                      SEC
                      SBC.W #$0080
                      BPL ++
...
OWCancelMoveScroll:   SEP #$30                                  ; AXY->8
                      RTS

OW_TilePos_Calc:      LDA.B _0                                  ; Get overworld X pos/16 (X)
                      AND.W #$000F                              ; \
                      STA.B _4                                  ; |
                      LDA.B _0                                  ; |
                      AND.W #$0010                              ; |
...
CODE_0498C6:          STZ.W OWPlayerAnimation
                      LDA.B #$80
                      CLC
...
CODE_049903:          LDX.W OWLevelExitMode
                      BEQ Return0498C5
                      BMI Return0498C5
                      DEX
                      LDA.W DATA_049060,X
...
CODE_049A24:          REP #$20                                  ; A->16
                      ...
                      LDY.B #$41
                      LDX.W PlayerTurnOW
CODE_049A3F:          LDA.W OWPlayerYPos,X
                      CMP.W DATA_049964,Y
                      BNE CODE_049A85
                      LDA.W OWPlayerXPos,X
                      CMP.W DATA_049966,Y
...
CODE_049A85:          DEC.B _2
                      ...
                      DEY
                      BPL CODE_049A3F
CODE_049A90:          SEP #$20                                  ; A->8
                      RTS

CODE_049A93:          LDA.W PlayerTurnOW
                      AND.W #$00FF
                      LSR A
                      LSR A
                      TAX
...
LevelNameStrings:     db $47,$5A,$4A,$4B,$5A,$11,$82,$64        ;!
...
DATA_049CED:          db $CB,$01,$9D,$01,$9E,$01,$9F,$01        ;!
                   ...
                   if ver_is_japanese(!_VER)                    ;\======================== J ====================
CODE_049D07:          LDA.L DynStripeImgSize                    ;!
                      TAX                                       ;!
                      CLC                                       ;!
                      ADC.W #$0020                              ;!
                      STA.B _2                                  ;!
                    ...
                    + LDA.B #$FF                                ;!
                      ...
                      RTS                                       ;!
																;!
CODE_049D7F:          LDA.W LevelNameStrings,Y                  ;!
                      PHP                                       ;!
                      CPX.B _2                                  ;!
                      BCS +++                                   ;!
                      AND.B #$7F                                ;!
...
CODE_049D07:          LDA.L DynStripeImgSize                    ;!
                    ...
                    + REP #$20                                  ;! A->16
                      ...
                      SEP #$20                                  ;! A->8
                      JSR CODE_049D7F                           ;!
CODE_049D6A:          CPX.B _2                                  ;!
                      BCS CODE_049D76                           ;!
                      LDY.W #$01CB                              ;!
                      JSR CODE_049D7F                           ;!
                      BRA CODE_049D6A                           ;!
																;!
CODE_049D76:          LDA.B #$FF                                ;!
                      STA.L DynamicStripeImage+4,X              ;!
                      REP #$20                                  ;! A->16
                      RTS                                       ;!
																;!
...
CODE_049D9A:          LDA.W IsTwoPlayerGame
                      BEQ CODE_049DAF
                      LDA.W PlayerTurnLvl
                      EOR.B #$01
                      TAX
...
CODE_049DD1:          LDA.W PlayerTurnLvl
                      EOR.B #$01
                      STA.W PlayerTurnLvl
                      TAX
...
CODE_049E22:          DEC.W KeepModeActive
                      BPL +
                      LDA.B #$02
                      STA.W KeepModeActive
...
CODE_049E4C:          LDA.B #$03
                      STA.W OverworldProcess
                      RTS

CODE_049E52:          LDA.W StarWarpLaunchSpeed
                      BNE CODE_049E63
...
CODE_049E63:          LDA.B TrueFrame
                      AND.B #$07
                      BNE +
CODE_049E69:          INC.W StarWarpLaunchSpeed
...
DATA_04A0E4:          db $02,$02,$02,$02,$02,$00,$02,$02
                   ...
                   if ver_is_japanese(!_VER)                    ;\======================= J =====================
LevelNames:           db $00,$00,$52,$44,$53,$44,$90,$22        ;!
...
OWBorderStripe:       db $50,$00,$41,$3E,$FE,$38,$50,$A0
...
OWTileNumbers:        db $9B,$75,$81,$20,$01
...
CODE_04D770:          STA.L Map16TilesHigh,X
...
CODE_04DA49:          REP #$30                                  ; AXY->16
                      ...
                      STA.B _6
                      LDA.B [_4],Y
CODE_04DA83:          CMP.L DATA_04DA1D,X
                      BEQ CODE_04DA8F
                      DEX
                      BPL CODE_04DA83
                      JMP CODE_04DA9D

CODE_04DA8F:          LDA.L DATA_04DA33,X
                      STA.B [_4],Y
                      CPX.W #$0015
                      BNE CODE_04DA9D
                      INY
                      STA.B [_4],Y
CODE_04DA9D:          LDA.B _F
                      JSR CODE_04E677
                      SEP #$10                                  ; XY->8
...
DATA_04DAB3:          db $01,$18
                      dl OWLayer2Tilemap
                      dw $2000

CODE_04DABA:          SEP #$20                                  ; A->8
                      REP #$10                                  ; XY->16
                      LDA.B [_0],Y
                      STA.B _3
                      AND.B #$80
...
CODE_04DB18:          REP #$20                                  ; A->16
                      ...
                      EOR.W DATA_04DB10,X
                      BMI +
CODE_04DB43:          %LorW_X(LDA,DATA_04DB0C)
                      STA.W OWTransitionXCalc
...
CODE_04DB7A:          CPX.B _0
                    ...
                    + STA.W WindowTable+$4E,Y
                      ...
                      STA.B Layer12Window
                      LDA.B #$33
CODE_04DB95:          STA.B OBJCWWindow
                      LDA.B #$80
                      STA.W HDMAEnable
                      RTS

CODE_04DB9D:          LDA.W PlayerTurnOW
                      LSR A
                      LSR A
                      TAX
                      LDA.W OWPlayerSubmap,X
...
OverworldMusic2:      db !BGM_DONUTPLAINS
                      db !BGM_YOSHISISLAND
                      db !BGM_VANILLADOME
                      db !BGM_FORESTOFILLUSION
                      db !BGM_VALLEYOFBOWSER
...
CODE_04DBCF:          STZ.W OWSubmapSwapProcess
                      LDA.B #$04
                      STA.W OverworldProcess
                      LDA.W PlayerTurnOW
                      LSR A
                      ...
                      BEQ +
CODE_04DBF3:          LDA.W OWPlayerSubmap,Y
                      TAX
...
CODE_04E453:          SEP #$30                                  ; AXY->8
                    ...
                    - JSR CODE_04E496
                      ...
                      CMP.W EventLength
                      BNE -
CODE_04E493:          SEP #$30                                  ; AXY->8
                      RTS

CODE_04E496:          REP #$30                                  ; AXY->16
                      LDA.W EventTileIndex
                      ASL A
                      ASL A
                      TAX
...
CODE_04E4A9:          SEP #$20                                  ; A->8
                    ...
                    + JSR CODE_04E520
CODE_04E4CD:          SEP #$30                                  ; AXY->8
                      RTS

CODE_04E4D0:          LDA.W #$0001
                      STA.B _0
CODE_04E4D5:          LDX.B _4
                      LDA.W #$0001
                      STA.B _C
CODE_04E4DC:          SEP #$20                                  ; A->8
                      LDA.B [_9],Y
                      STA.L OWLayer2Tilemap,X
                      INX
                      LDA.B [_6],Y
...
CODE_04E520:          LDA.W #$0005
                      STA.B _0
CODE_04E525:          LDX.B _4
                      LDA.W #$0005
                      STA.B _C
CODE_04E52C:          SEP #$20                                  ; A->8
                      LDA.B [_9],Y
                      STA.L OWLayer2Tilemap,X
                      INX
                      LDA.B [_6],Y
...
CODE_04E570:          LDA.W OverworldEventProcess
                      JSL ExecutePtr
...
CODE_04E5EE:          LDA.W OWLevelExitMode
                      CMP.B #$02
                      BNE +
                    ...
                    + LDA.W CreditsScreenNumber
                      ...
                      AND.L DATA_04E44B,X
                      BEQ CODE_04E640
CODE_04E61A:          LDX.B #$07
CODE_04E61C:          LDA.W DATA_04E5E6,X
...
CODE_04E640:          INC.W OverworldEventProcess
                      LDA.W OverworldEvent
                      JSR CODE_04E677
                      TYA
                      ASL A
...
CODE_04E677:          SEP #$30                                  ; AXY->8
                      LDX.B #$17
CODE_04E67B:          CMP.L DATA_04E5D6,X
                      BEQ CODE_04E68A
                      DEX
                      BPL CODE_04E67B
CODE_04E684:          LDA.B #$02
                      STA.W OverworldEventProcess
                      RTS

CODE_04E68A:          STX.W StructureCrushIndex
                      TXA
                      ASL A
                      TAX
                      LDA.B #$7E
                      ...
                      LDX.W #$0004
                      LDA.B [_A],Y
CODE_04E6A7:          CMP.L DATA_04E5A7,X
                      BEQ CODE_04E6B3
                      DEX
                      BPL CODE_04E6A7
                      JMP CODE_04E684
...
CODE_04E6D3:          INC.W OverworldEventProcess
                      LDA.W OverworldEvent
                      ASL A
                      TAX
                      REP #$20                                  ; A->16
...
CODE_04E76C:          LDA.W #$0001
                      STA.B _6
                      LDA.L DynStripeImgSize
                      TAX
CODE_04E776:          LDA.B _2
                      STA.L DynamicStripeImage,X
                      INX
                      INX
                      LDY.W #$0300
...
CODE_04E824:          LDA.W #$0005
                      STA.B _6
                      LDA.L DynStripeImgSize
                      TAX
CODE_04E82E:          LDA.B _2
                      STA.L DynamicStripeImage,X
                      INX
                      INX
                      LDY.W #$0B00
...
CODE_04E9EC:          LDA.W OverworldEvent
                      STA.B _F
CODE_04E9F1:          LDX.B #$2B
CODE_04E9F3:          CMP.L DATA_04E8E4,X
                      BEQ CODE_04EA25
CODE_04E9F9:          DEX
                      BPL CODE_04E9F3
                      LDA.W OverworldEventProcess
...
CODE_04EA5A:          SEP #$30                                  ; AXY->8
                      PLX
                      LDA.B _F
                      JMP CODE_04E9F9

CODE_04EA62:          STZ.W ColorFadeTimer
                      STZ.W ColorFadeDir
                      LDX.B #$6F
                    - LDA.W MainPalette,X
...
CODE_04EAA4:          LDA.W ColorFadeTimer
                      CMP.B #$40
                      BCC CODE_04EAC9
                      INC.W OverworldEventProcess
                      JSR CODE_04EE30
...
CODE_04EBEB:          DEC.W OverworldEventSize
                      BPL +
                      INC.W OverworldEventProcess
                      RTS
...
CODE_04EC67:          LDA.W OverworldEventXPos
                      SEC
                      SBC.B Layer2XPos
                      STA.B _0
...
CODE_04EC78:          LDA.B #$7E
                      ...
                      SEP #$20                                  ; A->8
                      LDA.B [_D],Y
CODE_04EC97:          CMP.L DATA_04DA1D,X
                      BEQ CODE_04ECA8
                      DEX
                      BPL CODE_04EC97
                      SEP #$10                                  ; XY->8
...
CODE_04ED83:          LDA.B #Map16TilesLow>>16
                      STA.B _F
                      REP #$30                                  ; AXY->16
                      LDA.W #Map16TilesLow
                      STA.B _D
                      ...
                      SEP #$20                                  ; A->8
                      LDA.B [_D],Y
CODE_04EDA2:          CMP.L DATA_04DA1D,X
                      BEQ CODE_04EDAB
                      DEX
                      BNE CODE_04EDA2
CODE_04EDAB:          REP #$30                                  ; AXY->16
                      STX.B _E
                      LDA.W OverworldEvent
                      AND.W #$00FF
                      ASL A
...
CODE_04F290:          LDY.W KeyholeYPos+1
                      CPY.B #$0C
                      BCC +
                    ...
                    + STA.W KeyholeYPos
                      LDX.B #$00
CODE_04F309:          STX.W KeyholeXPos
CODE_04F30C:          LDA.B #$10
                      STA.W KeyholeXPos+1
                      INC.W KeyholeYPos+1
CODE_04F314:          DEC.W KeyholeXPos+1
                      LDA.W KeyholeYPos
                      STA.B _F
                      LDX.B #$00
CODE_04F31E:          PHX
...
CODE_04F407:          STZ.B Layer12Window
                      STZ.B Layer34Window
                      STZ.B OBJCWWindow
                      STZ.W HDMAEnable
                      RTS
...
DATA_04F411:          db $04,$FC
                   ...
                   if ver_is_japanese(!_VER)                    ;\======================= J =====================
DATA_04F413:          db $48,$00                                ;!
                   else                                         ;<================ U, SS, E0, & E1 ==============
DATA_04F413:          db $68,$00                                ;!
                   endif                                        ;/===============================================

CODE_04F415:          LDX.B #$00
...
CODE_04F44B:          CLC
                   ...
                   if ver_is_japanese(!_VER)                    ;\======================== J ====================
ClearOWBoxStripe:     db $51,$C9,$40,$14,$FC,$38,$52,$08        ;!
                      db $40,$1E,$FC,$38,$52,$2F,$40,$02        ;!
                      db $FC,$38,$52,$48,$40,$1C,$FC,$38        ;!
                      db $FF                                    ;!
                   else                                         ;<================ U, SS, E0, & E1 ==============
ClearOWBoxStripe:     db $51,$C4,$40,$24,$FC,$38,$52,$04        ;!
...
CODE_04F513:          LDA.W byetudlrP1Frame
...
OverworldSprites:     db $00 : dw $0100,$00E0
                      db $00 : dw $0100,$0060
                      db $06 : dw $0170,$0020
                      db $07 : dw $0038,$018A
                      db $00 : dw $0058,$007A
...
ExtraOWGhostXPos:     dw $0030,$0100,$FF10
ExtraOWGhostYPos:     dw $0020,$FF70,$0010

DATA_04F672:          db $01,$40,$80
...
CODE_04F675:          PHB
                      ...
                      LDX.B #$0C
                      LDY.B #$4B
CODE_04F67C:          LDA.W OverworldSprites-$0F,Y
...
ADDR_04F68A:          LDA.B #$40
                    ...
                    + LDA.W OverworldSprites-$0E,Y
                      ...
                      BPL CODE_04F67C
                      LDX.B #$0D
CODE_04F6B1:          STZ.W OWSpriteMisc0E25,X
                      LDA.W DATA_04FD22
                      DEC A
...
CODE_04F708:          LDA.B #$F7
                      ...
                      LDA.B #!SFX_THUNDER
                      STA.W SPCIO3                              ; / Play sound effect
CODE_04F73B:          DEC.W LightningTimer
                      BPL +
...
CODE_04F76E:          LDX.B #$02
CODE_04F770:          LDA.W OWSpriteNumber,X
                      BNE +
                      LDA.B #$05
                    ...
                    - TXA
                      ...
                      BPL -
                      LDX.B #$04
CODE_04F7B9:          STX.B _0
CODE_04F7BB:          STX.B _1
                      LDX.B _0
...
CODE_04F87C:          LDY.W OWSpriteNumber,X
                      LDA.W Return04F828,Y
CODE_04F882:          STA.B _0
                      ...
                      CPY.B #$01
                      BNE CODE_04F8A3
CODE_04F892:          LDA.W PlayerTurnOW
                      LSR A
                      LSR A
                      TAY
                      LDA.W OWPlayerSubmap,Y
...
ADDR_04F8E8:          STX.B _6
                    ...
                    + JSR ADDR_04FEEF
                      ...
                      LDA.W #$0040
                      BCS +
ADDR_04F95D:          LDA.W OWSpriteMisc0E05-1,X
                      EOR.B _2
                      ASL A
                      BCC +
                      LDA.B _8
...
CODE_04FA3E:          LDA.W OWSpriteMisc0DF5,X
                      BNE CODE_04FA83
                      LDA.W OverworldLayer1Tile
                      SEC
                      SBC.B #$4E
                      ...
                      LDA.B #!SFX_SWIM
                      STA.W SPCIO0                              ; / Play sound effect
CODE_04FA7D:          LDA.B #$0F
                      STA.W OWSpriteMisc0E25,X
Return04FA82:         RTS

CODE_04FA83:          DEC.W OWSpriteZSpeed,X
                      LDA.W OWSpriteZSpeed,X
                      CMP.B #$E4
                      BNE +
                      JSR CODE_04FA7D
...
CODE_04FB06:          SEC
                      LDY.W DATA_04F843,X
CODE_04FB0A:          STA.W OAMTileNo+$40,Y                     ;Tilemap
                      XBA
...
CODE_04FB37:          LDA.B #$02                                ;\Overworld Sprite X Speed
                      STA.W OWSpriteXSpeed,X                    ;/
...
CODE_04FB77:          XBA
                      LDA.B #$44
CODE_04FB7A:          SEC
CODE_04FB7B:          LDY.W OWCloudOAMIndex
                      JSR CODE_04FB0A
...
CODE_04FB98:          LDA.W OWSpriteMisc0DF5,X
                      BNE ADDR_04FBD8
                      LDA.W OverworldLayer1Tile
                      SEC
                      SBC.B #$49
...
CODE_04FC46:          LDA.W PlayerTurnOW
                      LSR A
                      LSR A
                      TAY
                      LDA.W OWPlayerSubmap,Y
...
CODE_04FD24:          JSR CODE_04FE90
                    ...
                    + XBA
                      ...
                      JSR CODE_04FD55
                      LDY.W OWSpriteMisc0DF5,X
CODE_04FD55:          LDA.W OWSpriteXSpeed,X
                      CLC
                      ADC.W DATA_04FD10,Y
                      STA.W OWSpriteXSpeed,X
                      CMP.W DATA_04FD1A,Y
...
CODE_04FDA5:          LDA.B #$34
                    ...
                    + XBA
                      ...
                      JSR CODE_04FDD2
                      LDA.B #$04
CODE_04FDD2:          ORA.W OWSpriteMisc0DF5,X
                      TAY
...
CODE_04FE62:          TXA
                      ...
                      DEC.B _3
                    + LDY.B #$00
CODE_04FE7D:          LDA.W OWSpriteXPosHigh,X
                      XBA
                      LDA.W OWSpriteXPosLow,X
                      REP #$20                                  ; A->16
                      SEC
...
CODE_04FE90:          TXA                                       ;Transfer X to A
                    ...
                    + TXA                                       ;Transfer X to A
                      ...
                      JSR CODE_04FEAB
                      LDX.W SaveFileDelete
CODE_04FEAB:          LDA.W OWSpriteXSpeed,X                    ;Load OW Sprite X Speed
...
ADDR_04FEEF:          LDA.W OWSpriteXPosHigh,X
                      XBA
                      LDA.W OWSpriteXPosLow,X
                      REP #$20                                  ; A->16
                      CLC
...
ADDR_04FF2E:          JSR ADDR_04FEEF
                    ...
                    + STY.B _C
                      LDX.B #$04
ADDR_04FF6B:          CPX.B _C
                      BNE ADDR_04FF73
                      LDA.B #$20
                      BRA +

ADDR_04FF73:          STZ.W HW_WRDIV
                      LDA.B _6,X
                      STA.W HW_WRDIV+1
                      LDA.W _6,Y
                      STA.W HW_WRDIV+2
